...
for (const id of ids) {
  ...
  let n = 0, w = 0;
  for (const k of KAT) for (const x of b[k] || []) {
    n++;
    w += String(x).split(/\s+/).length;
  }
  je.push(n);
  sum += n;
  woerter += w;
...
je.sort((a, b) => a - b);
console.log(`${ids.length} Presets \xB7 ${sum} Eintr\xE4ge gesamt \xB7 ${woerter} W\xF6rter`);
console.log(`je Preset: kleinstes ${je[0]}, Median ${je[Math.floor(je.length / 2)]}, gr\xF6\xDFtes ${je[je.length - 1]}`);
console.log(`Kategorien je Preset: ${KAT.join(", ")}`);
var b0 = BUILTIN_PRESETS[ids[0]];
console.log("\nBeispiel " + ids[0] + ":");
for (const k of KAT) console.log("  " + k.padEnd(11) + (b0[k] || []).length);
