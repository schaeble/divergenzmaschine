...
wahr("subjektlose Inversion f\xE4llt", !satzPlausibel("Im Sommer ohne Nacht folgte der Schritt, \xFCber den Wipfeln liegt wie Wasser."));
for (const [name, satz] of [
  ...
  ["PP-Fragment", "Am Vorabend einer Abreise in einer W\xFCste mit T\xFCren."]
]) wahr(`bleibt: ${name}`, satzPlausibel(satz), satz);
{
  let n = 0;
  const durch = [];
