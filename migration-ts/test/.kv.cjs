...
function istEigenePerson(teil2) {
  const p = clean(teil2);
  if (!p) return false;
...
function kurzform(haupt, genus) {
  ...
  const teil2 = letzt.includes("-") ? letzt.split("-").pop() : letzt;
  return `${art} ${teil2}`;
}
...
function polishGerman(text2, opts = {}) {
  ...
  t = t.split(/(?<=[.!?…])(\s+)/).map((teil2) => /^\s+$/.test(teil2) ? teil2 : ergaenzeArtikel(teil2)).join("");
  return t.trim();
}
...
var teil = (anteil) => {
  const b = {};
  for (const k of KAT) b[k] = phil[k].slice(0, Math.max(3, Math.round(phil[k].length * anteil)));
  return b;
};
console.log("\u2500\u2500 Wie gro\xDF muss ein Preset sein? (dasselbe Material, nur weniger davon) \u2500\u2500");
console.log("  Eintr\xE4ge   Bericht Ziel 450   Prosa Ziel 400");
for (const anteil of [0.3, 0.45, 0.6, 0.75, 0.9, 1]) {
  const b = teil(anteil);
  const n = KAT.reduce((s, k) => s + b[k].length, 0);
  let wb = 0;
  const N2 = 30;
  for (let i = 0; i < N2; i++) wb += W(text(buildBericht(b, {
  ...
  let wp = 0;
  for (let i = 0; i < N2; i++) wp += W(buildStory(b, {
    where: "im H\xF6rsaal",
    when: "am Abend",
    who: "die Denkerin",
  ...
  console.log(`  ${String(n).padStart(8)}   ${String(Math.round(wb / N2)).padStart(7)} (${String(Math.round(100 * wb / N2 / 450)).padStart(3)}%)   ${String(Math.round(wp / N2)).padStart(6)} (${String(Math.round(100 * wp / N2 / 400)).padStart(3)}%)`);
}
