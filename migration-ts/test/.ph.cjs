...
for (const k of KAT) {
  n += b[k].length;
  w += b[k].reduce((a, x) => a + x.split(/\s+/).length, 0);
  console.log("  " + k.padEnd(11) + b[k].length);
}
console.log(`Philosophie: ${n} Eintr\xE4ge, ${w} W\xF6rter \xB7 Verwandlungen: ${(b.verwandlungen || []).length}`);
