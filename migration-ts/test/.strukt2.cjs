...
var BAUER = [
  ...
  /Sie nennen es \w+\. Ich nenne es/,
  /hatte .{0,40} schon in der Hand, denn/,
  /greift nach dem, was bleibt, und/,
  /legt .{0,40} beiseite und/,
  /nimmt .{0,40} und (tritt näher|fragt nach|hält den Blick aus)/
];
console.log("\u2500\u2500 Wie oft steht ein Satz im Text, den der BAUER fest vorgibt? \u2500\u2500");
console.log("   (nur Wendungen, die es in der Rekombination nicht gibt)\n");
for (const stk of STRUK) {
  let mit = 0, treffer = 0, n = 0;
  const anfaenge = /* @__PURE__ */ new Set();
  let saetze = 0;
  for (let i = 0; i < 120; i++) {
    ...
    n++;
    const k = BAUER.filter((r) => r.test(t)).length;
    if (k) mit++;
    treffer += k;
    for (const s of t.split(/(?<=[.!?…])\s+/)) {
      const w = s.trim().split(/\s+/).slice(0, 3).join(" ").toLowerCase();
      if (w) {
        anfaenge.add(w);
        saetze++;
      }
  ...
  console.log(`  ${stk.padEnd(14)} Texte mit fester Wendung ${String(Math.round(100 * mit / n)).padStart(3)} %   je Text ${(treffer / n).toFixed(1)}   verschiedene Satzanf\xE4nge ${anfaenge.size} von ${saetze} S\xE4tzen (${Math.round(100 * anfaenge.size / saetze)} %)`);
}
