...
var RE = /(^|\s)(ein|eine|einem|einen|einer|eines|der|die|das|dem|den|des|und|oder|aber|wie|mit|an|auf|zu|im|am|vor|nach|für|ohne|als|bei|aus|ist|sind|wird)$/i;
var n = 0;
var tr = 0;
var bsp = [];
for (const id of Object.keys(BUILTIN_PRESETS)) {
  ...
  for (const k of Object.keys(b)) if (Array.isArray(b[k])) for (const s of b[k]) {
    ...
    if (RE.test(bare)) {
      tr++;
      if (bsp.length < 8) bsp.push(id + " \xB7 " + s);
    }
  }
}
console.log("Bank-S\xE4tze bis 12 W\xF6rter:", n, "| enden auf Funktionswort:", tr, `(${(100 * tr / n).toFixed(1)} %)`);
