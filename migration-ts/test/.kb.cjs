// src/text-utils.ts
var HAENGT_IN_DER_LUFT = /(^|\s)(ein|eine|einem|einen|einer|eines|der|die|das|dem|den|des|und|oder|aber|wie|als|im|am|bei|für|ohne|mit|an|auf|zu|zum|zur|von|vom|aus|nach|vor|über|unter|durch|gegen|seit|um|beim|dass|weil|denn|sondern|sowie|bzw|etwa|nur|noch|sehr|dessen|deren|welche[rsmn]?)$/i;
function kuerzeAmBruch(text) {
  ...
  if (HAENGT_IN_DER_LUFT.test(t) || t.split(/\s+/).filter(Boolean).length < 5) return "";
  return t;
}
...
console.log("\u2500\u2500 kuerzeAmBruch \u2500\u2500");
for (const p of [
  "In der Toskana wird der italienische Festungsbaumeister und Milit\xE4r Rochus zu Lynar geboren, der insbesondere durch Bauten im Dienst deutscher F\xFCrsten wie der",
  "In der Toskana wird der italienische Festungsbaumeister und Milit\xE4r Rochus zu Lynar geboren, der insbesondere durch Bauten im Dienst deutscher F\xFCrsten wie der \u2026",
  "Die Archivarin sucht eine Akte",
  "Ein Mann geht, und",
  "wie der",
  "Der Flughafen Salzburg, Eigenbezeichnung Salzburg Airport \u2013 W.A."
]) console.log("  " + JSON.stringify(p.slice(0, 70)) + "\n    \u2192 " + JSON.stringify(kuerzeAmBruch(p)));
console.log("\n\u2500\u2500 wasPhrase (Kappung bei 170) \u2500\u2500");
console.log("  " + JSON.stringify(wasPhrase(LANG)));
console.log("\n\u2500\u2500 formeWas (rettet den Vorrat) \u2500\u2500");
console.log("  " + JSON.stringify(formeWas("In der Toskana wird der italienische Festungsbaumeister und Milit\xE4r Rochus zu Lynar geboren, der insbesondere durch Bauten im Dienst deutscher F\xFCrsten wie der")));
