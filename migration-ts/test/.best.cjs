...
var WER = [
  ...
  "das Stadttheater",
  "Dr. Ing. Richard Doll",
  "eine Grabr\xE4uberin mit Doktortitel",
...
for (const w of WER) {
  for (let i = 0; i < 20; i++) {
    const t = buildBericht(bank, {
      where: "in D\xFCrrhausen",
      when: "im Herbst 1923",
      who: w,
      what: "verfolgt eine L\xFCge",
      tone: "nuechtern",
      form: "bericht",
      lenTarget: 220,
      mode: "auto",
    ...
    const m = txt.match(/[^.]*besteht seit[^.]*\./);
    if (m && !/Werft|Ltd|Theater|GmbH|AG|Verein|Werk/i.test(w)) {
      falsch++;
      if (falsch <= 4) console.log("\u2717", w, "\u2192", m[0].trim());
    }
    if (i === 0) {
      const h = txt.match(/[^.]*(besteht seit|ist seit|reicht bis)[^.]*\./);
      console.log(String(w).padEnd(34), "\u2192", h ? h[0].trim() : "(kein Bestandssatz)");
    }
  }
}
console.log("\nfalsche Bestandss\xE4tze:", falsch, "von", WER.length * 20);
