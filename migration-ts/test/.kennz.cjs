...
function chooseInsertPos(sentences) {
  ...
  const candidates = [];
  for (let pos2 = 1; pos2 <= sentences.length; pos2++) {
    const prev = sentences[pos2 - 1];
    const next = sentences[pos2];
    ...
    candidates.push({ pos: pos2, weight: Math.max(1, w - 4) });
  }
...
function weaveMotif(text, motif) {
  ...
  let pos2 = chooseInsertPos(s);
  if (pos2 < 0) pos2 = Math.min(s.length - 1, Math.max(1, Math.floor(s.length * 0.55)));
  s.splice(pos2, 0, motifLine);
  return s.join(" ");
}
...
function weaveCast(text, _P, cast) {
  ...
  for (const b of beats) {
    ...
    let pos2 = chooseInsertPos(sent);
    if (pos2 < 0) pos2 = Math.min(sent.length, Math.max(1, Math.floor(sent.length * 0.5)));
    sent.splice(pos2, 0, line);
  }
  return sent.join(" ");
}
...
function applyRhythm(text, rhythm) {
  ...
  const insertFrag = (prob) => {
    if (chance(prob)) {
      const pos2 = chooseInsertPos(s);
      if (pos2 >= 0) s.splice(pos2, 0, pick(FRAGMENTS));
    }
  };
...
function applyTension(text, peak, material) {
  ...
  const width = 0.26;
  const intensity = (i, n) => {
    const pos2 = n <= 1 ? 0 : i / (n - 1);
    const d = (pos2 - center) / width;
...
function applyEmphasis(text, kit, w) {
  ...
  for (const line of uniq) {
    let pos2 = chooseInsertPos(sents);
    if (pos2 < 0) pos2 = sents.length;
    sents.splice(pos2, 0, line);
  }
  return sents.join(" ");
}
...
function applyStrangPoem(rawText, anchorLine = "", lenTarget = 0) {
  ...
  for (let k = 0; k < imgCount; k++) {
    ...
    const pos2 = chooseInsertPos(lines);
    if (pos2 >= 0) lines.splice(pos2, 0, img);
  }
...
var pos = [];
var wo = [];
for (const s of ["linear", "reverse", "circle", "fragment", "object"]) {
  for (let i = 0; i < 60; i++) {
    ...
    if (k >= 0) {
      pos.push(k / t.length);
      if (k / t.length < 0.5) wo.push(s + " @" + (100 * k / t.length).toFixed(0) + "% len=" + t.length);
    }
  }
}
pos.sort((a, b) => a - b);
console.log(
  ...
  pos.length,
  "| Median",
  (pos[Math.floor(pos.length / 2)] * 100).toFixed(0) + "%",
  "| Minimum",
  (pos[0] * 100).toFixed(0) + "%",
  "| unter 50 %:",
  pos.filter((x) => x < 0.5).length
);
wo.slice(0, 6).forEach((x) => console.log("   ", x));
