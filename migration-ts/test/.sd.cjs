...
var gross = ids.filter((i) => g(BUILTIN_PRESETS[i]) >= 120);
console.log(`${gross.length} von ${ids.length} Presets bei 120+ : ${gross.map((i) => i + " (" + g(BUILTIN_PRESETS[i]) + ")").join(", ")}`);
var rest = ids.filter((i) => !gross.includes(i)).map((i) => g(BUILTIN_PRESETS[i]));
console.log(`Rest: ${rest.length} Presets, Median ${rest.sort((a, b) => a - b)[Math.floor(rest.length / 2)]} Eintr\xE4ge`);
