...
function kurveWert(werte2, p) {
  const n2 = werte2.length;
  if (n2 === 0) return 0.3;
  if (n2 === 1) return klemm(werte2[0]);
  const x = klemm(p) * (n2 - 1);
  const i2 = Math.min(n2 - 2, Math.floor(x));
  const t = x - i2;
  return klemm(werte2[i2] * (1 - t) + werte2[i2 + 1] * t);
}
...
function toPresent(entry) {
  ...
  for (let i2 = 0; i2 < words.length; i2++) {
    ...
    if (base) {
      ...
      words[i2] = (/^[A-ZÄÖÜ]/.test(w) ? form.charAt(0).toUpperCase() + form.slice(1) : form) + zeichen;
      changed = true;
      continue;
    }
...
function hatFinitesVerb(seg) {
  ...
  for (let i2 = 0; i2 < ws.length; i2++) {
    ...
    const l = w.toLowerCase();
    const prev = (ws[i2 - 1] || "").toLowerCase(), next = ws[i2 + 1] || "";
...
function buildDramaturgie(kit) {
  ...
  const folge = d?.folge && d.folge.length && d.folge.every((n2) => SCHLAG_NAMEN.has(n2)) ? d.folge : SCHLAG_STANDARD;
  const beats = [];
...
function kernwoerter(satz) {
  ...
  woerter3.forEach((w, i2) => {
    const m = w.match(/^[„»(]?([A-ZÄÖÜ][a-zäöüß]{4,})/);
    if (!m) return;
    if (i2 === 0 || FUNKTION.test(m[1])) return;
...
function setzeEchos(text, bank, maxEchos = 2) {
  ...
  const n2 = saetze.length;
  if (n2 < 8) return { text, echos: [] };
  ...
  const schlussAb = Math.max(hoch, n2 - 2);
  const kandidaten = [];
  const genutzt = /* @__PURE__ */ new Set();
  for (let i2 = 1; i2 < Math.floor(n2 / 2); i2++) {
    for (const k of kernwoerter(saetze[i2])) {
      ...
      kandidaten.push({ i: i2, kern: k, echo: sorted[0] });
      genutzt.add(k);
  ...
  const gewaehlt = kandidaten.length <= maxEchos ? kandidaten : [kandidaten[0], kandidaten[Math.floor(kandidaten.length / 2)]].filter((x, i2, a) => a.indexOf(x) === i2);
  const out = [...saetze];
  const echos = [];
...
function normWho(s) {
  ...
  const fixed = parts.map((p, i2) => {
    ...
    if (i2 === 0 && /^[A-ZÄÖÜa-zäöüß][a-zäöüß-]+$/.test(p) && PERSON_NOMEN.test(p) && !/^(männer|leute)$/i.test(p)) {
      const wort = cap2(p);
...
function coherenceRepairV2(t, input) {
  ...
  String(input?.who || "").split(/[,;]/).map((x) => x.trim()).filter(Boolean).forEach((n2) => {
    ...
    const wieder = namensErsetzer(n2);
    try {
...
function istPluralFigur(who) {
  ...
  if (m) {
    const n2 = m[1].toLowerCase();
    if (/(innen|leute|kinder|eltern|geschwister|männer|frauen)$/.test(n2)) return true;
    return /en$/.test(n2) && !/(chen|lein)$/.test(n2);
  }
  return false;
}
function pluralKongruenz(t, who) {
  ...
  let out = t.replace(new RegExp(`(\\b${esc})\\s+([a-z\xE4\xF6\xFC\xDF]+t)\\b`, "giu"), (m, n2, v) => istVerbform(v) ? `${n2} ${beuge(v)}` : m);
  out = out.replace(new RegExp(`\\b([a-z\xE4\xF6\xFC\xDF]+t)\\s+(${esc})\\b`, "giu"), (m, v, n2) => istVerbform(v) ? `${beuge(v)} ${n2}` : m);
  return out;
}
...
function pruefeSatz(satz) {
  ...
  const hatVerb = ws.some((w, i2) => verbKandidat(w, i2 === 0));
  if (!hatVerb) {
  ...
  for (const teil of bare.split(/,\s*/).slice(1)) {
    ...
    const undIdx = tw.findIndex((w, i2) => i2 > 0 && /^(und|oder)$/i.test(w));
  ...
  for (const teil of bare.split(/,\s*/)) {
    ...
    const vi = tw.findIndex((w, i2) => i2 > 1 && verbKandidat(w, false));
    if (vi < 2) continue;
  ...
  for (const teil of bare.split(/[,;:—–]\s*/)) {
    ...
    for (let i2 = 0; i2 + 3 < tw.length; i2++) {
      ...
      if (HILFSVERB.has(tw[i2 + 3].toLowerCase())) return 6;
    }
  }
  {
...
function normalizePlace(W) {
  ...
  const n2 = normWhere(w);
  if (/^(im|am|in|auf|bei|unter|über|vor|hinter|an|zwischen|neben)\b/i.test(n2)) return n2;
  const m = n2.match(/^([A-ZÄÖÜ][a-zäöüß]+?)(e|er|es|en|em)\s+([A-ZÄÖÜ][a-zäöüß-]+)(.*)$/);
  if (m) {
...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank, tone = "neutral") {
  ...
  const shots = [];
  const kameras = [];
  for (let i2 = 0; i2 < shotCount; i2++) {
    const erster = i2 === 0, letzter = i2 === shotCount - 1, mitte = i2 === Math.floor(shotCount / 2);
    const teile = [];
    if (erster) {
    ...
    if (folge && schlaegeJeShot[i2].length) {
      const saetze = schlaegeJeShot[i2].map(schlagSatz).filter((x) => !!x && !/:\s*\.$/.test(x) && !/^(Nah|Regel|Es geht um|Etwas kippt)[: ]+\.$/.test(x) && x.replace(/[^A-Za-zÄÖÜäöüß]/g, "").length > 3);
    ...
    if (stufe >= 2 && !erster && !letzter && hindernisse.length && i2 >= Math.floor(shotCount / 3)) {
...
function buildVideoSequenceText(kit, shotCount = 5, totalSec = 15, lenTarget = 0, bank, tone = "neutral") {
  ...
  const dur = total / n2;
  const { shots, kamera } = buildVideoShots(kit, n2, lenTarget, bank, tone);
  ...
  for (let i2 = 0; i2 < shots.length; i2++) {
    out.push(`Shot ${i2 + 1} (${fmtSec(dur)})`, `DE: ${shots[i2]}`, `KAMERA: ${kamera[i2]}.`, "");
  }
  return out.join("\n");
}
...
function enforceWordTarget(text, target, bank, model, markovMode = "mix") {
  ...
  const einfuegeStellen = () => {
    const idx = [];
    for (let i2 = 1; i2 < schlussAb; i2++) if (saetze[i2] !== "\n\n") idx.push(i2);
    if (!idx.length) return [schlussAb];
    if (kurve.an) return idx.map((i2) => [i2, kurveWert(kurve.werte, i2 / Math.max(1, saetze.length - 1))]).sort((a, b) => a[1] - b[1]).map((x) => x[0]);
  ...
  const addition = (davor) => {
    ...
    const st2 = staemme(davor);
    const anschluss = st2.size ? fresh.filter((c) => {
      for (const x of staemme(c)) if (st2.has(x)) return true;
      return false;
    }) : [];
...
function buildRekombination(bank, input, model) {
  ...
  for (let s = 0; s < 600; s++) {
    ...
    while (offeneSlots(text) && guard++ < 3) {
      ...
      const beginntMitEingefuehrter = f.fuehrt_ein.some((n2) => !!w1 && n2.toLowerCase().startsWith(w1.toLowerCase()));
...
function sachNomen(was) {
  ...
  for (let i2 = 0; i2 < woerter3.length; i2++) {
    const w = (woerter3[i2] || "").replace(/[^A-Za-zÄÖÜäöüß-]/g, "");
    ...
    const zwei = (woerter3[i2 - 2] || "").toLowerCase().replace(/[^a-zäöüß]/g, "");
    const davor = (woerter3[i2 - 1] || "").toLowerCase().replace(/[^a-zäöüß]/g, "");
...
function strukturell(form, text, ziel) {
  ...
  const woerter3 = (text.match(/[A-Za-zÄÖÜäöüß]+/g) || []).length;
  if (!text.trim()) return ["leerer Text"];
  if (woerter3 < ziel * 0.55) out.push("L\xE4nge unter 55 % der Marke");
