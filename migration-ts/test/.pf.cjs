...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank, tone = "neutral") {
  ...
  for (let i2 = 0; i2 < shotCount; i2++) {
    ...
    if (folge && schlaegeJeShot[i2].length) {
      const saetze = schlaegeJeShot[i2].map(schlagSatz).filter((x) => !!x && x.replace(/[^A-Za-zÄÖÜäöüß]/g, "").length > 3);
    ...
    if (stufe >= 2 && !erster && !letzter && hindernisse.length && i2 >= Math.floor(shotCount / 3)) teile.push(`${cap(zieh(hindernisse, kit.obstacle))}.`);
