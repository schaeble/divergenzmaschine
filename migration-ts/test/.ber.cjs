...
console.log("\u2500\u2500 L\xE4nge und Einheiten je Ressort, je 60 Berichte \u2500\u2500");
var meterGesamt = {};
for (const r of RESSORT_IDS) {
  let w = 0, n = 0, abs = 0;
  const einh = /* @__PURE__ */ new Map();
  for (let i = 0; i < 60; i++) {
    const t = buildBericht(bank, inp(r), r);
    const txt = typeof t === "string" ? t : t.text;
    w += W(txt);
    n++;
    abs += txt.split(/\n\n+/).length;
    for (const m of txt.matchAll(/\b\d[\d.,]*\s+([A-ZÄÖÜ][a-zäöüß]+(?:\s+je\s+[A-ZÄÖÜ][a-zäöüß]+)?)/g)) {
      einh.set(m[1], (einh.get(m[1]) || 0) + 1);
    }
  }
  meterGesamt[r] = einh.get("Meter") || 0;
  const top = [...einh.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4).map(([k, v]) => `${k} ${v}`).join(", ");
  console.log(`  ${r.padEnd(14)} ${Math.round(w / n)} W\xF6rter, ${(abs / n).toFixed(1)} Abs\xE4tze   Einheiten: ${top}`);
}
console.log("\n\u2500\u2500 Meter je Ressort (60 Berichte) \u2500\u2500");
console.log("  " + Object.entries(meterGesamt).map(([k, v]) => `${k} ${v}`).join(" \xB7 "));
