...
function verfugen(teile2) {
  const out = [];
  for (let i = 0; i < teile2.length; i++) {
    let t = teile2[i].trim().replace(/\s+([.,;:!?])/g, "$1");
    if (!t) continue;
    ...
    if (!endet) t += ".";
    if (naechsterFolgtDirekt && i + 1 < teile2.length) {
      const n2 = teile2[i + 1].trim();
      teile2[i + 1] = n2.charAt(0).toLowerCase() + n2.slice(1);
    }
    out.push(t);
  }
...
function buildVideoShots(kit, shotCount, lenTarget = 0) {
  ...
  if (lenTarget > 0) {
    ...
    for (let runde = 0; runde < 20 && nachschubVorrat.length && zaehl2() < lenTarget * 0.92; runde++) {
      ...
      for (let i = 0; i < fertig.length && nachschub.length && zaehl2() < lenTarget * 0.92; i++) {
    ...
    for (let runde = 0; runde < 12 && zaehl2() < lenTarget * 0.92; runde++) {
      for (let i = 0; i < fertig.length && zaehl2() < lenTarget * 0.92; i++) {
        const frei2 = (liste) => {
...
function buildRekombination(bank, input, model) {
  ...
  const figuren = splitSpeakers(normWho(input.who || ""));
  const waehleFigur = () => {
...
function hergang(fb, bank, b, benutzt, extra, vorrat, blick) {
  ...
  if (z2) teile2.push(zahlSatz(z2));
  if (c3) teile2.push(`${cap(c3.zeit)} ${w.schritt(b.organisation(fb))}.`);
  const a1 = fb.abgeleitet[0];
  if (a1) teile2.push(w.haelfte(a1.label, a1.wortform));
  ...
  if (eins.length) {
    ...
    teile2.push(w.einsatz(mehr, aufzaehlung(zwei.map((x) => x.t))));
  }
  ...
  if (bt.length >= 3) {
    ...
    teile2.push(w.weitere(aufzaehlung(aus)));
  }
  return mische(teile2, frei).join(" ");
}
...
function hintergrund(fb, bank, b, benutzt, extra, vorrat) {
  ...
  if (c1) teile2.push(RK ? RK(b.organisation(fb), c1.zeit) : fb.wer.art === "person" ? `${cap(b.organisation(fb))} ist seit ${c1.zeit} dabei.` : `${cap(b.organisation(fb))} besteht seit ${c1.zeit}.`);
...
function buildBericht(bank, input, ressort = "auto") {
  ...
  if (extra >= 3) {
    ...
    for (let i = 0; i < extra - 2; i++) {
      ...
      if (roh) teile2.push(`${cap(roh)}.`);
    }
    if (teile2.length) abschnitte.push(`Zur Einordnung: ${teile2.join(" ")}`);
  }
  {
    const R = RESSORTS[fb.ressort];
    const teile2 = [];
    for (let i = 0; i < Math.min(R.zusatz.rahmen.length, 1 + Math.floor(extra / 3)); i++) {
      ...
      if (roh) teile2.push(`${R.zusatz.rahmen[i]} ${roh}.`);
    }
    if (teile2.length) abschnitte.push(`${R.zusatz.titel}: ${teile2.join(" ")}`);
  }
...
function vorspann2(fb) {
  ...
  const kern = tragtEigenesSubjekt(fb.was) || !werTaugt(fb) ? cap(fb.was) : `${cap(fb.wer.haupt)} ${fb.was}`;
  ...
  phrases.forEach((p, src) => {
    ...
    for (let a = 0; a < 1; a++) {
      for (let n2 = a + 2; n2 <= Math.min(a + 8, words.length); n2++) {
        ...
        seen.add(key);
        out.push({ text, syll: haikuSyllOf(text), src, ganz: a === 0 && n2 === words.length });
      }
    }
  });
  return out;
...
var W4 = [
  ["in einer Stra\xDFe, die zweimal existiert", "in der Stunde, die nicht gez\xE4hlt wird", "das Register aller falschen Namen", "h\xF6rt den eigenen Namen aus dem Nebenzimmer"],
  ...
  ["im Archiv", "am Morgen", "eine Archivarin ohne Namen, voller ungestellter Fragen", "gibt die Spur bewusst"],
  ["in der Markthalle", "im ewigen Nachmittag", "ein Schulm\xE4dchen, das Karten f\xE4lscht", "l\xE4uft der eigenen Spur nach"],
  ["am Deich", "im Winter", "die Archivarin", "sucht eine Akte"]
];
var zaehl = /* @__PURE__ */ new Map();
var bsp = /* @__PURE__ */ new Map();
var melde = (art, satz) => {
  zaehl.set(art, (zaehl.get(art) || 0) + 1);
  if (!bsp.has(art)) bsp.set(art, satz.trim().slice(0, 110));
};
var teile = (t) => t.split(/(?<=[.!?…])\s+/).map((x) => x.trim()).filter(Boolean);
var FINIT2 = /\b(ist|sind|war|waren|wird|werden|hat|hatte|haben|kann|muss|will|soll|darf|bleibt|steht|geht|kommt|liegt|zeigt|trägt|schweigt|bricht|reicht|führt|beginnt|endet|dauert|passt|fehlt|gilt|scheint|wirkt|[a-zäöüß]{3,}(?:t|te|en|et|st))\b/;
var n = 0;
var saetze = 0;
for (let i = 0; i < 150; i++) {
  const [wo, wann, wer, was] = W4[i % W4.length];
  const t = buildStory(BUILTIN_PRESETS[ids[i % ids.length]], {
    where: wo,
    when: wann,
    who: wer,
    what: was,
    tone: i % 3 ? "nuechtern" : "traeumerisch",
    form: "prose",
    lenTarget: 220,
    tension: "auto",
    cast: "auto",
  ...
  for (let k = 0; k < ss.length; k++) {
    const s = ss[k];
    if (/[a-zäöüß]+t\s+(?:gibt|steht|hält|läuft|geht|bricht|wird|ist)\b[^,;:.!?]*$/.test(s) && !/,|\bund\b|\boder\b|\baber\b|\bdenn\b/.test(s)) melde("zwei Verben ohne Fuge", s);
    if (k > 0 && ss[k - 1].replace(/[.!?…—–]/g, "").trim().toLowerCase() === s.replace(/[.!?…—–]/g, "").trim().toLowerCase()) melde("Satz doppelt hintereinander", s);
    if (/^(Voller ungestellter Fragen|voller ungestellter Fragen)\s+[a-zäöüß]+t\b/.test(s)) melde("Phrase als Subjekt", s);
    if (/^Faktenkasten\b|·\s*Betroffen:|Gegen \d{2}:\d{2}/.test(s)) melde("Ger\xFCst/Zeitstempel in der Prosa", s);
    const w = s.match(/\b(im|am|in|auf|über|unter|vor|hinter) [a-zäöüß]+\b/gi) || [];
    if (new Set(w.map((x) => x.toLowerCase())).size < w.length) melde("dieselbe F\xFCgung zweimal im Satz", s);
    const rel = s.match(/,\s*(der|die|das|dem|den)\s+([^,.!?]{3,60})[.!?]$/);
    if (rel && !FINIT2.test(rel[2])) melde("Relativsatz ohne finites Verb", s);
  }
}
console.log(`${n} Texte, ${saetze} S\xE4tze`);
[...zaehl].sort((a, b) => b[1] - a[1]).forEach(([k, v]) => console.log(`  ${String(v).padStart(4)}\xD7  ${k}
         \u201E${bsp.get(k)}\u201C`));
