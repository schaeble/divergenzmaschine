...
var STRUK = ["linear", "reverse", "circle", "fragment", "object"];
console.log("\u2500\u2500 Wo im Text steht das SCHLUSSBILD? (Anteil der Position, 0 = Anfang) \u2500\u2500");
for (const stk of STRUK) {
  const pos = [];
  for (let i = 0; i < 80; i++) {
    buildStory(BUILTIN_PRESETS[ids[i % ids.length]], {
      where: "im Archiv",
      when: "am Morgen",
      who: "die Archivarin",
  ...
  const m = pos.length ? pos.reduce((a, b) => a + b, 0) / pos.length : NaN;
  console.log(`  ${stk.padEnd(10)} Schlussbild im Mittel bei ${isNaN(m) ? "\u2014" : (m * 100).toFixed(0) + " %"} des Textes  (in ${pos.length} von 80 Texten)`);
}
console.log("\n\u2500\u2500 Je ein Beispiel \u2500\u2500");
for (const stk of STRUK) {
  const t = buildStory(BUILTIN_PRESETS[ids[3]], {
    where: "im Archiv",
    when: "am Morgen",
    who: "die Archivarin",
    what: "sucht eine Akte",
