...
function findeDoppel(bank, schwelle = 0.6) {
  const eintraege = [];
  for (const [kat, liste] of Object.entries(bank)) {
    if (!Array.isArray(liste) || kat === "verwandlungen") continue;
    for (const t of liste) {
      const text = (t || "").trim();
      if (text) eintraege.push({ text, kat, st: staemme(text), kern: kernwoerter(text) });
    }
  }
  const out = [];
  for (let i = 0; i < eintraege.length; i++) {
    for (let j = i + 1; j < eintraege.length; j++) {
      ...
      if (A.text.toLowerCase() === B.text.toLowerCase()) {
        out.push({ a: A.text, b: B.text, katA: A.kat, katB: B.kat, grad: 1, kern: "", art: "identisch" });
        continue;
      }
      if (!A.st.size || !B.st.size) continue;
      let gemeinsam = 0;
      let kern = "";
      for (const s of A.st) if (B.st.has(s)) {
        gemeinsam++;
        if (!kern) kern = s;
      }
      const grad = gemeinsam / Math.min(A.st.size, B.st.size);
      const ohneRequisit = A.kat !== "props" && B.kat !== "props";
      if (ohneRequisit && gemeinsam >= 2 && grad >= schwelle) {
        out.push({ a: A.text, b: B.text, katA: A.kat, katB: B.kat, grad, kern, art: "\xE4hnlich" });
        continue;
      }
      if (A.kat === B.kat && A.kat !== "props") {
        let k = "";
        for (const s of A.kern) if (B.kern.has(s)) {
          k = s;
          break;
        }
  ...
  const rang = { identisch: 0, "\xE4hnlich": 1, kernwort: 2 };
  return out.sort((x, y) => rang[x.art] - rang[y.art] || y.grad - x.grad);
}
function kernwortGruppen(bank, mindestens = 2) {
  ...
  for (const [kat, liste] of Object.entries(bank)) {
    ...
    for (const t of liste) {
      const text = (t || "").trim();
      if (!text) continue;
      for (const k of kernwoerter(text)) {
        const g2 = gruppen.get(k) || [];
        g2.push({ text, kat });
        gruppen.set(k, g2);
      }
    }
  }
  return [...gruppen.entries()].filter(([, e]) => e.length >= mindestens).map(([kern, eintraege]) => ({ kern, eintraege })).sort((a, b) => b.eintraege.length - a.eintraege.length);
}
...
wahr("St\xE4mme lassen Funktionsw\xF6rter weg", !staemme("Ein Brief, der ohne Absender ankommt").has("ohne"));
wahr("\xC4hnlichkeit erkennt Umformulierung", aehnlichkeit("die Nacht als offenes Tor", "Alles bleibt offen wie ein Tor bei Nacht.") >= 0.6);
wahr("und trennt Verschiedenes", aehnlichkeit("Ein Kranich, der im Dunkeln flackert", "Die Menge folgt einem Zeichen") < 0.3);
var probe = {
  motifs: ["ein Samowar, der nie ganz erkaltet", "die Nacht als offenes Tor", "Ein Tuch"],
  hooks: ["Der Samowar kocht, obwohl das Feuer erlischt", "Alles bleibt offen wie ein Tor bei Nacht.", "ein tuch"],
  props: ["ein Samowar"],
  turns: ["Ein Brief, der ohne Absender ankommt", "Ein vergessener Brief taucht pl\xF6tzlich wieder auf"],
  obstacles: [],
  stakes: [],
  endings: []
};
var d = findeDoppel(probe);
ist("identisch (auch \xFCber Kategorien, Gro\xDF/Klein egal)", d.filter((x) => x.art === "identisch").length, 1);
wahr("\xE4hnlich: Nacht/Tor", d.some((x) => x.art === "\xE4hnlich" && /Tor/.test(x.a)));
wahr("kernwort: Brief/Brief in derselben Kategorie", d.some((x) => x.art === "kernwort" && x.kern === "brief"));
wahr("Requisite gegen Bild z\xE4hlt nicht (ein Samowar / Samowar-Bild)", !d.some((x) => x.katA === "props" || x.katB === "props"));
ist("Reihenfolge: identisch zuerst", d[0].art, "identisch");
var g = kernwortGruppen(probe);
wahr("Kernwort-Gruppe Samowar \xFCber Bild und Haken", g.some((x) => x.kern === "samow" && x.eintraege.length === 2));
wahr("Requisiten bleiben aus den Gruppen", !g.some((x) => x.eintraege.some((e) => e.kat === "props")));
{
  let ident = 0;
  for (const b of Object.values(BUILTIN_PRESETS)) ident += findeDoppel(b).filter((x) => x.art === "identisch").length;
  wahr("h\xF6chstens f\xFCnf identische Paare in allen 51 Presets", ident <= 5, String(ident));
}
{
  const q = (0, import_fs.readFileSync)("src/ui/wordbankView.ts", "utf8");
  wahr("Bereich \u201E\xC4hnliche Eintr\xE4ge\u201C in der Wortbank", /" Ähnliche Einträge"/.test(q));
  wahr("\xD7 entfernt aus der aktiven Bank und zeichnet neu", /saveBank\(b\); load\(\); renderFull\(\); hygZeichnen\(\);/.test(q));
  wahr("\u201EPreset aus Text\u201C warnt vor Beinahe-Doppeln", /Beinahe-Doppel, z\. B\./.test(q));
}
console.log(`Pr\xFCfstand Preset-Hygiene \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Preset-Hygiene: ${fails.length} Fehler:`);
  ...
  console.log(`
\u2705 Preset-Hygiene: alle ${geprueft} Pr\xFCfungen bestanden.`);
}
