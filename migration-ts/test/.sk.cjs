...
function speichereKurve(k) {
  try {
    localStorage.setItem(KEY, JSON.stringify({ an: k.an, werte: k.werte.map(klemm) }));
  } catch {
  }
}
...
ist("sieben St\xFCtzstellen", ladeKurve().werte.length, STUETZEN);
speichereKurve({ an: true, werte: [0, 0.5, 1, 0.5, 0, 0.5, 1] });
ist("gespeichert", ladeKurve().an, true);
ist("Interpolation an St\xFCtzstelle", kurveWert([0, 1, 0], 0.5), 1);
...
ist("Klemmung", kurveWert([0, 1], 3), 1);
{
  ...
  wahr("Sp\xE4te Wende: Maximum sp\xE4t", sp.max > 0.75, String(sp.max));
  ...
  ist("Katastrophe zuerst: H\xF6hepunkt ganz vorn", f.indexOf("hoehepunkt") <= 1, true);
  const g = schlagfolgeAusKurve(KURVEN_VORLAGEN["spaet"].werte);
  wahr("Sp\xE4te Wende: H\xF6hepunkt im letzten Drittel", g.indexOf("hoehepunkt") / g.length > 0.6);
  ...
  wahr("der Einstieg steht vorn \u2014 bei Katastrophe zuerst gleich nach dem H\xF6hepunkt", [g, o].every((x) => x[0] === "einstieg") && f.indexOf("einstieg") <= 1);
  ist("Regler aus der Kurve: sp\xE4t \u2192 unten", reglerAusKurve(KURVEN_VORLAGEN["spaet"].werte), "low");
...
{
  const basis = Array.from({ length: 14 }, (_, i) => `Der Satz Nummer ${i + 1} liegt ruhig im Text, und niemand r\xFChrt sich an dieser Stelle.`).join(" ");
  const mittel = (t) => {
    const s = t.split(/(?<=[.!?…])\s+/);
    return s.reduce((n, x) => n + x.split(/\s+/).length, 0) / s.length;
  };
  let hoch = 0, tief = 0;
  for (let i = 0; i < 12; i++) {
    hoch += mittel(applyTension(basis, "off", void 0, () => 0.95));
    tief += mittel(applyTension(basis, "off", void 0, () => 0.05));
  }
  wahr("hohe Kurve \u2192 k\xFCrzere S\xE4tze als tiefe", hoch < tief, `${(hoch / 12).toFixed(1)} < ${(tief / 12).toFixed(1)}`);
}
{
...
var inp = { where: "im Hafen", when: "am Abend", who: "Der Bote", what: "h\xF6rt die Glocke", tone: "mystery", varLevel: "wild", form: "prose", structure: "linear", mode: "myth", perspective: "third", rhythm: "auto", markovMode: "off", disruptor: "off", archetypeA: "neutral", archetypeB: "psychopath", instability: 0, polish: false, polishStyle: "surreal_precise", tension: "off" };
wahr("aus: der Bau l\xE4uft unver\xE4ndert", buildStory(DEFAULT_BANK, inp).split(/\s+/).length > 40);
console.log(`Pr\xFCfstand Spannungskurve \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Spannungskurve: ${fails.length} Fehler:`);
  fails.forEach((f) => console.error("  - " + f));
  proc.process?.exit(1);
} else console.log(`
\u2705 Spannungskurve: alle ${geprueft} Pr\xFCfungen bestanden.`);
