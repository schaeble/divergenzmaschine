...
var ALT = /(^|\s)(ein|eine|einem|einen|einer|eines|der|die|das|dem|den|des|und|oder|aber|wie|mit|an|auf|zu|im|am|vor|nach|für|ohne|als|bei|aus|ist|sind|wird)$/i;
var alt = 0;
var neu = 0;
var gerettet = [];
var bleibt = [];
for (const id of Object.keys(BUILTIN_PRESETS)) {
  ...
  for (const k of Object.keys(b)) if (Array.isArray(b[k])) for (const s of b[k]) {
    const bare = String(s).trim().replace(/[.!?…]+$/, "").trim();
    if (bare.split(/\s+/).length > 12) continue;
    const a = ALT.test(bare), n = istAbgeschnitten(bare);
    if (a) alt++;
    if (n) neu++;
    if (a && !n && gerettet.length < 6) gerettet.push(s);
    if (n && bleibt.length < 6) bleibt.push(s);
  }
}
console.log("verworfen alt:", alt, "| neu:", neu, "| gerettet:", alt - neu);
console.log("GERETTET:");
gerettet.forEach((x) => console.log("   ", x));
console.log("WEITERHIN VERWORFEN:");
bleibt.forEach((x) => console.log("   ", x));
for (const f of ["ein Blick auf der", "die T\xFCr und", "der Schatten des", "ein Weg zu", "der Rand ohne"])
  console.log(istAbgeschnitten(f) ? "  \u2713 erkannt: " : "  \u2717 DURCH:   ", f);
