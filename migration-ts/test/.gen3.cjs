...
// ../../gen3.ts
var fehlt = /* @__PURE__ */ new Set();
for (const id of Object.keys(BUILTIN_PRESETS)) {
  const b = BUILTIN_PRESETS[id];
  for (const kat of Object.keys(b)) {
    if (!Array.isArray(b[kat])) continue;
    for (const p of b[kat]) {
      const m = String(p).match(/^(ein|eine|einen|einem|einer|eines)\s+(.*)$/i);
      if (!m) continue;
      const w = (m[2].split(" ").find((x) => /^[A-ZÄÖÜ]/.test(x)) || "").replace(/[^A-Za-zÄÖÜäöüß]/g, "");
      if (w && !guessGender(w)) fehlt.add(w);
    }
  }
}
console.log([...fehlt].sort().join("\n"));
console.log("---", fehlt.size, "Nomen ohne Genus");
