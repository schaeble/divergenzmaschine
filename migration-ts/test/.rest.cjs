...
for (let i = 0; i < 600; i++) {
  ...
  const m = t.match(/(^|[.!?…]\s+)[Vv]oller ungestellter Fragen\s+[a-zäöüß]+t\b[^.!?…]*/g);
  if (m) {
    n++;
    if (n <= 4) console.log((i % 2 ? "rekombination" : "linear") + " \xB7 " + m[0].trim().slice(0, 100));
  }
}
console.log("Treffer in 600 Texten:", n);
