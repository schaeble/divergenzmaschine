...
ist("und ohne Wort bleibt ein Ding \xFCbrig", objektName(""), "das Ding");
wahr(
  "ein unbekanntes Wort bekommt trotzdem einen Artikel",
  /^(der|die|das) Zwirbelgerät$/.test(objektName("Zwirbelger\xE4t"))
);
{
  const ohne = [];
...
}
{
  ...
  wahr(
    "aber das Ding meldet sich kurz",
...
for (const p of ["third", "first", "second", "we", "split"]) {
  const bank = BUILTIN_PRESETS[ids[0]];
...
{
  const f = (t, p = "we") => applyPerspective([t], p, "Der Wanderer", "Stab")[0];
  wahr(
    "nach dem Semikolon bleibt das Pronomen klein",
  ...
  wahr(
    "nach dem Doppelpunkt weiter gross",
    /: Wir /.test(f("Es geht weiter: Der Wanderer nimmt einen Stab."))
  );
}
{
  const p = (persp, satz) => applyPerspective([satz], persp, "Der Gast", "")[0];
  ist("wir: beide Verben", p("we", "Der Gast r\xE4umt ein Zimmer und findet ein Leben."), "Wir r\xE4umen ein Zimmer und finden ein Leben.");
  ist("ich: beide Verben", p("first", "Der Gast r\xE4umt ein Zimmer und findet ein Leben."), "Ich r\xE4ume ein Zimmer und finde ein Leben.");
  ist("du: beide Verben", p("second", "Der Gast wartet und z\xE4hlt die Tage."), "Du wartest und z\xE4hlst die Tage.");
  ...
  ist("Adverb nach und wird nicht gebeugt", p("we", "Der Gast \xF6ffnet die T\xFCr und dort wartet der Hund."), "Wir \xF6ffnen die T\xFCr und dort wartet der Hund.");
}
...
\u274C Perspektive: ${fails.length} Fehler:`);
  ...
  console.log(`
\u2705 Perspektive: alle ${geprueft} Pr\xFCfungen bestanden.`);
}
