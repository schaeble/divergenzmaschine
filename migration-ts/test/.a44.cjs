...
function pruefeMeldung(text, fb) {
  ...
  if (nachDoppel && !tragtPraedikat(nachDoppel) && !/^(ein|eine|einen|einem|einer|der|die|das|den|dem|kein|keine)\b/i.test(nachDoppel)) {
    m.push({ art: "Kernsatz ohne Pr\xE4dikat", stelle: nachDoppel.slice(0, 50) });
  }
...
var FAELLE = [
  ["", "am 15. August 2026", "", "Der deutsche Leicht\xADathlet Owen Ansah hat bei der EM in Birmingham Bronze \xFCber 100 m und Gold \xFCber 200 m gewonnen"],
  ...
  ["", "am 13. September 1830", "", "Die Schriftstellerin Marie von Ebner-Eschenbach (Lotti, die Uhrmacherin; Das Gemeinde\xADkind) wird geboren"],
...
for (const [wo, wann, wer, was] of FAELLE) {
  ...
  const erg = buildMeldung(inp, "gesellschaft");
  const t = erg.text ?? String(erg);
  const p = pruefeMeldung(t, erg.fb);
  console.log("\u2500\u2500 Wo:", JSON.stringify(wo), "Wann:", JSON.stringify(wann), "Wer:", JSON.stringify(wer));
  console.log("   " + t.replace(/\n/g, " "));
  console.log("   Pr\xFCfung:", JSON.stringify(p).slice(0, 220));
  console.log();
}
