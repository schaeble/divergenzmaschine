...
console.log("Philosophie-Paare:");
for (const p of b.verwandlungen) {
  const r = pruefePaar(p);
  console.log("  " + (r.ok ? "\u2713 " : "\u2717 ") + p.padEnd(26) + r.grund);
}
console.log("\nDein Beispiel:");
for (const p of ["Telefon\u2192Stille", "Katze\u2192Schatten", "Regen\u2192Nebel", "Jazz\u2192Wind", "Schl\xFCssel\u2192T\xFCr"]) {
  const r = pruefePaar(p);
  console.log("  " + (r.ok ? "\u2713 " : "\u2717 ") + p.padEnd(20) + r.grund);
}
console.log("\nangenommen:", leseVerwandlungen(b.verwandlungen).length, "von", b.verwandlungen.length);
