...
import {
  ...
  rasteRahmen, spaltenBreite, spaltenSpanne, spaltenZahl, bildplatz, spaltenlagen, verschmelzeBaender,
...
// ── 10 · Bänder verschmelzen ────────────────────────────────────────────────
// Zwei Bilder, deren Bänder einander überschneiden, sind EIN gesperrter
// Bereich. Getrennt gezählt sperrten sie mehr Höhe, als die Bilder einnehmen.
ist("überlappende Bänder werden eins",
  JSON.stringify(verschmelzeBaender([{ oben: 100, hoehe: 100 }, { oben: 150, hoehe: 100 }])),
  JSON.stringify([{ oben: 100, hoehe: 150 }]));
ist("getrennte bleiben getrennt", verschmelzeBaender([{ oben: 10, hoehe: 20 }, { oben: 100, hoehe: 20 }]).length, 2);
ist("unsortiert wird geordnet", verschmelzeBaender([{ oben: 100, hoehe: 20 }, { oben: 10, hoehe: 20 }])[0]!.oben, 10);
