...
var n = 0;
var unveraendert = 0;
var fehlt = /* @__PURE__ */ new Map();
for (const id of Object.keys(BUILTIN_PRESETS)) {
  const b = BUILTIN_PRESETS[id];
  for (const kat of ["hooks", "props"]) {
    for (const p of b[kat] || []) {
      const m = p.match(/^(ein|eine|einen|einem|einer|eines)\s+(.*)$/i);
      if (!m) continue;
      n++;
      if (declineHookPhrase(p, "acc") === p.trim() && /^ein\s/i.test(p)) {
        unveraendert++;
        const w = (m[2].split(" ").find((x) => /^[A-ZÄÖÜ]/.test(x)) || "").replace(/[^A-Za-zÄÖÜäöüß]/g, "");
        if (w && !guessGender(w)) fehlt.set(w, (fehlt.get(w) || 0) + 1);
      }
    }
  }
}
console.log(`Haken/Requisiten mit unbestimmtem Artikel: ${n} \xB7 Akkusativ NICHT gebildet: ${unveraendert}`);
console.log(`davon wegen unbekanntem Genus: ${[...fehlt.values()].reduce((a, b) => a + b, 0)} bei ${fehlt.size} verschiedenen Nomen`);
console.log([...fehlt.keys()].slice(0, 40).join(" \xB7 "));
