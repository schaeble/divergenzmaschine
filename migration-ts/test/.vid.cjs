...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank, tone = "neutral") {
  ...
  for (let i = 0; i < shotCount; i++) {
    ...
    shots2.push(teile.filter(Boolean).join(" "));
    kameras.push(zieh(kamera, "Statische Einstellung, 35 mm"));
  }
  return { shots: shots2, kamera: kameras };
}
...
function buildVideoSequenceText(kit, shotCount = 5, totalSec = 15, lenTarget = 0, bank, tone = "neutral") {
  ...
  const dur = total / n;
  const { shots: shots2, kamera } = buildVideoShots(kit, n, lenTarget, bank, tone);
  ...
  for (let i = 0; i < shots2.length; i++) {
    out.push(`Shot ${i + 1} (${fmtSec(dur)})`, `DE: ${shots2[i]}`, `KAMERA: ${kamera[i]}.`, "");
  }
  return out.join("\n");
}
...
var inp = {
  where: "in einem verlassenen Bahnhof",
  when: "vor dem ersten Zug",
  who: "Eine Uhrmacherin",
  what: "sucht einen Ton, der fehlt",
  tone: "mystery",
  varLevel: "wild",
  form: "video",
  structure: "linear",
...
var bau = (o = {}) => buildStory(DEFAULT_BANK, { ...inp, ...o });
var shots = (t) => t.split("\n").filter((l) => l.startsWith("DE: ")).map((l) => l.slice(4));
{
  const t = bau({ lenTarget: 200 });
  const sh = shots(t);
  ist("f\xFCnf Shots", sh.length, 5);
  wahr("der erste Shot verankert Ort und Figur", /^In einem verlassenen Bahnhof: Eine Uhrmacherin nahe /.test(sh[0]));
  wahr("jeder Shot au\xDFer dem letzten hat ein Nah-Detail", sh.slice(0, -1).every((s) => /Nah: /.test(s)));
  wahr("jeder Shot au\xDFer dem letzten endet mit einem Schnitt", sh.slice(0, -1).every((s) => /(Schnitt\.|das Bild geht\.|Atemzug lang\.|Überblendung ins Nächste\.|das Bild nicht\.|Das Bild reißt\.)$/.test(s)));
  wahr("der letzte Shot tr\xE4gt den Schluss und l\xE4sst etwas sichtbar", /Nur (der Riss|das Fenster|die Karte|das Licht) bleibt sichtbar\.$/.test(sh[4]));
...
}
{
  const w = (lt) => {
    let s = 0;
    for (let i = 0; i < 6; i++) s += bau({ lenTarget: lt }).split(/\s+/).length;
    return s / 6;
  };
  const a = w(110), b = w(200), c = w(300);
  wahr("110 < 200 < 300 im Wortma\xDF", a < b && b < c, `${a.toFixed(0)} < ${b.toFixed(0)} < ${c.toFixed(0)}`);
  wahr("bei 300 \xFCber 250 W\xF6rter (vorher blieb es d\xFCnn)", c > 250, c.toFixed(0));
}
{
  let doppelt = 0;
  for (let i = 0; i < 15; i++) {
    const sh = shots(bau({ lenTarget: 200 }));
  ...
  ist("Requisiten wiederholen sich nicht (15 L\xE4ufe)", doppelt, 0);
  wahr("Nah steht im Nominativ", !/Nah: einen /.test(bau({ lenTarget: 300 })));
}
{
  setDramaData({
    einstieg: ["Der Bahnhof atmet zum ersten Mal"],
    mitte: ["Ein Zug, der nie kommt"],
    hoehepunkt: ["Die Uhr schl\xE4gt ohne Zeiger"],
    schluss: [],
    ausloeser: ["ein Fahrplan aus Wachs"],
    veraenderungen: ["die Gleise beginnen zu singen"],
    konflikte: [],
    zeitanomalien: [],
    regeln: []
  });
  ...
  wahr("H\xF6hepunkt des Bogens in der Mitte", /Die Uhr schlägt ohne Zeiger/.test(sh[2]));
  setDramaData(null);
}
{
  const B = {
    einstieg: ["Der Bahnhof atmet zum ersten Mal"],
    mitte: ["Ein Zug, der nie kommt", "Eine Bank ohne Sitzfl\xE4che"],
    hoehepunkt: ["Die Uhr schl\xE4gt ohne Zeiger"],
    schluss: ["Zur\xFCck bleibt ein Fahrplan aus Wachs"],
    ausloeser: ["ein Fahrplan aus Wachs"],
    veraenderungen: ["die Gleise beginnen zu singen"],
    konflikte: ["den letzten Zug"],
    zeitanomalien: [],
    regeln: []
  };
  setDramaData({ ...B, folge: ["hoehepunkt", "einstieg", "hook", "mitte", "konflikt", "ausloeser", "wende", "einsatz", "schluss"] });
  let sh = shots(bau({ lenTarget: 200 }));
  wahr("Katastrophe zuerst: H\xF6hepunkt im ersten Shot", /Die Uhr schlägt ohne Zeiger/.test(sh[0]));
  ...
  sh = shots(bau({ lenTarget: 200 }));
  wahr("R\xFCckw\xE4rts: Schluss im ersten Shot", /Fahrplan aus Wachs/.test(sh[0]));
  wahr("R\xFCckw\xE4rts: Einstieg im letzten Shot", /Der Bahnhof atmet zum ersten Mal/.test(sh[4]));
  setDramaData(null);
...
{
  const artikel = "Neumeier hat gerade seinen Dienstwagen abgestellt, auf einem Parkplatz neben den Bahngleisen, unweit des Bonner Hauptbahnhofs. Da, der Flohzirkus!\xAB, fl\xFCstert Neumeier. Ein Sozialarbeiter aus der Unterkunft, in der Micha Neumeier lebt, nennt ihn \xBBabsolut zuverl\xE4ssig\xAB. Irgendetwas darin f\xFChlte sich nach einem echten Neuanfang an. Jetzt l\xE4uft er durch eine Unterf\xFChrung, einen unbeleuchteten Tunnel, auf dem Boden liegen aufgeweichte Pappen und alte Kleider herum. Ein B\xFCndel Papier mit Wasserflecken. Eine Decke im Regen. Es geht um ein Zuhause. Oder in einem Heim. Nur ein Zuhause finden sie nicht. Ein kleiner Trost bleibt trotzdem. Zum ersten Mal seit Langem scheint der Weg wieder offen. Es reicht diesmal f\xFCr alle.";
  const bank = presetAusText(artikel).bank;
  let dubl = 0, praet = 0, halbeZitate = 0, nahMitVerb = 0, schlechterOrt = 0;
  for (let i = 0; i < 10; i++) {
    const t = buildStory(bank, { ...inp, where: "Urbane Stra\xDFe mit Graffiti-Wand", lenTarget: 200 });
  ...
  ist("keine Dubletten mehr (10 L\xE4ufe)", dubl, 0);
  ist("Pr\xE4teritum wird zum Pr\xE4sens", praet, 0);
  ist("halbe Zitate fallen (Satz-W\xE4chter)", halbeZitate, 0);
  ist("Nah verlangt eine Nominalphrase", nahMitVerb, 0);
  ist("Ort mit Adjektiv ohne Artikel wird gebeugt", schlechterOrt, 0);
  wahr("\u201EAuf einer urbanen Stra\xDFe\u201C", /Auf einer urbanen Straße mit Graffiti-Wand/.test(buildStory(bank, { ...inp, where: "Urbane Stra\xDFe mit Graffiti-Wand" })));
}
console.log(`Pr\xFCfstand Multi-Shot \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Multi-Shot: ${fails.length} Fehler:`);
  ...
  console.log(`
\u2705 Multi-Shot: alle ${geprueft} Pr\xFCfungen bestanden.`);
}
