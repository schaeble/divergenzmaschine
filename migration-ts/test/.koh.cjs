...
var ids = Object.keys(BUILTIN_PRESETS).filter((x) => x !== "philosophie");
var phil = BUILTIN_PRESETS["philosophie"];
var groesse = (b) => KAT.reduce((s, k) => s + (b[k] || []).length, 0);
var misch = {};
for (const k of KAT) misch[k] = [];
for (const id of ids.slice(0, 3)) for (const k of KAT) misch[k].push(...BUILTIN_PRESETS[id][k] || []);
for (const k of KAT) misch[k] = misch[k].slice(0, phil[k].length);
var W = (s) => s.split(/\s+/).filter(Boolean).length;
var text = (e) => typeof e === "string" ? e : e.text;
console.log(`Philosophie: ${groesse(phil)} Eintr\xE4ge \xB7 Mischung aus drei Presets: ${groesse(misch)} Eintr\xE4ge`);
console.log("\n  Bank".padEnd(22) + "Bericht 450".padStart(14) + "Prosa 400".padStart(13) + "Phrasenwdh".padStart(13));
for (const [name, b] of [["Philosophie (eine Hand)", phil], ["Mischung (drei H\xE4nde)", misch]]) {
  let wb = 0, wp = 0, rep = 0;
  const N2 = 40;
  for (let i = 0; i < N2; i++) {
  ...
  console.log("  " + name.padEnd(20) + `${Math.round(wb / N2)} (${Math.round(100 * wb / N2 / 450)}%)`.padStart(14) + `${Math.round(wp / N2)} (${Math.round(100 * wp / N2 / 400)}%)`.padStart(13) + (rep / N2).toFixed(3).padStart(13));
}
