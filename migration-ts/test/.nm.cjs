...
function namenMeldung(e2) {
  if (!e2.ersetzt.length) return "";
  const gesehen = /* @__PURE__ */ new Map();
  for (const x of e2.ersetzt) if (!gesehen.has(x.name)) gesehen.set(x.name, x.durch.toLowerCase());
  return `${e2.ersetzt.length} ${e2.ersetzt.length === 1 ? "Name" : "Namen"} ersetzt: ${[...gesehen.entries()].slice(0, 4).map(([n, d]) => `${n} \u2192 ${d}`).join(", ")}${gesehen.size > 4 ? " \u2026" : ""}`;
}
...
var e = (t) => entnamen(t).text;
wahr("die Liste kennt \xFCber vierhundert Vornamen", VORNAMEN.size > 400, String(VORNAMEN.size));
...
ist("Dativ nach Pr\xE4position", e("Mit Vera kommt der Regen."), "Mit ihr kommt der Regen.");
ist("Akkusativ nach Pr\xE4position", e("F\xFCr Tom bleibt nichts."), "F\xFCr ihn bleibt nichts.");
ist("Dativ als indirektes Objekt", e("Er gibt Vera den Schl\xFCssel."), "Er gibt ihr den Schl\xFCssel.");
ist("Genitiv \u2192 Possessiv, nach Genus des Nomens", e("Veras Mantel h\xE4ngt; Toms Haus brennt."), "Ihr Mantel h\xE4ngt; sein Haus brennt.");
ist("Possessiv vor Plural", e("Toms B\xFCcher liegen da."), "Seine B\xFCcher liegen da.");
...
ist("W\xF6rter, die wie Namen aussehen, bleiben (August, Heide)", e("Im August \xFCber die Heide."), "Im August \xFCber die Heide.");
ist("kein Name: unver\xE4ndert", e("Sp\xE4ter bleibt eine Tasche stehen."), "Sp\xE4ter bleibt eine Tasche stehen.");
wahr("die Meldung nennt Zahl und Beispiele", /^2 Namen ersetzt: Vera → sie, Tom → er$/.test(namenMeldung(entnamen("Vera und Tom warten."))));
{
  ...
  for (const b of Object.values(BUILTIN_PRESETS)) for (const l of Object.values(b)) if (Array.isArray(l)) for (const x of l) {
    gesamt++;
    if (entnamen(x).ersetzt.length) geaendert++;
  }
  wahr("h\xF6chstens ein halbes Prozent der eingebauten Bausteine wird angefasst", geaendert / gesamt < 5e-3, `${geaendert} von ${gesamt}`);
}
{
  const qw = (0, import_fs.readFileSync)("src/ui/wordbankView.ts", "utf8");
  wahr("Wortbank: \u201EAlle \xFCbernehmen\u201C l\xE4sst den W\xE4chter laufen und meldet", /const e = entnamen\(x\);/.test(qw) && /Namen-Wächter: \$\{namen\}/.test(qw));
  const qt = (0, import_fs.readFileSync)("src/features/textpreset.ts", "utf8");
  wahr("Preset aus Text: jedes Teilst\xFCck ohne Namen", /entnamen\(u\.ok && u\.changed \? u\.text : s0\)\.text/.test(qt));
  const qe = (0, import_fs.readFileSync)("src/ui/erzaehlerbankView.ts", "utf8");
  wahr("Erz\xE4hlerbank: Speichern ersetzt Namen und meldet", /const ne = entnamen\(textIn\.value\);/.test(qe) && /namenMeldung\(ne\)/.test(qe));
}
console.log(`Pr\xFCfstand Namen-W\xE4chter \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Namen-W\xE4chter: ${fails.length} Fehler:`);
  fails.forEach((f) => console.error("  - " + f));
  proc.process?.exit(1);
} else console.log(`
\u2705 Namen-W\xE4chter: alle ${geprueft} Pr\xFCfungen bestanden.`);
