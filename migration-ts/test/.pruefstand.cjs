...
function vorspann(fb, b) {
  ...
  const s2 = z ? `${cap(fb.wann.datum)} wurde bekannt, dass ${z.verbal || z.wortform} ${z.einheit} betroffen sind.` : `${cap(fb.wann.datum)} wurde der Schritt in ${fb.wo.ort} bekannt.`;
  return `${s1} ${s2}`;
}
function hergang(fb, bank, b, benutzt, extra, vorrat) {
  ...
  if (c3) teile.push(`${cap(c3.zeit)} folgte der Schritt, \xFCber den ${b.organisation(fb)} nun informiert.`);
...
function hintergrund(fb, bank, b, benutzt, extra, vorrat) {
  ...
  if (c1) teile.push(fb.wer.art === "person" ? `${cap(b.organisation(fb))} ist seit ${c1.zeit} dabei.` : `${cap(b.organisation(fb))} besteht seit ${c1.zeit}.`);
...
for (const wer of WER) for (const was of WAS) for (const wann of WANN) for (const wo of WO)
  for (const ziel of [140, 300]) {
    ...
    const b = buildBericht(
      ...
      { ...basis, who: wer, what: was, when: wann, where: wo, lenTarget: ziel },
      ressort
    );
    n++;
    const funde = [];
...
console.log(`Pr\xFCfstand Bericht: ${n} L\xE4ufe (${WER.length}\xD7${WAS.length}\xD7${WANN.length}\xD7${WO.length}\xD72)`);
