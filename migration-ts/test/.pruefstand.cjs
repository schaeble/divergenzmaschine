...
var VERBOTEN = [
  ...
  ["Nominativ hinter \xBBgab es\xAB", /gab es der erste\b/],
...
{
  const re = VERBOTEN.find(([n2]) => n2 === "Kopfsatz ohne Aussage")[1];
  const soll = re.test("Am Morgen. Die Archivarin bemerkt. Danach nichts.");
  const darfNicht = re.test("Ein Sch\xFCler sitzt seit dem Morgen und hat nichts bemerkt. ");
  if (!soll || darfNicht) {
    console.error(`  \u2717 Gegenprobe Kopfsatz: leerer Rahmen ${soll ? "erkannt" : "NICHT erkannt"}, vollst\xE4ndiger Satz ${darfNicht ? "F\xC4LSCHLICH erkannt" : "durchgelassen"}`);
    process.exitCode = 1;
  }
}
  ...
  for (const ton of TOENE) {
    ...
    if (ressort === "wetter") {
      const wf = [
        ...
        ["zur\xFCckgenommener Schritt im Wetter", /Ob der Schritt zurückgenommen wird/],
