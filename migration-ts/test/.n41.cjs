...
var RAHMEN2 = [
  "Ich liege hier und z\xE4hle mit.",
  "Man hat mich hier vergessen.",
  "Niemand fragt mich, und ich sehe alles.",
  "Ich habe keine Augen und trotzdem einen Blick.",
  "Ich bleibe, wo man mich hingestellt hat.",
  "Man geht an mir vorbei, seit Jahren."
];
var ohne2 = 0;
for (let i = 0; i < 40; i++) {
  const t = buildStory(BUILTIN_PRESETS[ids[i % ids.length]], { ...basis, perspective: "object", structure: i % 2 ? "rekombination" : "linear" });
  const zweiterDa = RAHMEN2.some((r) => t.includes(r) || t.includes(r.charAt(0).toLowerCase() + r.slice(1)));
  if (!zweiterDa) {
    ohne2++;
    if (ohne2 <= 2) console.log("OHNE:", t);
  }
}
console.log("zweiter Rahmensatz fehlt in", ohne2, "von 40");
