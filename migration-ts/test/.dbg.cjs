...
function phasenBonus(a, phase) {
  if (a.phase) return a.phase === phase ? 3.5 : -2.5;
...
function buildRekombination(bank, input, model) {
  ...
  for (let s = 0; s < 600; s++) {
    ...
    if (a.kategorie === "endings" && fortschritt > 0.55) break;
  }
...
for (const stk of ["linear", "reverse"]) {
  buildStory(BUILTIN_PRESETS[ids[3]], {
    where: "im Archiv",
    when: "am Morgen",
    who: "die Archivarin",
  ...
  const tr = getTrace();
  console.log("[" + stk + "]", tr.map((x) => `${x.phase}/${x.kategorie}`).join("  "));
}
