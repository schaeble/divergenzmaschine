...
function archetypeAugmentList(baseList, archA, archB, key) {
  ...
  const base2 = Array.isArray(baseList) ? baseList : [];
  if (extra.length) return base2.concat(extra, extra);
  return base2;
}
...
function mutateBank(bank, amount) {
  ...
  const injectAdj = (s) => s.replace(
    /\b(ein|eine|einen|einem|einer)\s+([A-Za-zÄÖÜäöüß]+)/i,
    (_m, a, n) => `${a} ${pick(ADJ)} ${n}`
  );
  const intensify = (s) => {
    if (/[.!?]$/.test(s)) {
      return chance(0.5) ? "Beinahe " + s.charAt(0).toLowerCase() + s.slice(1) : s.replace(/[.!?]$/, " \u2013 und mehr.");
    }
  ...
  const mutateList = (list, templates) => {
  ...
  const gen = (fn) => Array.from({ length: 6 }, fn);
  const out = normalizeBankShape(bank);
  out.motifs = mutateList(out.motifs, gen(() => `ein ${freshWord()}, das sich ver\xE4ndert`));
  ...
  out.endings = mutateList(out.endings, [
    ...
    "Es bleibt.",
    "Kein Ende.",
    "Und nichts kehrte zur\xFCck.",
    "So blieb die Frage offen."
  ]);
  return out;
}
function bankEntryCount(bank) {
  return BANK_KEYS.reduce((a, k) => a + (bank[k]?.length ?? 0), 0);
}
...
for (const structure of ["linear", "reverse", "circle", "fragment", "object"]) {
  for (const polish of [false, true]) {
    for (let i = 0; i < RUNS; i++) {
      let t = "";
      try {
        t = buildStory(DEFAULT_BANK, base({ form: "prose", structure, polish }));
      } catch (e) {
        fails.push(`prose/${structure}/polish=${polish} CRASH: ${e.message}`);
        continue;
      }
...
for (let i = 0; i < RUNS; i++) {
  let t = "";
  try {
    t = buildStory(DEFAULT_BANK, base({ form: "script" }));
  } catch (e) {
    fails.push(`script CRASH: ${e.message}`);
    continue;
  }
...
try {
  const v = buildStory(DEFAULT_BANK, base({ form: "video", shots: 5, totalSec: 15 }));
  check(/^SEQUENZ/.test(v), "video: kein SEQUENZ-Kopf");
} catch (e) {
  fails.push(`video CRASH: ${e.message}`);
}
for (const form of ["poem", "drama", "strang", "reim", "haiku"]) {
  for (let i = 0; i < 10; i++) {
    let t = "";
    try {
      t = buildStory(DEFAULT_BANK, base({ form }));
    } catch (e) {
      fails.push(`${form} CRASH: ${e.message}`);
      continue;
    }
...
{
  const silben2 = (l) => l.replace(/\s*–\s*$/, "").split(/\s+/).filter(Boolean).reduce((a, w) => a + estimateSyllables(w), 0);
  const soll = [5, 7, 5];
  let zeilen = 0, treffer = [0, 0, 0], bloecke = 0, dreiZeilig = 0;
  for (let i = 0; i < 60; i++) {
    const t = buildStory(DEFAULT_BANK, base({ form: "haiku", lenTarget: 48 }));
...
for (const amt of [0, 150, 300, 500]) {
  ...
  const mutated = mutateBank(DEFAULT_BANK, amt);
  check(bankEntryCount(mutated) >= before, `mutation@${amt}: schrumpft`);
  for (const k of Object.keys(DEFAULT_BANK)) {
    check(mutated[k].length <= 80, `mutation@${amt}/${k}: \xFCber Cap 80`);
...
if (fails.length) {
  ...
  console.log(`\u2705 Alle Property-Tests bestanden (${5 * 2 * RUNS} Prosa- + ${RUNS} Dialog-L\xE4ufe, Video, Mutation).`);
}
