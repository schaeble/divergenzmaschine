...
function pickSane(arr, minWords = 2) {
  ...
  const ok2 = list.filter(
    (x) => String(x ?? "").trim().split(/\s+/).filter(Boolean).length >= minWords
  );
  return (ok2.length ? pick(ok2) : pick(list)) ?? "";
}
function chance(p2) {
...
function kuerzeAmBruch(text2) {
  let t2 = (text2 || "").replace(/\s*…\s*$/, "").replace(/\s*[.,;:–—-]+\s*$/, "").trim();
  ...
  }
  {
    const komma = t2.lastIndexOf(",");
    if (komma >= 12) {
      const schwanz = t2.slice(komma + 1).trim();
...
function loadKnobs() {
  try {
    ...
    return {
      fuegeteil: klemm(Number(p2.fuegeteil) || KNOB_VORGABE.fuegeteil, KNOB_SPANNE.fuegeteil),
      w4max: klemm(Number(p2.w4max) || KNOB_VORGABE.w4max, KNOB_SPANNE.w4max),
      abstand: klemm(Number(p2.abstand) || KNOB_VORGABE.abstand, KNOB_SPANNE.abstand),
      bogen: klemm(p2.bogen === void 0 ? KNOB_VORGABE.bogen : Number(p2.bogen), KNOB_SPANNE.bogen),
      ton: klemm(p2.ton === void 0 ? KNOB_VORGABE.ton : Number(p2.ton), KNOB_SPANNE.ton),
      korpus: klemm(p2.korpus === void 0 ? KNOB_VORGABE.korpus : Number(p2.korpus), KNOB_SPANNE.korpus),
      phrase: klemm(p2.phrase === void 0 ? KNOB_VORGABE.phrase : Number(p2.phrase), KNOB_SPANNE.phrase),
      satzlaenge: klemm(p2.satzlaenge === void 0 ? KNOB_VORGABE.satzlaenge : Number(p2.satzlaenge), KNOB_SPANNE.satzlaenge)
    };
  } catch {
    return { ...KNOB_VORGABE };
  }
...
function naechsterSlot(text2) {
  const m = text2.match(/⟨(AKK|DAT|NOM|SATZ)⟩/);
  if (!m) return null;
  const k = m[1];
  if (k === "SATZ") return { rolle: "ergaenzung", kasus: "nom", art: "hauptsatz" };
  return { rolle: "objekt", kasus: k.toLowerCase(), art: "nominalphrase" };
}
function wirktSatzwertig(text2) {
  const haupt = text2.split(/[,;–—]/)[0] || text2;
...
var offeneSlots = (t2) => (t2.match(/⟨(AKK|DAT|NOM|SATZ)⟩/g) || []).length;
function verfugen(teile) {
  const out = [];
  for (let i = 0; i < teile.length; i++) {
    let t2 = teile[i].trim().replace(/\s+([.,;:!?])/g, "$1");
    if (!t2) continue;
    const vorOffen = i > 0 && /[:—]$/.test(out[out.length - 1] || "");
    t2 = vorOffen ? t2.charAt(0).toLowerCase() + t2.slice(1) : t2.charAt(0).toUpperCase() + t2.slice(1);
    t2 = t2.replace(
      /^(Und|Doch|Aber|Oder|Denn|Dann|Dabei|Also)\s+([A-ZÄÖÜ])(?=[a-zäöüß])/,
      (_m, k, c) => k + " " + c.toLowerCase()
    );
    const endet = /[.!?…:;—]$/.test(t2);
    const naechsterFolgtDirekt = t2.endsWith(":") || t2.endsWith("\u2014");
    if (!endet) t2 += ".";
...
var beatKopf = (p2) => {
  const w = p2.toLowerCase().replace(/^und\s+/, "").split(/[\s,:;—]+/).filter(Boolean);
  return w[0] || "";
};
...
function frameTurn(turn) {
  const t2 = clean(turn).replace(/[.!?…]+$/, "");
  const frames = [
    `Dann kippt es: ${t2}.`,
    `Dann kippt es \u2014 ${t2}.`,
    `Es braucht nur einen Atemzug, und ${t2}.`,
    `Erst ein Riss, kaum merklich, und ${t2}.`,
    `Und dann, ohne Vorwarnung: ${t2}.`,
    `Etwas gibt nach \u2014 ${t2}.`,
    `Kaum ausgesprochen, ${t2}.`,
    `Dann, unvermittelt: ${t2}.`
  ];
...
function weaveMotif(text2, motif) {
  if (!motif) return text2;
  const motifLine = looksLikeClausePhrase(motif) ? ensurePunct(cap(clean(motif))) : ensurePunct(`Dabei: ${motif}`);
  const s = splitSentences(text2);
  if (s.length < 2) return text2 + " " + motifLine;
...
function insertToneFlavor(text2, line) {
...
function weaveCast(text2, _P, cast) {
  const others = (cast || []).slice(1).map((c) => clean(c)).filter(Boolean);
  if (!others.length) return text2;
  ...
  const sent = splitSentences(text2);
...
function conjugateVerbToken(verb, person) {
  ...
  if (table && table[person]) {
    ...
    const p2 = person === "ich" || person === "du" || person === "wir" || person === "ihr" ? person : "er";
    out = beugeVerb(low2, p2) ?? low2;
  }
...
var ICH_DU_ZU_ER = (() => {
  ...
  for (const [dritte, formen] of Object.entries(VERB_CONJ)) {
    for (const p2 of ["ich", "du", "wir", "ihr"]) {
      const f = formen[p2];
      if (f && !m[f]) m[f] = dritte;
    }
  }
...
function istEigenePerson(teil) {
  const p2 = clean(teil);
  if (!p2) return false;
  if (SP_REL.test(p2) && SP_ENDS_VERB.test(p2)) return false;
  if (SP_CONJ.test(p2) || SP_PREP.test(p2)) return false;
  if (SP_DET.test(p2)) return true;
  if (/^[A-ZÄÖÜ]/.test(p2)) return true;
  return !/\s/.test(p2);
}
...
var weakLooksVerbal = (t2) => {
  const m = t2.match(/\b[a-zäöüß]{3,}(te|ten|test|tet)\b/g);
  if (!m) return false;
  return m.some((w) => {
...
function isPastTense(s) {
  const t2 = s || "";
  if (PRAES_MARK.test(t2)) return false;
  if (PRAET_STRONG.test(t2)) return true;
  if (PRAET_WEAK.test(t2) && weakLooksVerbal(t2) && !ADJ_CONTEXT.test(t2)) return true;
  return (t2.toLowerCase().match(/[a-zäöüß]+/g) || []).some((w) => !!PAST2PRES[w]);
}
...
var silben = (t2) => {
  const w = t2.toLowerCase().match(/[a-zäöüß]+/g) || [];
  return w.reduce((n, x) => n + Math.max(1, (x.match(/[aeiouäöüy]+/g) || []).length), 0);
};
var woerter = (t2) => (t2.match(/\S+/g) || []).length;
var tiefe = (t2) => (t2.match(/,\s*(dass|weil|obwohl|wenn|als|während|nachdem|bevor|damit|ob|indem|der|die|das|den|dem|welche)/gi) || []).length;
function subjektOf(t2, typ) {
  ...
  const m = t2.match(/\b(?:der|die|das|ein|eine)\s+([A-ZÄÖÜ][a-zäöüß-]+)/);
  const g = m ? guessGender(m[1]) : void 0;
  const genus = g === "m" ? "mask" : g === "f" ? "fem" : g === "n" ? "neut" : null;
  const plural = /\b(sie|die)\s+\w+en\b/.test(t2.toLowerCase()) || /\b(sind|waren|haben|werden)\b/.test(t2.toLowerCase());
...
function teilstuecke(text2) {
  return (text2 || "").replace(/\s+/g, " ").split(/(?<=[.!?…])\s+/).flatMap((s) => s.split(/\s*[;—–]\s*/)).map((s) => clean(s).replace(/^[„"«»]+|[.!?…„"«»]+$/g, "").trim()).filter((s) => {
...
function normWhere(s) {
  const t2 = (s || "").trim();
  if (!t2 || PREPS.test(t2)) return t2;
  const komma = t2.indexOf(",");
  if (komma > 0) {
    const kopf = normWhere(t2.slice(0, komma));
    return kopf + t2.slice(komma);
  }
  const zusatz = t2.match(/^(.+?)\s+((?:in|im|an|am|auf|bei|vor|hinter|neben|unter|über|zwischen|nahe|gegenüber|ohne|mit|voller|aus)\s+.+)$/);
  ...
  const np = parseNP(t2);
  if (!np) return t2;
  const nurWort = !np.art && !np.adj && /^[A-ZÄÖÜ][a-zäöüß-]+$/.test(t2);
  const inTabelle = !!(NOUN_GENDER[t2.toLowerCase()] || NOUN_GENDER_2[t2.toLowerCase()]);
  if (nurWort && !inTabelle && ORTSNAME_ENDUNG.test(t2) && !LAND_GATTUNG.has(t2.toLowerCase())) return `in ${t2}`;
  const g = genderOf(np.art, np.noun);
  if (!g) return !np.art && !np.adj && /^[A-ZÄÖÜ][a-zäöüß-]+$/.test(t2) ? `in ${t2}` : t2;
...
function normWhen(s) {
  const t2 = (s || "").trim();
  if (!t2 || PREPS.test(t2) || TIME_ADV.test(t2) || t2.includes(",") || /\d+\s*uhr/i.test(t2)) return t2;
  if (/^\d{3,4}$/.test(t2)) return `im Jahr ${t2}`;
  const one = t2.match(/^([A-ZÄÖÜa-zäöü][A-Za-zÄÖÜäöüß-]*)$/) ? t2 : null;
  if (!one) return t2;
  const w = one;
...
function normWho(s) {
  const t2 = (s || "").trim();
  if (!t2) return t2;
  const parts = t2.split(",").map((p2) => p2.trim()).filter(Boolean);
  const fixed = parts.map((p2, i) => {
    const m = p2.match(/^([a-zäöüß][a-zäöüß-]*)\s+([A-ZÄÖÜ][A-Za-zÄÖÜäöüß-]*)$/);
    ...
    if (i === 0 && /^[A-ZÄÖÜa-zäöüß][a-zäöüß-]+$/.test(p2) && PERSON_NOMEN.test(p2) && !/^(männer|leute)$/i.test(p2)) {
      const wort = cap2(p2);
      const klein = p2.toLowerCase();
    ...
    return i === 0 || istEigenePerson(p2) ? cap2(p2) : low(p2);
  });
  return fixed.join(", ");
}
...
function tokenize(text2) {
  return (text2 || "").replace(/\r/g, "").replace(/([.,!?;:()„""""—])/g, " $1 ").replace(/\s+/g, " ").trim().split(" ").filter(Boolean);
}
...
function applyToneRegister(text2, tone) {
  const reg = toneRegister(tone);
  if (!reg || !text2) return text2;
  if (reg === "plain") {
    let t2 = text2.replace(/\b(gleichsam|wie Honig im Winter|wie ein halb vergessenes Gedicht[^.,;]*)\b/gi, "").replace(/\s{2,}/g, " ");
    t2 = t2.split(/\n\n+/).map((para) => {
      ...
      for (const sen of sents) {
        ...
        if (wc > 16) {
          ...
          if (parts.length > 1) {
            parts.forEach((p2, i) => {
              let seg = p2.replace(/^,?\s*(und|aber|denn|während|sodass|wobei)\s+/i, "").trim();
              if (!seg) return;
              seg = cap1(seg);
              if (!/[.!?…]$/.test(seg)) seg += ".";
              out.push(i === 0 && /[.!?…]$/.test(p2) ? cap1(p2.trim()) : seg);
            });
            continue;
          }
        }
    ...
    return t2.replace(/\s+([,.;:!?…])/g, "$1").replace(/\s{2,}/g, " ").trim();
  }
  if (reg === "wry") {
...
function polishGerman(text2, opts = {}) {
  const { who = "" } = opts;
  let t2 = String(text2 ?? "");
  ...
  if (who.trim()) {
    ...
    try {
      t2 = t2.replace(new RegExp(`(?<![\\p{L}\\p{N}_])${w}(?![\\p{L}\\p{N}_])`, "giu"), wieder);
    } catch {
      t2 = t2.replace(new RegExp(`\\b${w}\\b`, "gi"), wieder);
    }
  }
  for (let k = 0; k < 6; k++) {
    const next = t2.replace(
  ...
  t2 = t2.split(/(?<=[.!?…])(\s+)/).map((teil) => /^\s+$/.test(teil) ? teil : ergaenzeArtikel(teil)).join("");
  return t2.trim();
}

// src/generation/shape.ts
function applyDisruptor(text2, level) {
  const p2 = level === "off" ? 0 : level === "on" ? 0.33 : 0.17;
  if (!chance(p2)) return { text: text2, fired: false, kind: "\u2013" };
  const kinds = [
    { kind: "Zeitbruch", fn: (t2) => t2 + " Drei Jahre sp\xE4ter ist die gleiche Stelle noch da, aber das Ger\xE4usch ist \xE4lter." },
    { kind: "Erz\xE4hlerwechsel", fn: (t2) => t2.replace(/\n\n/g, "\n\n\u2014\n\n") + "\n\nIch \xFCbernehme hier. Nur kurz. Nur, um das Offensichtliche zu sagen." },
    { kind: "Metakommentar", fn: (t2) => t2 + "\n\n(Diese Geschichte wei\xDF, dass sie erz\xE4hlt wird.)" },
    { kind: "Wiederholung", fn: (t2) => {
      const s = splitSentences(t2);
      if (s.length < 3) return t2;
      return t2 + "\n\n" + s[Math.floor(s.length * 0.65)];
    } },
    { kind: "Fragmentierung", fn: (t2) => {
      const s = splitSentences(t2);
      if (s.length < 4) return t2;
      s.splice(Math.floor(s.length / 2), 0, "\u2014");
      return s.join(" ");
    } }
  ];
  const k = pick(kinds);
  return { text: k.fn(text2), fired: true, kind: k.kind };
}
...
function applyRhythm(text2, rhythm) {
  const s = splitSentences(text2);
  ...
  if (rhythm === "staccato") {
    ...
    if (s.length >= 4 && chance(0.6)) {
      ...
      if (cut > 10 && cut < 80 && !NEBENSATZ_ANFANG.test(t2.slice(cut + 2))) {
        s[i] = t2.slice(0, cut) + ".";
        s.splice(i + 1, 0, t2.slice(cut + 2));
      }
    }
    if (chance(0.35)) {
...
function applyTension(text2, peak, material) {
  if (!peak || peak === "off") return text2;
  const center = TENSION_CENTER[peak];
  if (center === void 0) return text2;
  const s = splitSentences(text2);
  if (s.length < 5) return text2;
  const width = 0.26;
  const intensity = (i, n) => {
  ...
  }
  {
    ...
    if (idx > 0 && idx < s.length - 1 && chance(0.5)) {
      const t2 = s[idx].replace(/[.!?…]+$/, "");
      const nachbarn = [s[idx - 1] || "", s[idx + 1] || ""].join(" ").toLowerCase();
      const bruch = pick(["und genau hier kippt es.", "kein Zur\xFCck.", "jetzt.", "und nichts h\xE4lt mehr."].filter((b) => !(b === "jetzt." && /\bjetzt\b/.test(nachbarn + " " + t2.toLowerCase()))));
      if (t2.length > 12 && !isFragmentSentence(t2) && !t2.includes("\u2014")) {
        s[idx] = t2 + " \u2014";
...
function guessPronoun(P2) {
  const p2 = clean(P2);
  if (/^(der|ein)\s/i.test(p2)) return "er";
  if (/^(die|eine)\s/i.test(p2)) return "sie";
  if (/^das\s/i.test(p2)) return "es";
  if (/(a|e|in)$/i.test(p2)) return "sie";
  return "er";
}
...
function objektName(o) {
  const t2 = clean(o);
  if (!t2) return "das Ding";
  if (/^(der|die|das|den|dem|des|ein|eine|einen|einem|einer|eines)\s/i.test(t2)) return t2;
  const kern = t2.split(" ")[0].replace(/[^A-Za-zÄÖÜäöüß]/g, "");
  return `${DEF_ART[guessGender(kern) || "n"]} ${t2}`;
}
var DING_VORRAT = [
  "T\xFCr",
  "Uhr",
...
function applyPerspective(paras, perspective, who, objName) {
  ...
  if (perspective === "object") {
    ...
    return paras.map((p2, i) => i === 0 ? `${einstieg} ${p2}` : p2);
  }
  const cycle = ["first", "second", "third", "object"];
  return paras.map((p2, i) => {
    const k = cycle[i % cycle.length];
    if (k === "first") return toFirst(p2);
    if (k === "second") return toSecond(p2);
    if (k === "object") return toObject(p2);
    return p2;
  });
}
function pronominalize(text2, P2, pronoun) {
  const name = clean(P2);
  if (!name || !pronoun) return text2;
  let re;
  try {
...
function applySatzlaenge(text2, ziel) {
  if (!ziel || ziel < 6) return text2;
...
function glaetten(t2) {
  return t2.replace(/[ \t]{2,}/g, " ").replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").replace(/[ \t]+([,.;:!?])/g, "$1").trim();
}
...
function schliesseFigurenkomma(text2, who) {
  const roh = (who || "").trim();
  if (!roh || !roh.includes(",")) return text2;
  const figur = personKopf(splitSpeakers(normWho(roh))[0] || "");
  if (!figur.includes(",")) return text2;
  try {
    const re = new RegExp("(" + escapeRegExp(figur) + ")(\\s+)(?=[a-z\xE4\xF6\xFC\xDF])", "gi");
    return text2.replace(re, "$1,$2");
  } catch {
    return text2;
  }
}
function coherencePass(text2, input) {
  try {
    if (isLineForm(input)) return text2;
    const t2 = String(text2 || "").replace(/\.\s*\.+/g, ".");
    const paras = t2.split(/\n{2,}/);
    const freq = {};
    coherenceWords(t2).forEach((w) => {
      freq[w] = (freq[w] || 0) + 1;
    });
    ...
    const maxRemove = Math.max(1, Math.floor(splitSentences(t2).length * 0.25));
    let removed = 0;
    const outParas = [];
    paras.forEach((p2, pi) => {
      const sents = splitSentences(p2);
    ...
    return result.length >= 60 ? result : text2;
  } catch {
    return text2;
  }
}
function coherenceRepairV2(t2, input) {
  t2 = String(t2 ?? "");
  t2 = t2.replace(/\(\s*[A-ZÄÖÜ][\wäöüß-]{2,}\s*\)/g, " ");
  t2 = t2.replace(/,\s*([.!?…])/g, "$1");
  t2 = t2.replace(/([.!?…])\s*,/g, ",");
  t2 = t2.replace(/\s*,\s*,\s*/g, ", ");
  t2 = t2.replace(/„\s+/g, "\u201E").replace(/\s+"/g, '"');
  t2 = t2.replace(/([.!?…])\s*\1+/g, "$1");
  if ((t2.match(/"/g) || []).length % 2 === 1) t2 = t2.replace(/"/g, "");
  {
    const o = (t2.match(/„/g) || []).length, c = (t2.match(/[“”]/g) || []).length;
    if (o !== c) t2 = t2.replace(/[„“”]/g, "");
  }
  t2 = t2.replace(/\bich'(?=\s)/gi, "meine").replace(/\bdu'(?=\s)/gi, "deine").replace(/\bwir'(?=\s)/gi, "unsere").replace(/\ber'(?=\s)/gi, "seine").replace(/\bsie'(?=\s)/gi, "ihre").replace(/\bes'(?=\s)/gi, "seine");
  t2 = t2.replace(/(:\s+)([a-zäöüß][^.!?…]*)/g, (m, p1, rest) => looksLikeFullClause(null, rest) || /^(warum|weshalb|wieso|wie|was|wer|wen|wem|wann|wo|wohin|woher|ob)\b/i.test(rest) ? p1 + rest.charAt(0).toUpperCase() + rest.slice(1) : m);
  String(input?.who || "").split(/[,;]/).map((x) => x.trim()).filter(Boolean).forEach((n) => {
    const esc = escapeRegExp(n);
    const wieder = namensErsetzer(n);
    try {
      t2 = t2.replace(new RegExp("\\b(" + esc + ")(s|')?\\b", "giu"), (_m, kern, suf) => wieder(kern) + (suf || ""));
    } catch {
    }
  });
  if (isLineForm(input)) {
  ...
  const ABS = "\u241E";
  t2 = t2.replace(/[ \t]*\n{2,}[ \t]*/g, " " + ABS + " ");
  const sents = t2.split(/(?<=[.!?…])\s+/).filter(Boolean);
  const kept = [];
  for (let s of sents) {
    const bare = s.trim().replace(/["“”»«]+$/, "").replace(/[.!?…]+$/, "").trim();
  ...
  t2 = kept.join(" ").replace(/\s*\u241E\s*/g, "\n\n");
  t2 = t2.replace(/(\bich und [A-ZÄÖÜ][\wäöüß]+[^.!?…]*?)\bsie sich\b/gu, "$1wir uns");
  t2 = t2.replace(/([A-ZÄÖÜ][\wäöüß]+ und ich[^.!?…]*?)\bsie sich\b/gu, "$1wir uns");
  const CONN = [/\bDann kippt es\b/gi, /\bDabei:\s*plötzlich\b/gi, /\bUnd immer wieder\b/gi, /\bAm Ende bleibt klar\b/gi];
  CONN.forEach((re) => {
    let n = 0;
    t2 = t2.replace(re, (m) => ++n > 1 ? "" : m);
  });
  t2 = glaetten(t2).replace(/„[ \t]+/g, "\u201E");
  return t2;
}
function kleinerArtikel(t2) {
  return (t2 || "").replace(/[ \t]+([,;.!?])/g, "$1").replace(
...
function kommaVorInversion(t2) {
  return (t2 || "").replace(NEBENSATZ, "$1, $2 $3");
}
...
function pluralKongruenz(t2, who) {
  const name = (who || "").trim();
  if (!name || !istPluralFigur(name)) return t2;
  const esc = escapeRegExp(name);
  const beuge = (v) => {
    const p2 = beugeVerb(v, "wir");
    return p2 && p2 !== v ? p2 : v;
  };
  let out = t2.replace(new RegExp(`(\\b${esc})\\s+([a-z\xE4\xF6\xFC\xDF]+t)\\b`, "giu"), (m, n, v) => istVerbform(v) ? `${n} ${beuge(v)}` : m);
...
function kleinesPronomen(t2) {
  return (t2 || "").replace(/([;—–][ \t]+)(Ich|Er|Es|Wir|Du|Man|Ihr|Angeblich|Natürlich|Vielleicht|Jedenfalls|Immerhin|Trotzdem|Allerdings|Jetzt|Dann|Hier|Dort|Aber|Und|Doch|Oder|Nur|Noch|Schon)\b/g, (_m, sp, w) => sp + w.toLowerCase()).replace(
...
function postProcessText(txt, input) {
  ...
  if (name) {
    ...
    try {
      t2 = t2.replace(new RegExp(`(?<![\\p{L}\\p{N}_])${esc}(?![\\p{L}\\p{N}_])`, "giu"), wieder);
    } catch {
  ...
  if (!isLineForm(input) && input?.tone && TONE_DATA[input.tone]) {
    ...
    if (td.opener.length) {
      const kopf = t2.match(OBJEKT_KOPF_RE);
      t2 = kopf ? `${kopf[1]} ${pick(td.opener)} ${t2.slice(kopf[0].length)}` : `${pick(td.opener)} ${t2}`;
    }
    if (td.flavor.length) {
      const wc = t2.trim().split(/\s+/).filter(Boolean).length;
  ...
  if (!isLineForm(input)) t2 = applySatzlaenge(t2, loadKnobs().satzlaenge);
  if (!isLineForm(input)) t2 = entferneDubletten(t2);
...
function buildCircle(kit) {
  ...
  let t2 = joinBeats([a, b, c, frameTurn(kit.turn), reframeStake(kit.stake), kit.ending], kit.P);
  t2 = weaveMotif(t2, kit.motif);
  t2 += " " + ensurePunct(pick([`Und wieder: ${kit.hook}`, `Und von vorn: ${kit.hook}`, `Der Kreis schlie\xDFt sich: ${kit.hook}`]));
  return t2;
}
...
function verwandleMotive(text2, paare) {
  if (!text2 || !paare.length) return text2;
  let t2 = text2;
  for (const { von, nach } of paare) {
    ...
    try {
      ...
      t2 = t2.replace(re, (ganz, davor, wort) => {
        gesehen++;
...
function isSaneMarkov(s) {
  ...
  const pc = {};
  for (const p2 of phrases) pc[p2] = (pc[p2] || 0) + 1;
...
function smoothMarkov(s) {
  ...
  t2 = t2.replace(/[\s,;:—–-]+$/, "");
  if (t2 && !/[.!?…]$/.test(t2)) t2 += ".";
  t2 = t2.replace(/^([a-zäöüß])/, (c) => c.toUpperCase());
  return t2;
}
...
function enforceWordTarget(text2, target, bank, model, markovMode = "mix") {
  const t0 = (text2 || "").trim();
  if (!t0) return t0;
  const tol = 10;
  let out = t0;
...
function pruefeAbgleich(endtext) {
  const norm = (t2) => t2.toLowerCase().replace(/[^a-zäöüß ]/g, " ").replace(/\s+/g, " ").trim();
...
var traegtPerson = (t2) => (t2.toLowerCase().match(/[a-zäöüß]+/g) || []).some((w) => !!ICH_DU_ZU_ER[w]);
function buildPool(bank, perspektive, what, figur, model, markovMode) {
  ...
  if (w) {
    ...
    for (const t2 of saetze) {
      const d = deriveAtom(t2);
...
var wirktNominal = (t2) => /^\s*(ein|eine|einen|einem|eines|einer|der|die|das|den|dem|des|mein|meine|meinen|sein|seine|ihr|ihre|kein|keine|viele|manche|jede|jeden|etwas|nichts|[A-ZÄÖÜ])/.test(t2);
...
function buildRekombination(bank, input, model) {
  ...
  const anfangVon = (t2) => t2.toLowerCase().replace(/[^a-zäöüß ]/g, "").trim().split(/\s+/).slice(0, 3).join(" ");
  resetTrace();
  let fuegeteile = 0;
  ...
  for (let s = 0; s < 600; s++) {
    ...
    if (!a) break;
    let text2 = fuelleKontext(a.text, { ...ctx, figur: waehleFigur() });
    const fueller = [];
    let guard = 0;
    while (offeneSlots(text2) && guard++ < 3) {
      const kf = { ...k, vorheriges: a, offenerKopf: false };
      const slot = naechsterSlot(text2);
      ...
      text2 = fuelleSlot(text2, fill);
      fueller.push({ text: fill, kategorie: f.kategorie || "\u2014", quelle: f.quelle });
      k.benutzt.add(f.id);
      benutztBei.set(f.id, out.length);
    }
    if (offeneSlots(text2)) continue;
    const sig = text2.toLowerCase().replace(/[^a-zäöüß ]/g, "").replace(/\s+/g, " ").trim();
    ...
    for (const wert of [ctx.ort, ctx.zeit]) {
      if (zaehleIn(text2, wert) && zaehleIn(bisher, wert) >= W4_MAX) {
        zuOft = true;
        break;
      }
    }
    ...
    if (PHRASE > 0) {
      ...
      const wds = ohne4W(text2).match(/[a-zäöüß]{2,}/g) || [];
    ...
    pushTrace({ text: text2, quelle: a.quelle, kategorie: a.kategorie || "\u2014", typ: a.typ, phase, fueller: fueller.length ? fueller : void 0 });
...
function buildVersAtome(bank, input, model) {
  ...
  const raus = [];
  for (const a of pool) {
    const t2 = fuelleKontext(a.text, ctx);
    if (offeneSlots(t2)) continue;
    if (a.typ === "kopf") continue;
    const rein = t2.replace(/[.!?…:;]+$/, "").trim();
...
var Buchfuehrung = class {
  ...
  person(p2) {
    if (this.drin.has(p2.id)) return p2.kurz;
    this.drin.add(p2.id);
    return `${p2.rolle} ${p2.name}`;
  }
  eingefuehrt(p2) {
    return this.drin.has(p2.id);
  }
  organisation(fb) {
...
function zitat(fb, bank, b, benutzt, welche, vorrat) {
  const p2 = fb.personen[welche];
  if (!p2 || !p2.zitierfaehig) return "";
  const kern = satzOhneZahl(bank, ["hooks", "stakes"], benutzt, vorrat) || "Wir haben lange gewartet";
  return `\u201E${cap(kern)}\u201C, sagte ${b.person(p2)}.`;
}
...
function quelle(fb) {
  const p2 = fb.personen[0];
  if (!p2) return "";
  return `Das teilt ${p2.rolle} ${p2.name} mit.`;
}
function schritt(fb) {
...
function applyEmphasis(text2, kit, w) {
  const gens = [
    [w.wo, () => placeLine(kit)],
    [w.wann, () => timeLine(kit)],
    [w.wer, () => charLine(kit)],
  ...
  if (!uniq.length) return text2;
  const sents = splitSentences(text2);
...
function reimDedupePhrases(phrases) {
  ...
  for (const p2 of phrases) {
    const prefix = p2.toLowerCase().replace(/[.,;:!?…]/g, "").split(/\s+/).filter(Boolean).slice(0, 3).join(" ");
...
function verseLine(s) {
  let t2 = capLine(s);
  const q2 = (t2.match(/["„“”]/g) || []).length;
  if (q2 % 2 === 1) t2 = t2.replace(/["„“”]/g, "");
  t2 = t2.replace(/^[\s"„“”'’]+/, "");
...
function applyReimPoem(rawText, anchorLine = "", lenTarget = 0, atome = []) {
  ...
  let t2 = normalizeNewlines(rawText || "").trim().replace(/\([^()]*\)/g, " ").replace(/\bShot\s*\d+\b.*$/gim, "").replace(/\b\d{1,2}\s*:\s*\d{2}\b\s*—\s*/g, "").replace(/\s+/g, " ").trim();
  let phrases = [];
  if (atome.length >= 6) {
    phrases = atome.map((a) => a.trim()).filter((a) => a.length >= 6);
  } else {
    for (const s of splitSentences(t2)) phrases.push(...String(s).split(/[,;:—–]\s*/g).map((p2) => p2.trim()).filter(Boolean));
  }
  phrases = phrases.map((p2) => p2.replace(/^Und\s+/i, "").trim()).filter((p2) => p2.length >= 6);
...
function haikuCandidatesFromPhrases(phrases) {
  const out = [];
  const seen = /* @__PURE__ */ new Set();
  phrases.forEach((p2, src) => {
    const words = p2.replace(/[.,;:!?…()]/g, "").split(/\s+/).filter(Boolean);
    for (let a = 0; a < 1; a++) {
      for (let n = a + 2; n <= Math.min(a + 8, words.length); n++) {
        ...
        const text2 = sub.join(" "), key = text2.toLowerCase();
        if (seen.has(key)) continue;
        seen.add(key);
        out.push({ text: text2, syll: haikuSyllOf(text2), src, ganz: a === 0 && n === words.length });
      }
    }
  });
  return out;
...
function applyHaikuPoem(rawText, anchorLine = "", lenTarget = 0, atome = []) {
  ...
  let t2 = normalizeNewlines(rawText || "").trim().replace(/\([^()]*\)/g, " ").replace(/[„“”"»«]/g, " ").replace(/\b(den|dem|einen|einem|der|die|das)\s+Satz\b/gi, " ").replace(/\bShot\s*\d+\b.*$/gim, "").replace(/\b\d{1,2}\s*:\s*\d{2}\b\s*—\s*/g, "").replace(/\s+/g, " ").trim();
  let phrases = [];
  for (const s of splitSentences(t2)) phrases.push(...String(s).split(/[,;:—–]\s*/g).map((p2) => p2.trim()).filter(Boolean));
  phrases = phrases.map((p2) => p2.replace(/^Und\s+/i, "").trim()).filter((p2) => p2.length >= 4);
  const concrete = phrases.filter((p2) => !/^(aber|denn|weil|dass|ob|doch|also)\b/i.test(p2)).filter((p2) => !/\b(Wahrheit|Bedeutung|Einsatz|Gültigkeit|Prinzip|Kontrolle|bedeutet|vielleicht)\b/i.test(p2));
  ...
  const sourceWords = [];
  for (const p2 of phrases) sourceWords.push(...p2.replace(/[.,;:!?…]/g, "").split(/\s+/).filter(Boolean));
  ...
  const greedyLine = (target) => {
    ...
    for (let versuch = 0; versuch < 8; versuch++) {
      ...
      const text2 = lw.join(" ");
      const abw = Math.abs(haikuSyllOf(text2) - target);
      if (abw < besteAbw) {
        besteAbw = abw;
        bester = text2;
      }
  ...
  for (let h = 0; h < opts.maxHaikus; h++) {
    ...
    let l2 = fromMaterial(t22) || fromBank(HAIKU_NATURE7, t22) || fromMaterial(t22, false) || greedyLine(t22);
    const ganzKnapp = (() => {
    ...
    const f2 = passeSilben(l2.replace(/\s*–\s*$/, ""), t22, haikuSyllOf) + (/–\s*$/.test(l2) ? " \u2013" : "");
...
function applyStrangPoem(rawText, anchorLine = "", lenTarget = 0) {
  ...
  let t2 = normalizeNewlines(rawText || "").trim().replace(/\([^()]*\)/g, " ").replace(/\bShot\s*\d+\b.*$/gim, "").replace(/\bHandheld\b.*$/gim, "").replace(/\b\d{1,2}\s*:\s*\d{2}\b\s*—\s*/g, "").replace(/\s+/g, " ").trim();
  t2 = t2.replace(/\bDer\s+Einsatz\s+ist\s*[:,]?\s*([^.!?\n]+)[.!?]?/gi, (_m, x) => {
    const k = (x || "").trim();
    return k ? `Wenn es wahr wird, verlieren wir ${k}.` : "";
  });
  let phrases = [];
  for (const s of splitSentences(t2)) phrases.push(...String(s).split(/[,;:—–]\s*/g).map((p2) => p2.trim()).filter(Boolean));
  phrases = phrases.map((p2) => p2.replace(/^(Und|Aber|Denn|Doch|Also)\s+/i, "").trim()).filter((p2) => p2.length >= 6).filter((p2) => !/^Alles\s+ist\s+korrekt/i.test(p2));
  phrases = reimDedupePhrases(phrases);
  const anchor = anchorLine.trim();
  ...
  for (const p2 of phrases) {
    ...
    for (const b of breakIntoLines(p2, target, opts.maxCharsPerLine)) lines.push(b);
    li++;
  }
...
function buildDramaConflict(whoA, whoB, seed) {
  const p2 = (arr, i) => arr[Math.abs(i) % arr.length];
  const h = hashString(seed || (whoA || "A") + "|" + (whoB || "B"));
  return {
    whoA: whoA || "A",
    whoB: whoB || "B",
    WILL: `${whoA || "A"} ${p2(DRAMA_WILLS, h)}`,
    BLOCKADE: `Aber ${p2(DRAMA_BLOCK, h + 7)}.`,
    VERLUST: `Wenn ${whoA || "A"} es versucht, ${p2(DRAMA_LOSS, h + 13)}.`
  };
}
var toSentences = (text2) => normalizeNewlines(text2).replace(/\s+/g, " ").split(/(?<=[.!?])\s+/).map((s) => s.trim()).filter(Boolean);
var toLines = (text2) => normalizeNewlines(text2).split("\n").map((l) => l.trim()).filter(Boolean);
var looksLineBased = (text2) => {
  const lines = text2.split("\n").filter(Boolean);
  return lines.length >= 6 && lines.length > text2.split(".").length;
};
var isPureMeta = (u) => /^(\d{1,2}:\d{2}|shot\s*\d+|\(.*s\s*pro\s*shot.*\)|handheld|micro-?shake)\b/i.test(u);
var stripCinematicMarkers = (text2) => text2.split("\n").filter((l) => !isPureMeta(l.trim())).join("\n");
function enforceCinematicConsequence(text2) {
  const lines = normalizeNewlines(text2).split("\n");
  const out = [];
  for (let i = 0; i < lines.length; i++) {
...
function reduceAbstraction(units) {
  ...
  return units.map((u) => u.trim()).filter((u) => {
    ...
    if (!abstract.some((p2) => low2.includes(p2))) return true;
...
function dedupeSoft(units, maxRepeat = 2) {
  ...
  out = out.filter((u) => {
    const keyTokens = (u.toLowerCase().match(/[a-zäöüß]+/g) || []).filter((t2) => t2.length >= 6);
    for (const t2 of keyTokens) counts.set(t2, (counts.get(t2) || 0) + 1);
    const over = keyTokens.filter((t2) => (counts.get(t2) || 0) > maxRepeat).length;
...
function applyDramaModule(rawText, conflict, userOpts = {}) {
  ...
  let text2 = normalizeNewlines(rawText).trim();
  text2 = opts.allowCinematicMarkers ? enforceCinematicConsequence(text2) : stripCinematicMarkers(text2);
  const lineBased = looksLineBased(text2);
  let units = (lineBased ? toLines(text2) : toSentences(text2)).map((u) => u.trim()).filter((u) => u.length > 0).filter((u) => !isPureMeta(u));
...
function asProsePoem(text2) {
  const s = text2.replace(/\s+/g, " ").split(/(?<=[.!?…])\s+/).filter(Boolean);
  const lines = [];
  for (let i = 0; i < s.length; i++) {
    lines.push(s[i]);
...
function asStrang(text2, anchor = "", lenTarget = 0) {
  return applyStrangPoem(text2, anchor, lenTarget);
}
function asReim(text2, anchor = "", lenTarget = 0, atome = []) {
  return applyReimPoem(text2, anchor, lenTarget, atome);
}
function asHaiku(text2, anchor = "", lenTarget = 0, atome = []) {
  return applyHaikuPoem(text2, anchor, lenTarget, atome);
}
function asDrama(text2, whoA, whoB) {
  return applyDramaModule(text2, buildDramaConflict(whoA, whoB, (whoA || "") + "|" + (whoB || "")));
}
...
function buildStory(bank, input, model) {
  ...
  let text2 = input.form === "prose" && input.structure === "dramaturgie" && hasDramaData() ? buildDramaturgie({ ...kit }) : pickStructureBuilder(effStructure)({ ...kit });
  if (input.form === "prose" && kit.cast.length >= 2) text2 = weaveCast(text2, kit.P, kit.cast);
  if (input.form === "prose" && input.emphasis) text2 = applyEmphasis(text2, kit, input.emphasis);
  text2 = applyDisruptor(text2, input.disruptor).text;
  text2 = applyRhythm(text2, kit.rhythm);
  if (input.form === "prose") text2 = applyTension(text2, input.tension, { motifs: bank.motifs, hooks: bank.hooks });
  text2 = paragraphize(text2);
  const paras = text2.split(/\n\n+/).map(clean).filter(Boolean);
  text2 = effStructure === "object" ? paras.join("\n\n") : applyPerspective(paras, kit.perspective, kit.P, pick(kit.mode.nouns)).join("\n\n");
  if (kit.perspective === "third") text2 = pronominalize(text2, kit.P, guessPronoun(kit.P));
  const finalText = postProcessText(text2, input);
...
ist("Widerstand \u2192 Hindernis", kategorieFuer("Das Gebet wird erh\xF6rt, aber anders", false), "obstacles");
ist("Wende \u2192 Wende", kategorieFuer("Pl\xF6tzlich kippt die Stimmung im Dorf", false), "turns");
...
var t = teilstuecke("Ein Satz. Zu kurz. Ein zweiter Satz; und sein Teil \u2014 noch ein Teil dahinter.");
wahr("an Semikolon und Strich geteilt", t.includes("und sein Teil") && t.includes("noch ein Teil dahinter"));
wahr("Zweiw\xF6rter fallen weg", !t.includes("Zu kurz"));
var text = (0, import_fs.readFileSync)("test/regression.data.ts", "utf8").length > 0 ? `Es begann leise, so wie das Schlimmste meistens beginnt. Der Glaube verlangt einen Sprung. Eine Prozession zieht durch nasse Felder. Ein St\xFCck Brot vom Altar. Der Beichtstuhl bleibt heute leer. Das Licht wechselt die Quelle. Eine Taube, die durch Mauern fliegt. Das Brot reicht, und niemand kann es erkl\xE4ren. Der Ausweg ist verstellt, seit Wochen. Die Prozession f\xE4llt aus, zum ersten Mal seit hundert Jahren. Ein Weihrauchfass an einer Kette. Eine Bibel mit Zetteln zwischen den Seiten. Das Wasser tr\xE4gt nicht. Das Gebet wird erh\xF6rt, aber anders. Pl\xF6tzlich kippt die Stimmung im Dorf. Es geht um den Glauben selbst. Dann beginnt die Glocke von allein zu l\xE4uten. Am Ende bleibt nur ein Wappen im Staub des Hofes.` : "";
var p = presetAusText(text);
wahr("keine Kategorie bleibt leer", Object.values(p.bank).every((v) => v.length >= 1));
wahr("nichts Verwertbares geht verloren", Object.values(p.bank).reduce((n, v) => n + v.length, 0) === p.stuecke);
...
var ok = 0;
for (let i = 0; i < 5; i++) {
  const txt = buildStory(p.bank, inp);
...
\u274C Preset aus Text: ${fails.length} Fehler:`);
  ...
  console.log(`
\u2705 Preset aus Text: alle ${geprueft} Pr\xFCfungen bestanden.`);
}
