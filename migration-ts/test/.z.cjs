...
var SPRACHE_LABEL = {
  ...
  amtlich: "amtlich",
  erzaehlend: "erz\xE4hlend",
  feierlich: "feierlich",
  koerperlich: "k\xF6rperlich",
  bildhaft: "bildhaft"
};
var REGISTER = {
  // ── real ──────────────────────────────────────────────────────────────────
...
var EIGEN_KEY = "divergenz_eigene_register_v1";
function ladeEigene() {
  try {
    const r = JSON.parse(localStorage.getItem(EIGEN_KEY) || "{}");
    return r && typeof r === "object" ? r : {};
  } catch {
    return {};
  }
}
function sichereEigene(m) {
  try {
    localStorage.setItem(EIGEN_KEY, JSON.stringify(m));
    return true;
  } catch {
    return false;
  }
}
function setzeEigenes(id, welt, sprache) {
  const m = ladeEigene();
  const k = id.replace(/^user:/, "");
  if (welt && sprache) m[k] = { welt, sprache };
  else delete m[k];
  sichereEigene(m);
}
function registerVon(id) {
  const k = id.replace(/^builtin:/, "").replace(/^user:/, "");
  return REGISTER[k] || ladeEigene()[k] || null;
}
var WELT_RANG = { real: 0, gehoben: 1, irreal: 2 };
var SPRACH_RANG = {
  nuechtern: 0,
  amtlich: 1,
  erzaehlend: 2,
...
function abstand(a, b) {
  ...
  return Math.round((w + s) * 500) / 1e3;
}
function mischAbstand(ids2) {
  const regs = ids2.map(registerVon).filter((r) => r !== null);
...
function bekannteRegister(ids2) {
  return ids2.filter((i) => registerVon(i) !== null).length;
}
function waehleGespreizt(vorrat2, anzahl, rnd = Math.random, versuche = 6) {
  const n = Math.max(1, Math.min(anzahl, vorrat2.length));
  if (!vorrat2.length) return [];
  const frei = vorrat2.slice();
  const zieh = () => frei.splice(Math.min(frei.length - 1, Math.floor(rnd() * frei.length)), 1)[0];
  const raus = [zieh()];
...
ist(
  "jedes eingebaute Preset hat ein Register",
  ids.filter((i) => !REGISTER[i]).join(", "),
  ""
);
ist(
  "und kein Register zeigt auf ein Preset, das es nicht gibt",
...
for (const [id, r] of Object.entries(REGISTER)) {
  if (!WELT_LABEL[r.welt] || !SPRACHE_LABEL[r.sprache]) {
    fails.push(`\u201E${id}\u201C tr\xE4gt ein unbekanntes Register: ${r.welt}/${r.sprache}`);
  }
}
...
wahr("der Vorsatz \u201Ebuiltin:\u201C st\xF6rt nicht", registerVon("builtin:kafka") !== null);
...
ist("Faust erz\xE4hlt", REGISTER.faust?.sprache, "erzaehlend");
ist("Eichendorff beschw\xF6rt", REGISTER.eichendorff?.sprache, "feierlich");
wahr(
  "beide Stufen sind besetzt",
  Object.values(REGISTER).some((r) => r.sprache === "erzaehlend") && Object.values(REGISTER).some((r) => r.sprache === "feierlich")
);
ist("Clown spricht k\xF6rperlich", REGISTER.clown?.sprache, "koerperlich");
...
ist(
  "dasselbe Register hat Abstand null",
  abstand({ welt: "real", sprache: "amtlich" }, { welt: "real", sprache: "amtlich" }),
  0
);
ist(
  "die beiden Enden haben Abstand eins",
  abstand({ welt: "real", sprache: "nuechtern" }, { welt: "irreal", sprache: "bildhaft" }),
  1
);
ist(
  "ein voller Weltwechsel allein ergibt ein halbes",
  abstand({ welt: "real", sprache: "amtlich" }, { welt: "irreal", sprache: "amtlich" }),
  0.5
);
ist(
  "ein voller Sprachwechsel allein ebenso",
  abstand({ welt: "real", sprache: "nuechtern" }, { welt: "real", sprache: "bildhaft" }),
  0.5
);
ist("ohne Register kein Abstand", abstand(null, { welt: "real", sprache: "amtlich" }), 0);
var weit = mischAbstand(["formalismus", "griechischetragoedie"]);
wahr(`Formalismus und Trag\xF6die liegen weit auseinander (${weit})`, weit >= 0.5);
var nah = mischAbstand(["biologie", "geologie"]);
wahr(`zwei Naturpresets nicht (${nah})`, nah < 0.2);
var gemischt = mischAbstand(["biologie", "geologie", "myth"]);
...
var juergens = mischAbstand(["bergwelt", "formalismus", "griechischetragoedie"]);
wahr(`die gemeldete Mischung ist gespreizt (${juergens})`, juergens >= 0.3);
var vorrat = Object.keys(REGISTER);
var besser = 0;
...
wahr(`gespreizt schl\xE4gt blind in ${besser} von 400 L\xE4ufen`, besser >= 240);
var paare = new Set(Array.from({ length: 200 }, () => waehleGespreizt(vorrat, 2, zufall).join("+")));
wahr(`die Wahl streut \xFCber ${paare.size} verschiedene Paare`, paare.size >= 40);
...
wahr(
  "keine Wiederholung in einer Mischung",
  new Set(waehleGespreizt(vorrat, 3, zufall)).size === 3
);
localStorage.removeItem(EIGEN_KEY);
var weitOhne = mischAbstand(["formalismus", "griechischetragoedie", "user:Eigenes"]);
var weitMit = mischAbstand(["formalismus", "griechischetragoedie"]);
...
ist(
  "ein einzelnes bekanntes Preset ergibt keinen Abstand",
  mischAbstand(["formalismus", "user:A", "user:B"]),
  0
);
...
var wbQuelle = (0, import_fs.readFileSync)("src/ui/wordbankView.ts", "utf8");
wahr("es gibt eine eigene Zuordnungsliste", /Register eigener Presets/.test(wbQuelle));
ist(
  "und keine Abfrage beim Sichern mehr",
  /Welche Welt baut/.test(wbQuelle),
  false
);
wahr(
  "die Liste ruehrt keine Bank an",
...
console.log(`Pr\xFCfstand Register \u2014 ${geprueft} Pr\xFCfungen:`);
...
\u274C ${fails.length} Fehler bei den Registern:`);
  ...
  console.log(`
\u2705 Register: alle ${geprueft} Pr\xFCfungen bestanden.`);
}
