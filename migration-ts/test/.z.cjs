  ...
  const schlag = (name, erster) => {
    ...
    switch (name) {
      ...
      case "regel": {
        ...
        return z ? cap(ensurePunct(z)) : ensurePunct(pick(M2.rules));
      }
      case "mitte": {
...
Alice Amelie Beatrice Bianca Camilla C\xE9cile Diana Dora Edda Eleonore Elise Emilie Fanny Flora Gerlinde Hertha Ida Irma Jolanda Josefine K\xE4the Klementine Konstanze Leni
...
`;
var M = `
Achim Adam Adrian Albert Alexander Alfred Alois Andreas Anton Armin Arno Arnold Arthur Axel Benedikt Benjamin Bernd Bernhard Bertram Bj\xF6rn Bodo Boris Bruno Burkhard
...
Erik Ernst Erwin Eugen Fabian Falk Felix Ferdinand Florian Frank Franz Fred Friedrich Fritz Georg Gerd Gerhard Gernot Gottfried Gregor G\xFCnter G\xFCnther Gustav Hagen Hannes
Hans Harald Hartmut Heiner Heinrich Heinz Helmut Henning Herbert Hermann Holger Horst Hubert Hugo Ingo Jakob Jan Jens Joachim Jochen Johann Johannes Jonas Jonathan J\xF6rg
Josef Julian Julius J\xFCrgen Kai Karl Karsten Kaspar Klaus Konrad Konstantin Kurt Lars Leo Leon Leonhard Leopold Lorenz Lothar Ludwig Lukas Lutz Manfred Marcel Marco Marcus
...
Richard Robert Roland Rolf Roman Rudolf R\xFCdiger Samuel Sebastian Siegfried Simon Stefan Steffen Stephan Sven Theodor Thomas Tim Timo Tobias Tom Torsten Udo Ulrich Uwe
...
`;
var VORNAMEN = /* @__PURE__ */ new Map();
for (const n of W.split(/\s+/).filter(Boolean)) VORNAMEN.set(n, "f");
for (const n of M.split(/\s+/).filter(Boolean)) VORNAMEN.set(n, "m");

// src/features/namenwaechter.ts
var DATIV = /^(mit|von|bei|zu|aus|nach|seit|gegenüber|außer|entgegen|ab|an|auf|in|über|unter|vor|hinter|neben|zwischen)$/i;
var AKKUSATIV = /^(für|ohne|gegen|um|durch|bis|wider|entlang)$/i;
var PRON = {
  f: { nom: "sie", akk: "sie", dat: "ihr", poss: "ihr" },
  m: { nom: "er", akk: "ihn", dat: "ihm", poss: "sein" }
};
function possessiv(g, naechstes) {
...
function entnamen(text) {
  const ersetzt = [];
  if (!text) return { text, ersetzt };
  const teile = text.split(/(\s+)/);
  const wort = (i) => (teile[i] || "").replace(/[^A-Za-zÄÖÜäöüß]/g, "");
  const istSatzanfang = (i) => {
    for (let j = i - 1; j >= 0; j--) {
      if (!teile[j] || /^\s+$/.test(teile[j])) continue;
      return /[.!?…]["“»]?$/.test(teile[j]);
    }
    return true;
  };
  for (let i = 0; i < teile.length; i++) {
    ...
    if (!g && /s$/.test(kern) && VORNAMEN.has(kern.slice(0, -1))) {
      kern = kern.slice(0, -1);
      genitivS = "s";
      g = VORNAMEN.get(kern);
    }
    ...
    let jPrev = i - 1;
    while (jPrev >= 0 && /^\s+$/.test(teile[jPrev] || "")) jPrev--;
    const prev = wort(jPrev).toLowerCase();
    if (/^(der|die|das|den|dem|des|ein|eine|einen|einem|einer)$/.test(prev)) continue;
    let jNext = i + 1;
    while (jNext < teile.length && /^\s+$/.test(teile[jNext] || "")) jNext++;
    const naechstes = teile[jNext] || "";
    let durch;
    if (genitivS && /^[A-ZÄÖÜ]/.test(naechstes)) durch = possessiv(g, naechstes);
    else if (DATIV.test(prev)) durch = PRON[g].dat;
    else if (AKKUSATIV.test(prev)) durch = PRON[g].akk;
...
function bogenAus(e) {
  ...
  drama.name = `Erz\xE4hlerbank: ${e.titel || "Ohne Titel"}`;
  return drama;
}
...
function buildLinear(kit) {
  ...
  const modeSpice = pick([
    `Es riecht ${rot("mode.img", M2.images)}. ${rot("mode.rule", M2.rules)}`,
    `${rot("mode.rule", M2.rules)} Es riecht ${rot("mode.img", M2.images)}.`,
    `Irgendwo ${rot("mode.img", M2.images)}. ${rot("mode.rule", M2.rules)}`
  ]);
...
function buildReverse(kit) {
  ...
  const rule = `${rot("mode.rule", M2.rules)} Es riecht ${rot("mode.img", M2.images)}.`;
...
function buildCircle(kit) {
  ...
  const b = kit.AisClause ? `${kit.P} bemerkt: ${kit.Apure}. ${rot("mode.rule", M2.rules)}` : `${kit.P} ${kit.AleadVerb || (kit.AisInfinitiveLed ? "will" : "sucht")} ${kit.Apure}. ${rot("mode.rule", M2.rules)}`;
...
function buildFragment(kit) {
  ...
  const beats = [
    ...
    cap(ensurePunct(rot("mode.rule", M2.rules))),
    cap(ensurePunct(`Es riecht ${rot("mode.img", M2.images)}`)),
...
function buildObjectCentric(kit) {
  ...
  const d = ensurePunct(rot("mode.rule", M2.rules));
...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank, tone = "neutral") {
  ...
  for (let i = 0; i < shotCount; i++) {
    ...
    if (stufe >= 1 && !letzter2 && !teile.some((t) => t.startsWith("Nah: "))) {
      const r = zieh(requisiten, istNP(stripTailPunct(kit.prop)) ? stripTailPunct(kit.prop) : "");
      if (r) setze(teile, `Nah: ${nominativ(r)}.`);
    }
    if (stufe >= 2 && !erster && !letzter2 && hindernisse.length && i >= Math.floor(shotCount / 3)) {
    ...
    if (stufe >= 4) {
      const b22 = zieh(bilder, "");
      if (b22) setze(teile, `${cap(b22)}.`);
    }
    if (letzter2) teile.push(`Nur ${pick(["der Riss", "das Fenster", "die Karte", "das Licht"])} bleibt sichtbar.`);
    if (!letzter2) teile.push(zieh(SCHNITT, "") || "Schnitt.");
...
function placeLine(kit) {
  ...
  return pick([
    ...withW,
    ...withW,
    `Es riecht ${pick(M2.images)}.`,
    ensurePunct(cap(pick(M2.rules))),
...
function buildKit(bank, input, model) {
  ...
  const M2 = MODE_DATA[modeKey] || MODE_DATA.bureau;
...
{
  const qs = (0, import_fs.readFileSync)("src/ui/studio.ts", "utf8");
  wahr("der W\xFCrfel w\xFCrfelt die Bauform mit", /const rollBauform = \(\): void => \{/.test(qs) && /rollPresets\(\);\s*\n\s*rollBauform\(\);/.test(qs));
...
}
{
  ...
  wahr("w\xFCrfeln: Zahl der brauchbaren und der zuletzt gezogene", /Zuletzt gezogen: „/.test(qv));
  wahr("der Kopftext erkl\xE4rt die drei Dinge", /Drei Dinge, von oben nach unten: der Arbeitsplatz/.test(qv));
  const qs = (0, import_fs.readFileSync)("src/ui/studio.ts", "utf8");
