...
var VERB_INFINITIVE = new Set(GRUND.split(/\s+/).map((w2) => w2.trim()).filter((w2) => w2.length > 2));
...
function kenntInfinitiv(wort) {
  const w2 = wort.toLowerCase();
  if (VERB_INFINITIVE.has(w2)) return true;
  for (const p of VERB_PRAEFIXE) {
    if (w2.startsWith(p) && w2.length > p.length + 3 && VERB_INFINITIVE.has(w2.slice(p.length))) return true;
  }
  return false;
}
...
function istLexikonVerb(wort) {
  const w2 = wort.toLowerCase().replace(/[^a-zäöüß]/g, "");
  if (!w2 || w2.length < 3) return false;
  if (starkMitPraefix(w2)) return true;
  if (kenntInfinitiv(w2)) return true;
  if (PAST2PRES[w2]) return true;
  if (/^(bin|bist|sind|seid|habe|hast|habt|werde|wirst|werdet|wäre|wären|hätte|hätten|würde|würden|sei|seien)$/.test(w2)) return true;
  for (const suffix of ["etest", "test", "eten", "ten", "ete", "te", "est", "st", "et", "en", "t", "e", "tet"]) {
    if (!w2.endsWith(suffix) || w2.length - suffix.length < 2) continue;
    const st = w2.slice(0, -suffix.length);
    if (/ier$/.test(st)) return true;
    if (infinitivZuStamm(st)) return true;
  }
  const pz = w2.match(/^(?:[a-zäöü]{2,8})?ge(.+?)(?:t|en)$/);
...
function istVerbform(wort) {
  const w2 = wort.toLowerCase();
  if (starkMitPraefix(w2)) return true;
  if (KEIN_VERB.has(w2)) return false;
  if (/^[a-zäöüß]{3,}(t|st|e|en)$/.test(w2) && istLexikonVerb(w2)) return true;
  if (!/^[a-zäöüß]{3,}t$/.test(w2)) return false;
  if (/^ge[a-zäöüß]{2,}t$/.test(w2)) return GE_VERBEN.test(w2);
  return true;
}
function beugeVerb(form3, person) {
  const gross = /^[A-ZÄÖÜ]/.test(form3);
  const w2 = form3.toLowerCase();
  const fertig = (s) => gross ? s.charAt(0).toUpperCase() + s.slice(1) : s;
  if (person === "er" || person === "sie") return istVerbform(w2) ? form3 : null;
  const st = starkMitPraefix(w2);
  if (st) {
    const [p, [ich, du, wir, ihr]] = st;
    const f = person === "ich" ? ich : person === "du" ? du : person === "wir" ? wir : ihr || wir.replace(/e?n$/, "t");
    return fertig(p + f);
  }
  if (!istVerbform(w2)) return null;
  let stamm = w2.slice(0, -1);
  const bindevokal = /[td]et$/.test(w2) || /(chn|ffn|gn|tm|dm|ckn|kn)et$/.test(w2);
  if (bindevokal) stamm = w2.slice(0, -2);
  if (person === "ihr") return fertig(w2);
  if (person === "wir") {
  ...
  if (person === "du") {
    ...
    if (SIBILANT.test(stamm)) return fertig(w2);
...
function istSubstantivierterInfinitiv(w2) {
  if (!/^[a-zäöüß]{4,}en$/.test(w2)) return false;
  const stamm = w2.slice(0, -2);
...
function guessGender(noun) {
  const w2 = (noun || "").toLowerCase().replace(/[^a-zäöüß]/g, "");
  const known = NOUN_GENDER2[w2];
  if (known === "m" || known === "f" || known === "n") return known;
  let best = "";
  for (const k in NOUN_GENDER2) {
    if (k.length >= 3 && w2.length >= k.length + 2 && w2.endsWith(k) && k.length > best.length) best = k;
  }
  if (best) return NOUN_GENDER2[best];
  if (/(ung|heit|keit|schaft|tät|ion|ik|enz|anz|ei|ade|age|üre|itis|ur)$/.test(w2)) return "f";
  if (/(chen|lein|ment|tum|um|nis|ma)$/.test(w2)) return "n";
  if (/(ling|ismus|ant|ent|ist|eur|or|ich|ig|ast)$/.test(w2)) return "m";
  if (istSubstantivierterInfinitiv(w2)) return "n";
  if (/^ge[a-zäöüß]{3,}e$/.test(w2)) return "n";
  if (/e$/.test(w2) && w2.length >= 4 && !E_AUSNAHME.test(w2)) return "f";
  if (/er$/.test(w2)) return "m";
  return void 0;
}
function ensureArticle(phrase) {
  ...
  let nounIdx = words.findIndex((w2) => /^[A-ZÄÖÜ]/.test(w2));
...
function wirktFinit(w2) {
  if (w2.length < 4 || NICHT_VERB_T.has(w2)) return false;
  if (/^ge[a-zäöüß]+t$/.test(w2)) return false;
  return /^[a-zäöüß]+[^aeiouäöü]t$/.test(w2) || /^[a-zäöüß]+et$/.test(w2);
}
function looksLikeInfinitive(w2) {
  if (INFINITIVE_VERBS.has(w2)) return true;
  if (w2.length < 5 || NOT_INFINITIVE.has(w2) || NOUN_GENDER[w2]) return false;
  return /(?:[a-zäöüß]{3,})(?:en|ern|eln)$/.test(w2);
}
function extractLeadVerb(text) {
  ...
  const raw = m[1];
  const w2 = raw.toLowerCase();
  if (VERB_CONJ[w2]) return { verb: raw, rest: m[2] };
  if (/^[a-zäöüß]/.test(raw) && looksLikeInfinitive(w2)) {
    return { verb: null, rest: `${m[2]} ${w2}`, isInfinitiveLed: true };
  }
  if (/^[a-zäöüß]+iert$/.test(w2)) return { verb: raw, rest: m[2] };
  const dritte = ICH_DU_ZU_ER[w2];
  if (dritte && /^[a-zäöüß]/.test(raw)) return { verb: dritte, rest: m[2] };
  if (/^[a-zäöüß]/.test(raw) && (EXTRA_FINITE_RE.test(w2) || wirktFinit(w2))) {
...
function schwachesPraeteritum(w2, satz) {
  const l = w2.toLowerCase();
  ...
  if (inf) return true;
  const re = new RegExp("\\b" + w2 + "\\b(?=\\s+[A-Z\xC4\xD6\xDC])");
...
var weakLooksVerbal = (t) => {
  ...
  if (!m) return false;
  return m.some((w2) => schwachesPraeteritum(w2, t));
};
function isPastTense(s) {
  ...
  return (t.toLowerCase().match(/[a-zäöüß]+/g) || []).some((w2) => !!PAST2PRES[w2]);
}
...
function properNames(text) {
  ...
  for (const sent of splitSentences(text)) {
    const w2 = sent.trim().split(/\s+/);
    for (let i = 1; i < w2.length; i++) {
      const raw = w2[i].replace(/[^A-Za-zÄÖÜäöüß-]/g, "");
      ...
      const prev = (w2[i - 1] || "").replace(/[^A-Za-zÄÖÜäöüß]/g, "");
...
function toPresent(entry) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const w2 = zeichen ? roh.slice(0, -zeichen.length) : roh;
    if (!/^[A-Za-zÄÖÜäöüß]+$/.test(w2)) continue;
    ...
    if (base) {
      ...
      words[i] = (/^[A-ZÄÖÜ]/.test(w2) ? form.charAt(0).toUpperCase() + form.slice(1) : form) + zeichen;
      changed = true;
      continue;
    }
    if (/^[a-zäöüß]{4,}(te|ete)$/.test(low2)) unsure.push(w2);
  }
...
function praesensUmschreiben(entry) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const w2 = satzzeichen ? roh.slice(0, -satzzeichen.length) : roh;
    const m = w2.match(/^([a-zäöüß]{3,}?)(e?te|e?ten|e?test)$/);
    if (!m || KONJUNKTIV.test(w2)) continue;
    const stamm = m[1], endung = m[2];
    const eindeutig = /^e/.test(endung) && EINDEUTIG.test(w2);
    ...
    if (kenntInfinitiv(w2) || kenntInfinitiv(w2.replace(/e$/, "en")) || kenntInfinitiv(w2.replace(/en$/, "n"))) continue;
...
function toPresentSicher(entry) {
  ...
  for (let i = 0; i < words.length; i++) {
    const w2 = words[i];
    if (!/^[A-Za-zÄÖÜäöüß]/.test(w2)) continue;
    const konjNachAls = vorher === "als" && /^(wollte|wollten|sollte|sollten|könnte|könnten|müsste|hätte|hätten|wäre|wären|würde|würden)/i.test(w2);
    const ambig = /^(verloren|verstanden|entstanden|bestanden|erschienen)[.,;:!?]*$/i.test(w2) && !/^(wir|sie|die|alle|beide|viele|manche|einige|leute|kinder|männer|frauen)$/.test(vorher);
    const schuetzen = !erstesWort && /^[A-ZÄÖÜ]/.test(w2) || perfekt && /en[.,;:!?]*$/.test(w2) || konjNachAls || ambig;
    vorher = w2.toLowerCase().replace(/[^a-zäöüß]/g, "");
    erstesWort = false;
    if (schuetzen) {
      marker.push(w2);
  ...
  let text = r.text;
  marker.forEach((w2, k) => {
    text = text.replace(`\xA7${k}\xA7`, w2);
  });
...
var silben = (t) => {
  const w2 = t.toLowerCase().match(/[a-zäöüß]+/g) || [];
  return w2.reduce((n, x) => n + Math.max(1, (x.match(/[aeiouäöüy]+/g) || []).length), 0);
};
...
function dekliniere(phrase, kasus) {
  ...
  if (neu.toLowerCase() !== art.toLowerCase()) {
    const w2 = rest2.split(/\s+/);
    let kernIdx = w2.findIndex((x) => /^[A-ZÄÖÜ]/.test(x));
    if (kernIdx < 0) kernIdx = w2.length;
    for (let i = 0; i < kernIdx; i++) {
      const x = w2[i];
      if (/^[a-zäöüß]{3,}$/.test(x)) w2[i] = x.replace(/(?:e|er|es|em|en)$/, "") + "en";
    }
    r = w2.join(" ");
  }
  return neu + " " + r;
...
function ziehe(kandidaten, sollGewicht, bisher, phase) {
  ...
  const stems = (t) => new Set((t.toLowerCase().match(/[a-zäöüß]{5,}/g) || []).map((w2) => w2.slice(0, 5)));
...
function chooseInsertPos(sentences) {
  ...
  for (let pos = 1; pos <= sentences.length; pos++) {
    ...
    const w2 = clean(prev).split(/\s+/).filter(Boolean).length;
    candidates.push({ pos, weight: Math.max(1, w2 - 4) });
  }
...
var beatKopf = (p) => {
  const w2 = p.toLowerCase().replace(/^und\s+/, "").split(/[\s,:;—]+/).filter(Boolean);
  return w2[0] || "";
};
...
function buildDramaturgie(kit) {
  ...
  const schlag = (name, erster) => {
    ...
    switch (name) {
      ...
      case "einsatz": {
        if (benutzt.has(norm2(kit.stake))) return "";
        benutzt.add(norm2(kit.stake));
        quelleSchlag = "wortbank";
...
function kernwoerter(satz) {
  ...
  woerter3.forEach((w2, i) => {
    const m = w2.match(/^[„»(]?([A-ZÄÖÜ][a-zäöüß]{4,})/);
    if (!m) return;
...
function normWhen(s) {
  ...
  if (!one) return t;
  const w2 = one;
  if (WEEKDAYS.test(w2)) return `an einem ${cap2(w2)}`;
  if (MONTHS.test(w2) || SEASONS.test(w2)) return `im ${cap2(w2)}`;
  if (/^mitternacht$/i.test(w2)) return "um Mitternacht";
  if (/^nacht$/i.test(w2)) return "in der Nacht";
  if (/^dämmerung$/i.test(w2)) return "in der D\xE4mmerung";
  if (AM_TIMES.test(w2)) return `am ${cap2(w2)}`;
  const g = guessGender(w2);
  if (g === "f") return `in der ${cap2(w2)}`;
  if (g === "m" || g === "n") return `im ${cap2(w2)}`;
  return t;
}
function normWho(s) {
  const t = (s || "").trim();
...
function coherenceWords(s) {
  return tokenize(String(s || "").toLowerCase()).filter((w2) => w2.length > 3 && !COHERENCE_STOPWORDS.has(w2));
}
...
function polishGerman(text, opts = {}) {
  ...
  if (who.trim()) {
    const w2 = who.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const wieder = namensErsetzer(who.trim());
    try {
      t = t.replace(new RegExp(`(?<![\\p{L}\\p{N}_])${w2}(?![\\p{L}\\p{N}_])`, "giu"), wieder);
    } catch {
      t = t.replace(new RegExp(`\\b${w2}\\b`, "gi"), wieder);
    }
  }
  for (let k = 0; k < 6; k++) {
    const next = t.replace(
      /\b([A-Za-zÄÖÜäöüß]{2,})[ \t]+\1\b/gi,
      (m, w2) => DOPPELT_ERLAUBT.has(w2.toLowerCase()) ? m : w2
    );
    if (next === t) break;
    t = next;
  }
...
function applySatzlaenge(text, ziel) {
  ...
  const w2 = (x) => (x.match(/[A-Za-zÄÖÜäöüß]+/g) || []).length;
  return text.split(/\n{2,}/).map((absatz) => {
...
function hatFinitesVerbLeicht(satz) {
  return (satz.match(/[a-zäöüß]{3,}/g) || []).some((w2) => !!VERB_CONJ[w2] || /^(ist|sind|war|waren|hat|haben|wird|werden|kann|muss|will|bleibt|steht|geht|kommt)$/.test(w2));
}
...
var verbMoeglich = (w2) => /^[a-zäöüß]{2,}$/.test(w2) && !FUNKTION2.has(w2) && !KEIN_VERB.has(w2) && !/(em|er|es)$/.test(w2);
...
function coherencePass(text, input) {
  try {
    ...
    const paras = t.split(/\n{2,}/);
    const freq = {};
    coherenceWords(t).forEach((w2) => {
      freq[w2] = (freq[w2] || 0) + 1;
    });
    const motif = new Set(Object.keys(freq).filter((w2) => freq[w2] >= 2));
    [input?.who, input?.where, input?.what].forEach((s) => coherenceWords(s || "").forEach((w2) => motif.add(w2)));
    const bogen = loadDramaData();
    if (bogen) {
      for (const feld of [
        bogen.einstieg,
        ...
        for (const satz of feld || []) coherenceWords(satz).forEach((w2) => motif.add(w2));
      }
    }
    ...
    paras.forEach((p, pi) => {
      ...
      const kept = sents.filter((s, si) => {
        ...
        if (late) {
          ...
          if (cw.length >= 2 && !cw.some((w2) => motif.has(w2))) {
...
function kleinerArtikel(t) {
  return (t || "").replace(/[ \t]+([,;.!?])/g, "$1").replace(
    ...
    (_m, vor, sp, w2) => vor + sp + w2.charAt(0).toLowerCase() + w2.slice(1)
  );
}
var DU = [
  [/\btritt\b/g, "trittst"],
...
function istPluralFigur(who) {
  const w2 = (who || "").trim();
  if (!w2) return false;
  if (/^(zwei|drei|vier|fünf|sechs|sieben|acht|neun|zehn|beide|alle|viele|einige|mehrere|manche|zwölf|hundert)\b/i.test(w2)) return true;
  if (/\b(und|&)\b/.test(w2) && !/,/.test(w2)) return true;
  if (/^[A-ZÄÖÜ][a-zäöüß]+(en|innen|leute|kinder|eltern)$/.test(w2) && !/(chen|lein)$/.test(w2)) return true;
  const m = w2.match(/^die\s+([A-ZÄÖÜ][a-zäöüß-]+)$/i);
  if (m) {
...
function nomenNachAdverb(t) {
  return (t || "").replace(
    ...
    (m, vor, adv, w2) => guessGender(w2) ? `${vor}${adv} ${w2.charAt(0).toUpperCase()}${w2.slice(1)},` : m
  );
}
...
function postProcessText(txt, input) {
  ...
  t = t.replace(/\b(und|oder|aber|denn|sondern|sowie|nur|auch|selbst|sogar|erst|schon|noch|doch|nun|dann)(\s+)(die|der|das|den|dem|des|ein|eine|einen|einem|einer|sie|er|es|man|wir|ich|du|ihr|ihre|sein|seine|dann|dabei|dadurch|vielleicht|plötzlich)\b/gi, (_m, c, sp, w2) => c + sp + w2.charAt(0).toLowerCase() + w2.slice(1));
  const z = (was, f) => {
    const v = t;
    t = f(t);
...
function geschlecht(w2) {
  const kern = (w2 || "").trim().split(/\s+/).pop() || "";
...
var verbKandidat = (roh, istErstes = false) => {
  ...
  const w2 = roh.toLowerCase().replace(/[^a-zäöüß]/g, "");
  if (!w2 || FUNKTION3.has(w2) || KEIN_VERB.has(w2) || ADJEKTIV.has(w2)) return false;
  if (HILFSVERB.has(w2) || istVerbform(w2)) return true;
  return /(t|st|e|en|eln|ern|elt|ert)$/.test(w2) && !/(heit|keit|ung|schaft|tät|ment|iert)$/.test(w2) && !/(em|er|es)$/.test(w2) && w2.length >= 3;
};
var woerter2 = (s) => s.split(/\s+/).map((w2) => w2.replace(/[„“"»«().!?…;:]+/g, "")).filter(Boolean);
...
function pruefeSatz(satz) {
  ...
  const hatVerb = ws.some((w2, i) => verbKandidat(w2, i === 0));
  if (!hatVerb) {
  ...
  for (const teil of bare.split(/,\s*/).slice(1)) {
    ...
    const undIdx = tw.findIndex((w2, i) => i > 0 && /^(und|oder)$/i.test(w2));
    if (undIdx > 1 && verbKandidat(tw[undIdx + 1] || "", false) && !tw.slice(1, undIdx).some((w2) => verbKandidat(w2, false))) return 3;
  }
  ...
  for (const teil of bare.split(/,\s*/)) {
    ...
    if (tw.slice(1).some((w2) => /^zu$/i.test(w2))) continue;
    const vi = tw.findIndex((w2, i) => i > 1 && verbKandidat(w2, false));
    if (vi < 2) continue;
    if (tw.slice(1, vi).some((w2) => /^(es|er|sie|wir|ich|du|man|jemand|niemand|etwas|nichts|alles)$/i.test(w2))) continue;
...
function isSaneMarkov(s) {
  ...
  const freq = {};
  for (const w2 of words) freq[w2] = (freq[w2] || 0) + 1;
  ...
  let fn = 0;
  for (const w2 of words) if (functionWords.has(w2.toLowerCase())) fn++;
  ...
  const lw = words.map((w2) => w2.toLowerCase().replace(/[^a-zäöüß]/g, ""));
...
function smoothMarkov(s) {
  ...
  const norm2 = (w2) => w2.toLowerCase().replace(/[^a-zäöüß]/g, "");
  const dedup = [];
  for (const w2 of words) {
    const prev = dedup[dedup.length - 1];
    if (prev && norm2(prev) && norm2(prev) === norm2(w2)) continue;
    dedup.push(w2);
  }
  words = dedup;
...
function mergeWeights(a, b) {
  const out = {};
  for (const [k, w2] of Object.entries(a || {})) out[k] = (out[k] || 0) + w2;
  for (const [k, w2] of Object.entries(b || {})) out[k] = (out[k] || 0) + w2;
  return out;
}
function weightedPick(map) {
  const entries = Object.entries(map || {}).filter(([, w2]) => Number.isFinite(w2) && w2 > 0);
  if (!entries.length) return null;
  let sum = 0;
  for (const [, w2] of entries) sum += w2;
  let r = Math.random() * sum;
  for (const [k, w2] of entries) {
    r -= w2;
    if (r <= 0) return k;
  }
...
function normalizePlace(W) {
  const w2 = clean(W);
  if (!w2) return "an einem Ort";
  const n = normWhere(w2);
...
function getTraceFor(text) {
  return nachText2.get(schluessel3(text || "")) ?? [];
}
function pruefeAbgleich(endtext) {
  const norm2 = (t) => t.toLowerCase().replace(/[^a-zäöüß ]/g, " ").replace(/\s+/g, " ").trim();
  ...
  for (const s of spur) {
    ...
    if (!kern) continue;
    const w2 = kern.split(" ");
    const probe = w2.length > 4 ? w2.slice(1, -1).join(" ") : kern;
...
var traegtPerson = (t) => (t.toLowerCase().match(/[a-zäöüß]+/g) || []).some((w2) => !!ICH_DU_ZU_ER[w2]);
function buildPool(bank, perspektive, what, figur, model, markovMode) {
  const pool = [];
  let i = 0;
  const w2 = (what || "").trim();
  if (w2) {
    const lead = extractLeadVerb(w2);
...
function sachNomen(was) {
  ...
  for (let i = 0; i < woerter3.length; i++) {
    const w2 = (woerter3[i] || "").replace(/[^A-Za-zÄÖÜäöüß-]/g, "");
    if (!/^[A-ZÄÖÜ][a-zäöüß]{3,}$/.test(w2)) continue;
    if (KEIN_SACHNOMEN.test(w2)) continue;
    if (!PLURAL_ENDUNG.test(w2)) continue;
    if (EN_SINGULAR.test(w2)) continue;
...
function formeWas(roh) {
  let w2 = (roh || "").replace(/\u00ad/g, "").replace(/\u200b/g, "").replace(/\([^()]*\)/g, " ").replace(/\[[^\]]*\]/g, " ").replace(/\s+/g, " ").trim();
  const ende = w2.match(/^([\s\S]{10,}?[.!?…])\s+[A-ZÄÖÜ]/);
  if (ende && !/(?:\d|\b(?:Dr|Prof|Ing|Dipl|Nr|St|ca|bzw|usw|evtl|Abs|Art|Jh|Mio|Mrd|Bd|Hrsg|geb|gest|verh|u|z|B))\.$/.test(ende[1])) {
    w2 = ende[1];
  }
  const semi = w2.indexOf(";");
  if (semi > 12) w2 = w2.slice(0, semi);
  w2 = w2.replace(/[\s,;:–—-]+$/, "").replace(/[.!?…]+$/, "").trim();
  return kuerzeAmBruch(w2);
}
function kurzPerson(werRoh) {
  const w2 = (werRoh || "").trim().split(/\s+/).filter(Boolean).filter((x) => !/^(Dr\.|Prof\.|Ing\.|Dipl\.-?\w*\.?|med\.|jur\.|rer\.|nat\.|h\.c\.|Sir|Lady|Herr|Frau)$/i.test(x));
  if (!w2.length) return werRoh;
  const hatBegleiter = /^(der|die|das|den|dem|des|ein|eine|einen|einem|einer|eines|mein|meine|sein|seine|ihr|ihre|unser|unsere|kein|keine|jeder|jede|jedes|dieser|diese|dieses)$/i.test(w2[0]);
  const letztes = w2[w2.length - 1];
  if (hatBegleiter || w2.length > 3 || !/^[A-ZÄÖÜ]/.test(letztes)) return werRoh.trim();
  return letztes;
}
...
function istGattungsperson(haupt) {
  const w2 = haupt.trim().replace(/[^A-Za-zÄÖÜäöüß\s-]/g, "").split(/\s+/).filter(Boolean);
  const letztes = w2[w2.length - 1] || "";
  if (!letztes || !/^[A-ZÄÖÜ]/.test(letztes)) return false;
  return PERSON_NOMEN.test(letztes);
}
function istPerson(haupt) {
  let w2 = haupt.trim().split(/\s+/);
  if (istGattungsperson(haupt)) return true;
  if (/^(der|die|das|ein|eine)$/i.test(w2[0] || "")) return false;
  const mitTitel = w2.length;
  w2 = w2.filter((x) => !TITEL.test(x.replace(/[^A-Za-z.]/g, "")));
  if (w2.length === 1 && mitTitel > w2.length) return /^[A-ZÄÖÜ][a-zäöüß-]+$/.test(w2[0]);
  if (w2.length !== 2) return false;
  if (RECHTSFORM.test(w2[1].replace(/[^A-Za-z.]/g, ""))) return false;
  if (/^(FC|SV|TSV|SC|VfB|VfL|BSC|1\.)$/i.test(w2[0])) return false;
  return w2.every((x) => /^[A-ZÄÖÜ][a-zäöüß-]+$/.test(x));
}
...
function mitPraeposition(wann) {
  const w2 = (wann || "").trim();
  if (!w2) return "";
  if (/^(am|im|um|an|in|zu|seit|vor|nach|gegen|während|zwischen|beim|bis|ab)\b/i.test(w2)) return w2;
  if (/^\d{4}$/.test(w2)) return w2;
  if (ZEIT_ADVERB.test(w2)) return w2;
  if (w2.includes(",")) return w2;
  return "im " + w2;
}
...
function brauchtRahmen(satz) {
  const w2 = (satz || "").trim().split(/\s+/).filter(Boolean);
  if (w2.length === 0 || w2.length > 4) return false;
...
var WORTE = {
  sachlich: {
    ...
    haelfte: (l, w2) => `Betroffen ist damit ${l} \u2014 ${w2}.`,
  ...
  gut: {
    ...
    haelfte: (l, w2) => `${cap(l)} \u2014 ${w2} \u2014 entsteht im ersten Jahr.`,
...
function vorspann(fb, b, blick) {
  ...
  const s2 = z ? RV.vorspannFassungen ? pick(RV.vorspannFassungen(menge, blick === "gut")) : `Bekannt ${w2.vorspann(menge)}.` : `Bekannt wurde es erst sp\xE4ter.`;
  return `${s1} ${s2}`;
}
...
function hergang(fb, bank, b, benutzt, extra, vorrat, blick) {
  ...
  if (c3) teile.push(R0.schrittFassungen ? pick(R0.schrittFassungen(cap(c3.zeit), blick === "gut")) : `${cap(c3.zeit)} ${w2.schritt(b.organisation(fb))}.`);
  const a1 = fb.abgeleitet[0];
  if (a1) teile.push(w2.haelfte(a1.label, a1.wortform));
  ...
  if (eins.length) {
    ...
    teile.push(w2.einsatz(mehr, aufzaehlung(zwei.map((x) => x.t))));
  }
  ...
  if (bt.length >= 3) {
    ...
    teile.push(w2.weitere(aufzaehlung(aus)));
  }
...
function tragtEigenesSubjekt(was) {
  ...
  if (!w2) return false;
  const lead = extractLeadVerb(w2);
  return looksLikeFullClause(lead.verb, lead.rest) || /^(der|die|das|ein|eine)\s+\S+.*\b(ist|sind|war|waren|wird|werden|hat|haben|liegt|gilt|zählt|gehört|wandert|fährt|steht)\b/i.test(w2);
}
...
function tragtPraedikat(was) {
  ...
  return /\b(ist|sind|war|waren|wird|werden|wurde|wurden|hat|hatte|haben|hatten|kann|konnte|will|wollte|muss|musste|soll|sollte|darf|durfte|bleibt|blieb|steht|stand|geht|ging|kommt|kam|liegt|lag|geboren|gestorben)\b/i.test(w2) || /\b[a-zäöüß]{3,}(?:t|te|en|ten)\b\s*$/.test(w2);
}
...
function applyEmphasis(text, kit, w2) {
  const gens = [
    [w2.wo, () => placeLine(kit)],
    [w2.wann, () => timeLine(kit)],
    [w2.wer, () => charLine(kit)],
    [w2.was, () => plotLine(kit)]
  ];
  ...
  const geruest = (z) => {
    let g = z.toLowerCase();
    for (const w3 of werte2) if (w3) g = g.split(w3).join("\xA7");
  ...
  const genannt = /* @__PURE__ */ new Set();
  {
    ...
    for (const w3 of werte2) if (tl.includes(w3)) genannt.add(w3);
  }
  for (const [n, gen] of gens) {
    ...
    for (let i = 0; i < count2; i++) {
      for (let versuch = 0; versuch < 12; versuch++) {
        ...
        const dazu = werte2.filter((w3) => z.toLowerCase().includes(w3));
        if (dazu.some((w3) => genannt.has(w3))) continue;
        gesehen.add(g);
        dazu.forEach((w3) => genannt.add(w3));
        lines.push(z);
        break;
      }
    }
...
function stripDanglingTail(words) {
  ...
  let guard = 0;
  while (w2.length > 1 && REIM_DANGLING_RX.test((w2[w2.length - 1] || "").replace(/[.,;:!?…]/g, "")) && guard++ < 10) w2.pop();
  return w2;
}
...
function estimateSyllables(word) {
  const w2 = String(word || "").toLowerCase().replace(/[^a-zäöüß]/g, "");
  if (!w2) return 0;
  const clusters = w2.match(/[aeiouyäöü]+/g) || [];
...
function stanzaStems(line) {
  const out = /* @__PURE__ */ new Set();
  for (const w2 of line.toLowerCase().match(/[a-zäöüß]{4,}/g) || []) {
    if (!STANZA_STOP.has(w2)) out.add(w2.slice(0, 5));
  }
  return out;
}
...
function reimGroupOfWord(word) {
  const w2 = (word || "").toLowerCase().replace(/[.,;:!?…]/g, "");
  if (w2.length < 4) return null;
  for (const g of REIM_GROUPS) {
    if (w2.length > g.key.length && w2.endsWith(g.key)) return g;
    if (g.words.some((x) => x.toLowerCase() === w2)) return g;
  }
  return null;
}
function pickRhymeWord(group, exclude) {
  const ex = (exclude || "").toLowerCase().replace(/[.,;:!?…]/g, "");
  const options = group.words.filter((w2) => {
...
function lineWithRhyme(phrase, rhymeWord, targetWords, connector) {
  const ohneReim = String(phrase || "").replace(/\s+/g, " ").trim().split(" ").filter(Boolean).filter((w2) => w2.toLowerCase().replace(/[.,;:!?…]/g, "") !== rhymeWord.toLowerCase());
...
var darfEnden = (w2) => !KEIN_ENDE.test(w2.replace(/[^A-Za-zÄÖÜäöüß]/g, ""));
...
function passeSilben(line, ziel, syllOf) {
  ...
  if (ist2 === ziel + 1) {
    const w2 = line.split(/\s+/);
    for (let i = 0; i < w2.length; i++) {
      if (!STREICHBAR.test(w2[i])) continue;
      const rest = [...w2.slice(0, i), ...w2.slice(i + 1)];
...
var haikuSyllOf = (line) => String(line || "").split(/\s+/).filter(Boolean).reduce((a, w2) => a + estimateSyllables(w2), 0);
...
function fixHaikuCaps(line) {
  return String(line).split(/\s+/).map((w2, i) => i > 0 && HAIKU_LC.has(w2.toLowerCase()) ? w2.toLowerCase() : w2).join(" ");
}
...
// src/features/variabilitaet.ts
var VARIABILITAET_EINGEBAUT = {
  ...
  lebenreicher: 83,
  tanz: 78,
  griechischetragoedie: 82,
  glueck: 83,
  gruendungsmythos: 82,
  staatsphilosophie: 81,
  traumbilder: 83,
  mystery: 84,
  bureau: 84,
  tech: 84,
...
var KEY4 = "dm_variabilitaet_v1";
function bankHash(bank) {
  const b = bank;
  let h = 0;
  let n = 0;
...
function lade() {
  try {
    const v = JSON.parse(localStorage.getItem(KEY4) || "{}");
    return v && typeof v === "object" ? v : {};
  } catch {
    return {};
  }
}
function speichere(m) {
  try {
    localStorage.setItem(KEY4, JSON.stringify(m));
  } catch {
  }
}
function messeVariabilitaet(bank, laeufe = 8) {
  const inp = { where: "im Hafen", when: "am Abend", who: "Der Bote", what: "h\xF6rt die Glocke", tone: "neutral", varLevel: "wild", form: "prose", structure: "rekombination", mode: "myth", perspective: "third", rhythm: "auto", markovMode: "off", disruptor: "off", archetypeA: "neutral", archetypeB: "neutral", instability: 0, polish: false, polishStyle: "surreal_precise", lenTarget: 150 };
  ...
  for (let i = 0; i < L.length; i++) for (let j = i + 1; j < L.length; j++) {
    const A = new Set(L[i]), B = new Set(L[j]);
    let g = 0;
    for (const x of A) if (B.has(x)) g++;
    ov += g / Math.max(1, Math.min(A.size, B.size));
    paare++;
  }
  return Math.round((1 - ov / Math.max(1, paare)) * 100);
}
function variabilitaetFuer(id, bank) {
  const m = lade();
  const e = m[id];
  if (e && e.hash === bankHash(bank)) return e.wert;
  const kern = id.replace(/^builtin:/, "");
  if (VARIABILITAET_EINGEBAUT[kern] !== void 0 && (id.startsWith("builtin:") || !id.includes(":"))) return VARIABILITAET_EINGEBAUT[kern];
  return null;
}
function messeUndMerke(id, bank, laeufe = 8) {
...
function variabilitaetWort(w2) {
  return w2 >= 78 ? "hoch \u2014 wie die eingebauten" : w2 >= 65 ? "mittel" : w2 >= 45 ? "eher gleichf\xF6rmig \u2014 das Material kreist" : "gleichf\xF6rmig \u2014 kleines oder einseitiges Material";
}
...
wahr("die Tabelle deckt alle eingebauten Presets", Object.keys(BUILTIN_PRESETS).every((id) => VARIABILITAET_EINGEBAUT[id] !== void 0), Object.keys(BUILTIN_PRESETS).filter((id) => VARIABILITAET_EINGEBAUT[id] === void 0).join(","));
ist("eingebaut: Wert aus der Tabelle, sofort", variabilitaetFuer("builtin:kafka", BUILTIN_PRESETS["kafka"]), 82);
ist("unbekanntes eigenes Preset: null", variabilitaetFuer("user:meins", BUILTIN_PRESETS["kafka"]), null);
var w = messeUndMerke("user:meins", BUILTIN_PRESETS["kafka"], 3);
wahr("Messung liefert einen Prozentwert", w > 50 && w <= 100, String(w));
ist("gemerkt: derselbe Wert ohne Neumessung", variabilitaetFuer("user:meins", BUILTIN_PRESETS["kafka"]), w);
ist("ge\xE4nderte Bank \u2192 Hash anders \u2192 null", variabilitaetFuer("user:meins", BUILTIN_PRESETS["hafen"]), null);
wahr("Hash unterscheidet B\xE4nke", bankHash(BUILTIN_PRESETS["kafka"]) !== bankHash(BUILTIN_PRESETS["hafen"]));
wahr("Messung liegt im Band der Tabelle (\xB18 bei 6 L\xE4ufen)", Math.abs(messeVariabilitaet(BUILTIN_PRESETS["kafka"], 6) - 82) <= 8);
{
  const klein = { motifs: ["ein Brief ohne Absender", "eine Uhr ohne Zeiger", "ein Fenster ohne Glas"], hooks: ["Der Brief kommt an.", "Die Uhr bleibt stehen.", "Das Fenster ist offen."], props: ["ein Brief", "eine Uhr"], turns: ["Dann kippt es."], obstacles: ["aber niemand liest"], stakes: ["Es geht um den Brief."], endings: ["Zur\xFCck bleibt die Uhr."] };
  wahr("kleines Preset: deutlich gleichf\xF6rmiger als die eingebauten", messeVariabilitaet(klein, 6) < 40);
}
...
wahr("Klick misst sechs L\xE4ufe und merkt", /messeUndMerke\(v, pb\)/.test(q));
console.log(`Pr\xFCfstand Variabilit\xE4t \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Variabilit\xE4t: ${fails.length} Fehler:`);
  fails.forEach((f) => console.error("  - " + f));
  proc.process?.exit(1);
} else console.log(`
\u2705 Variabilit\xE4t: alle ${geprueft} Pr\xFCfungen bestanden.`);
