...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank, tone = "neutral") {
  ...
  for (let i = 0; i < shotCount; i++) {
    ...
    if (stufe >= 1 && !letzter2 && !teile.some((t) => t.startsWith("Nah: "))) teile.push(`Nah: ${nominativ(zieh(requisiten, stripTailPunct(kit.prop)))}.`);
    if (stufe >= 2 && !erster && !letzter2 && hindernisse.length && i >= Math.floor(shotCount / 3)) teile.push(`${cap(zieh(hindernisse, kit.obstacle))}.`);
