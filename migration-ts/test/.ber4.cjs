...
var gross = { motifs: [], hooks: [], props: [], turns: [], obstacles: [], stakes: [], endings: [] };
for (const id of ids.slice(0, 10)) for (const k of Object.keys(gross)) gross[k].push(...BUILTIN_PRESETS[id][k] || []);
console.log(
  "Vorrat: ein Preset ~",
  Object.keys(gross).reduce((a, k) => a + (BUILTIN_PRESETS[ids[0]][k] || []).length, 0),
  "Eintr\xE4ge \xB7 zehn Presets:",
  Object.keys(gross).reduce((a, k) => a + gross[k].length, 0)
);
for (const [name, bank] of [["ein Preset", BUILTIN_PRESETS[ids[0]]], ["zehn Presets", gross]]) {
  const zeile = [];
  for (const ziel of [220, 320, 450, 600, 800]) {
    let w = 0, n = 0;
    for (let i = 0; i < 25; i++) {
      const t = buildBericht(bank, {
      ...
      w += W(typeof t === "string" ? t : t.text);
      n++;
    }
    zeile.push(`${ziel}\u2192${Math.round(w / n)} (${Math.round(100 * w / n / ziel)}%)`);
  }
  console.log("  " + name.padEnd(14) + zeile.join("  "));
}
