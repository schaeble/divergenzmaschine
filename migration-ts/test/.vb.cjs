...
var verbMoeglich = (w2) => /^[a-zäöüß]{2,}$/.test(w2) && !FUNKTION.has(w2) && !KEIN_VERB.has(w2) && !/(em|er|es)$/.test(w2);
...
var verbKandidat = (roh, istErstes = false) => {
  ...
  if (!w2 || FUNKTION2.has(w2) || KEIN_VERB.has(w2) || ADJEKTIV.has(w2)) return false;
...
function smoothMarkov(s) {
  ...
  const norm = (w2) => w2.toLowerCase().replace(/[^a-zäöüß]/g, "");
  const dedup = [];
  for (const w2 of words) {
    const prev = dedup[dedup.length - 1];
    if (prev && norm(prev) && norm(prev) === norm(w2)) continue;
    dedup.push(w2);
  }
  words = dedup;
