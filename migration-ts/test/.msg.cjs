...
var groesse = (b) => KAT.reduce((s, k) => s + (b[k] || []).length, 0);
var ids = Object.keys(BUILTIN_PRESETS);
var phil = BUILTIN_PRESETS["philosophie"];
var median = ids.map((i) => [i, groesse(BUILTIN_PRESETS[i])]).filter(([i]) => i !== "philosophie").sort((a, b) => a[1] - b[1]);
var mittel = median[Math.floor(median.length / 2)][0];
var zehn = { motifs: [], hooks: [], props: [], turns: [], obstacles: [], stakes: [], endings: [] };
for (const id of ids.filter((x) => x !== "philosophie").slice(0, 10)) for (const k of KAT) zehn[k].push(...BUILTIN_PRESETS[id][k] || []);
var BAENKE = [
  [`${mittel} (${groesse(BUILTIN_PRESETS[mittel])})`, BUILTIN_PRESETS[mittel]],
  [`philosophie (${groesse(phil)})`, phil],
  [`zehn vereint (${groesse(zehn)})`, zehn]
];
var W = (s) => s.split(/\s+/).filter(Boolean).length;
var text = (e) => typeof e === "string" ? e : e.text;
console.log("\u2500\u2500 BERICHT: L\xE4ngentreue je Vorrat \u2500\u2500");
console.log("  Bank".padEnd(26) + [220, 320, 450, 600].map((z) => ("Ziel " + z).padStart(13)).join(""));
for (const [name, bank] of BAENKE) {
  const zeile = [220, 320, 450, 600].map((ziel) => {
    let w = 0;
    const n = 30;
    for (let i = 0; i < n; i++) w += W(text(buildBericht(bank, {
    ...
    return `${Math.round(w / n)} (${Math.round(100 * w / n / ziel)}%)`.padStart(13);
  });
  console.log("  " + name.padEnd(24) + zeile.join(""));
}
console.log("\n\u2500\u2500 PROSA (Rekombination): L\xE4ngentreue und Phrasenwiederholung \u2500\u2500");
console.log("  Bank".padEnd(26) + "Ziel 200".padStart(14) + "Ziel 400".padStart(14) + "Phrasenwdh".padStart(13));
for (const [name, bank] of BAENKE) {
  const l = [200, 400].map((ziel) => {
    let w = 0;
    const n2 = 40;
    for (let i = 0; i < n2; i++) w += W(buildStory(bank, {
      where: "im H\xF6rsaal",
      when: "am Abend",
      who: "die Denkerin",
    ...
    return `${Math.round(w / n2)} (${Math.round(100 * w / n2 / ziel)}%)`.padStart(14);
  });
  let rep = 0;
  const n = 40;
  for (let i = 0; i < n; i++) rep += phraseRepeatRatio(buildStory(bank, {
    where: "im H\xF6rsaal",
    when: "am Abend",
    who: "die Denkerin",
    what: "sucht einen Grund",
  ...
  console.log("  " + name.padEnd(24) + l.join("") + (rep / n).toFixed(3).padStart(13));
}
