...
function buildCircle(kit) {
  ...
  let t = joinBeats([a, b2, c, frameTurn(kit.turn), reframeStake(kit.stake), kit.ending], kit.P);
...
function buildObjectCentric(kit) {
  ...
  const b2 = `Ich kenne ${P2}. Ich kenne ${kit.hookAcc}.`;
...
for (let i = 0; i < 200; i++) {
  ...
  const neu = verwandleMotive(roh, paare);
  n++;
  if (neu !== roh) {
    texte++;
    const a = roh.split(/\s+/), c = neu.split(/\s+/);
    let d = 0;
    for (let k = 0; k < Math.min(a.length, c.length); k++) if (a[k] !== c[k]) d++;
    geaendert += d;
    if (!bsp) {
      const paar = paare.find((p) => roh.includes(p.von) && !neu.includes(p.von + " ")) || paare[0];
      const sr = roh.split(/(?<=[.!?…])\s+/), sn = neu.split(/(?<=[.!?…])\s+/);
      for (let k = 0; k < sr.length; k++) if (sr[k] !== sn[k]) {
        bsp = `   vorher : ${sr[k].trim()}
   nachher: ${sn[k].trim()}`;
        break;
      }
    }
...
console.log(`${n} Texte \xB7 ${texte} mit mindestens einer Verwandlung (${Math.round(100 * texte / n)} %) \xB7 ${geaendert} W\xF6rter verwandelt`);
console.log("\nBeispiel:\n" + bsp);
