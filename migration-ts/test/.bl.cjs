...
var blind = reglerListe().find((r) => r.id === "blindprobe");
console.log("\u2500\u2500 Blindprobe je L\xE4ufezahl (5 Wiederholungen) \u2500\u2500");
for (const N2 of [14, 24, 40, 60]) {
  const w = [];
  for (let k = 0; k < 5; k++) {
    const stg = blind.werte.map((x) => misseStellung(bank, basis, blind, x, N2));
    w.push(fasseZusammen("b", "B", stg).wirkung);
  }
  w.sort((a, b) => a - b);
  console.log(`  N=${String(N2).padStart(2)}  Median ${w[2].toFixed(2)}  Spanne ${w[0].toFixed(2)}\u2013${w[4].toFixed(2)}`);
}
