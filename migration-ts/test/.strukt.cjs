...
function buildBericht(bank, input, ressort = "auto") {
  ...
  const zeile2 = schlagzeile(fb);
  benutzt.add(satzSchluessel(zeile2));
  benutzt.add(satzSchluessel(fb.was));
  abschnitte.push(zeile2);
...
var STRUK = ["linear", "reverse", "circle", "fragment", "object", "rekombination"];
var W = (s) => s.split(/\s+/).filter(Boolean).length;
var proben = {};
var zeile = [];
for (const stk of STRUK) {
  let rep = 0, tem = 0, per = 0, w = 0, n = 0;
  proben[stk] = [];
  for (let i = 0; i < 120; i++) {
    ...
    tem += tenseBreakRatio(t);
    per += perspectiveBreakRatio(t, "third");
    w += W(t);
    n++;
    if (proben[stk].length < 40) proben[stk].push(t);
  }
  zeile.push(`${stk.padEnd(14)} Phrasenwiederholung ${(rep / n).toFixed(3)}  Tempusbruch ${(tem / n).toFixed(3)}  Perspektivbruch ${(per / n).toFixed(3)}  W\xF6rter ${Math.round(w / n)} (Ziel 200)`);
}
console.log("\u2500\u2500 Sechs Strukturen, je 120 Texte, alles andere festgehalten \u2500\u2500");
zeile.forEach((z) => console.log("  " + z));
var worte2 = (t) => new Set(t.toLowerCase().match(/[a-zäöüß]{4,}/g) || []);
var jac = (a, b) => {
  let s = 0;
  a.forEach((x) => {
    if (b.has(x)) s++;
...
console.log("\n\u2500\u2500 Wie \xE4hnlich klingen die Strukturen einander? (0 = fremd, 1 = gleich) \u2500\u2500");
console.log("                 " + STRUK.map((s) => s.slice(0, 6).padStart(7)).join(""));
for (const a of STRUK) {
  const row = STRUK.map((b) => {
    let sum = 0, c = 0;
    for (let i = 0; i < 20; i++) {
      const x = proben[a][i], y = proben[b][i];
      if (x && y) {
        sum += jac(worte2(x), worte2(y));
        c++;
      }
    }
    return (sum / c).toFixed(2).padStart(7);
  });
  console.log("  " + a.padEnd(14) + row.join(""));
}
var FIX = [
  /Was im Weg steht:/,
  /Du erfährst erst später:/,
  /Und dann, rückwärts betrachtet:/,
...
console.log("\n\u2500\u2500 Feste Programms\xE4tze je Text (Anteil der Texte mit mindestens einem) \u2500\u2500");
for (const stk of STRUK) {
  const mit = proben[stk].filter((t) => FIX.some((r) => r.test(t))).length;
  console.log("  " + stk.padEnd(14) + Math.round(100 * mit / proben[stk].length) + " %");
}
