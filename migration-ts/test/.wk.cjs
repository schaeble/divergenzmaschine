...
function reglerListe() {
  const s = (id, label, werte2, feld) => ({ id, label, werte: werte2, setzen: (e, w) => ({ ...e, [feld]: w }) });
  ...
  return [
    ...
    s("archetypeA", "Archetyp A", ohneAuto(ARCH_OPTS), "archetypeA"),
    {
      ...
      werte: ["0", "1", "2"],
      setzen: (e, w) => ({ ...e, instability: Number(w) })
    },
    {
...
console.log("\u2500\u2500 Welche gemessenen Stellungen gibt es wirklich? \u2500\u2500");
var ECHT = {
  tone: Object.keys(TONE_DATA),
  mode: Object.keys(MODE_DATA),
  structure: ["auto", "linear", "reverse", "circle", "fragment", "object", "dramaturgie", "rekombination"],
  perspective: ["auto", "third", "first", "second", "we", "object"],
  rhythm: ["auto", "breath", "staccato", "long", "fracture", "clean"],
  varLevel: ["low", "mid", "high"],
  markovMode: ["off", "mix", "on"],
  disruptor: ["auto", "off", "on"],
  archetypeA: ["neutral", "skorpion", "psychopath", "entdecker"],
  instability: ["0", "1", "2"]
};
for (const r of reglerListe()) {
  const echt = ECHT[r.id];
  if (!echt) {
    console.log("  " + r.label.padEnd(24) + "(Blindprobe)");
    continue;
  }
  const unbekannt = r.werte.filter((w) => !echt.includes(w));
  console.log(`  ${r.label.padEnd(24)} ${r.werte.length} gemessen, davon UNBEKANNT: ${unbekannt.length ? unbekannt.join(", ") : "\u2014"}`);
}
console.log("\n\u2500\u2500 Wirkung mit den gemessenen Stellungen (N=14) \u2500\u2500");
var N2 = 14;
for (const r of reglerListe()) {
  const st2 = r.werte.map((w) => misseStellung(bank, basis, r, w, N2));
  const m = fasseZusammen(r.id, r.label, st2);
  console.log(`  ${r.label.padEnd(24)} ${m.wirkung.toFixed(2)}  ${m.staerkstesMass || ""}`);
}
