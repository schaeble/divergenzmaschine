...
// src/text-utils.ts
function clean(s2) {
  return (s2 ?? "").toString().trim().replace(/\s+/g, " ");
}
function pick(arr) {
...
function ensureArticle(phrase) {
  const s2 = clean(phrase);
  if (/^(ein|eine|einen|einem|einer|eines|der|die|das|den|dem|des|kein|keine|mein|dein|sein|ihr|unser)\b/i.test(s2)) return s2;
  const words = s2.split(" ");
  if (words.length > 5) return s2;
  let nounIdx = words.findIndex((w) => /^[A-ZÄÖÜ]/.test(w));
  if (nounIdx === -1) return s2;
...
function declineHookPhrase(phrase, targetCase) {
  const s2 = clean(phrase);
  const m = s2.match(/^(ein|eine|einen|einem|einer|eines)\s+(.*)$/i);
  if (!m) return s2;
...
var recentMarkov = [];
var MK_KEEP = 24;
var mkNorm = (s2) => s2.toLowerCase().replace(/[^a-zäöüß ]/g, "").replace(/\s+/g, " ").trim();
function markovSeenRecently(s2) {
  const n = mkNorm(s2);
  return n.length > 0 && recentMarkov.includes(n);
}
function noteMarkov(s2) {
  const n = mkNorm(s2);
  if (!n) return;
  recentMarkov.push(n);
...
function tenseBreakRatio(text) {
  const sents = splitSentences(text).filter((s2) => s2.trim().length > 3);
  if (sents.length < 3) return 0;
  let past = 0, pres = 0;
  const tags = sents.map((s2) => {
    const isPast = PRAET_STRONG.test(s2) || PRAET_WEAK.test(s2);
    const isPres = PRAES_MARK.test(s2);
    if (isPast && !isPres) {
      past++;
      return "past";
    }
...
function isSecondPerson(s2) {
  return DU_FORM.test(s2 || "");
}
function isFirstPerson(s2) {
  return ICH_FORM.test(s2 || "");
}
function perspectiveBreakRatio(text, perspective) {
  ...
  for (const s2 of sents) {
    if (perspective !== "second" && DU_FORM.test(s2)) {
      off++;
      continue;
    }
    if (perspective !== "first" && perspective !== "we" && ICH_FORM.test(s2)) off++;
  }
...
function subjektOf(t, typ) {
  ...
  if (/\b(ich|mir|mich)\b/.test(s2)) return { person: 1, numerus: "sg", genus: null };
  if (/\b(wir|uns)\b/.test(s2)) return { person: 1, numerus: "pl", genus: null };
  if (/\b(du|dir|dich)\b/.test(s2)) return { person: 2, numerus: "sg", genus: null };
  if (/\b(ihr|euch)\b/.test(s2)) return { person: 2, numerus: "pl", genus: null };
...
function deriveAtom(raw) {
  ...
  return {
    ...
    oeffnet: typ === "kopf",
    rhythmus: { woerter: wcount, silben: s2, tiefe: tiefe(text), endzeichen: end, gewicht: wcount <= 4 ? "kurz" : wcount <= 9 ? "mittel" : "lang" },
    unsicher
  };
}
...
var KEY = "dm_knobs_v1";
var klemm = (v, s2) => Math.max(s2.min, Math.min(s2.max, v));
function loadKnobs() {
  try {
...
function ziehe(kandidaten, sollGewicht, bisher, phase) {
  ...
  const score = (a) => {
    let s2 = 1;
    if (phase) s2 += phasenBonus(a, phase);
    if (a.rhythmus.gewicht === sollGewicht) s2 += 1.5;
    const ov = [...stems(a.text)].filter((x) => kontext.has(x)).length;
    s2 += Math.min(ov, 2) * 0.8;
    if (ov > 3) s2 -= 2;
    s2 = Math.max(0.05, s2);
    if (a.quelle === "dramaturgie") s2 = bogenGewicht === 0 ? 1e-4 : s2 * bogenGewicht;
    return s2;
  };
...
function cap(s2) {
  s2 = (s2 ?? "").toString();
  return s2 ? s2[0].toUpperCase() + s2.slice(1) : s2;
}
function isFragmentSentence(s2) {
  const n = clean(s2).split(/\s+/).filter(Boolean).length;
  return n > 0 && n <= 3;
}
...
function looksLikeClausePhrase(phrase) {
  const s2 = clean(phrase);
  if (!s2) return false;
  if (/[.!?]$/.test(s2)) return true;
  const mainPart = (s2.split(",")[0] || s2).trim();
...
function weaveMotif(text, motif) {
  ...
  const s2 = splitSentences(text);
  if (s2.length < 2) return text + " " + motifLine;
  let pos = chooseInsertPos(s2);
  if (pos < 0) pos = Math.min(s2.length - 1, Math.max(1, Math.floor(s2.length * 0.55)));
  s2.splice(pos, 0, motifLine);
  return s2.join(" ");
}
...
function extractLeadVerb(text) {
  const s2 = clean(text);
  if (!s2) return { verb: null, rest: s2 };
  const m = s2.match(/^([A-Za-zÄÖÜäöüß]+)\s+(.+)$/);
  if (!m) return { verb: null, rest: s2 };
  const raw = m[1];
...
function splitSpeakers(who) {
  const parts = (who || "").split(",").map((s2) => clean(s2)).filter(Boolean);
...
var cap2 = (s2) => s2 ? s2.charAt(0).toUpperCase() + s2.slice(1) : s2;
var low = (s2) => s2 ? s2.charAt(0).toLowerCase() + s2.slice(1) : s2;
function parseNP(s2) {
  const m = s2.trim().match(/^(?:(der|die|das|ein|eine|einen|einem|einer)\s+)?(?:([a-zäöüß][a-zäöüß-]*)\s+)?([A-ZÄÖÜ][A-Za-zÄÖÜäöüß-]*)$/);
  if (!m) return null;
...
function makeDialogueScene(kit, lenTarget = 110) {
  ...
  const stripLead = (s2) => String(s2 || "").replace(/^(und|dann|aber|denn|so|doch)\s+/i, "").trim();
  ...
  const cleanDialogLine = (s2) => {
    s2 = clean(s2);
    s2 = s2.replace(/,\s*([.!?…])/g, "$1").replace(/\s*,\s*,\s*/g, ", ").replace(/„\s+/g, "\u201E").replace(/\s+"/g, '"').replace(/\.{2,}/g, ".").replace(/\s+([,.;:!?])/g, "$1").replace(/\)\s*\.$/, ")");
    return capFirst(s2);
  };
...
function coherenceWords(s2) {
  return tokenize(String(s2 || "").toLowerCase()).filter((w) => w.length > 3 && !COHERENCE_STOPWORDS.has(w));
}
...
var cap1 = (s2) => s2 ? s2[0].toUpperCase() + s2.slice(1) : s2;
...
function applyDisruptor(text, level) {
  ...
  const kinds = [
    ...
    { kind: "Wiederholung", fn: (t) => {
      const s2 = splitSentences(t);
      if (s2.length < 3) return t;
      return t + "\n\n" + s2[Math.floor(s2.length * 0.65)];
    } },
    { kind: "Fragmentierung", fn: (t) => {
      const s2 = splitSentences(t);
      if (s2.length < 4) return t;
      s2.splice(Math.floor(s2.length / 2), 0, "\u2014");
      return s2.join(" ");
    } }
  ];
  const k = pick(kinds);
...
function applyRhythm(text, rhythm) {
  ...
  const insertFrag = (prob) => {
    if (chance(prob)) {
      const pos = chooseInsertPos(s2);
      if (pos >= 0) s2.splice(pos, 0, pick(FRAGMENTS));
    }
  };
  if (rhythm === "clean") return s2.join(" ");
  if (rhythm === "breath") {
    insertFrag(0.55);
    if (s2.length >= 5 && chance(0.45)) {
      const i = Math.floor(1 + Math.random() * (s2.length - 2));
      s2[i] = "Und " + s2[i].charAt(0).toLowerCase() + s2[i].slice(1);
    }
  }
  if (rhythm === "staccato") {
    insertFrag(0.75);
    if (s2.length >= 4 && chance(0.6)) {
      const i = Math.floor(1 + Math.random() * (s2.length - 2));
      const t = s2[i];
      const cut = t.indexOf(", ");
      if (cut > 10 && cut < 80 && !NEBENSATZ_ANFANG.test(t.slice(cut + 2))) {
        s2[i] = t.slice(0, cut) + ".";
        s2.splice(i + 1, 0, t.slice(cut + 2));
      }
    }
    if (chance(0.35)) {
      const at = Math.min(2, s2.length);
      if (!isFragmentSentence(s2[at - 1] || "") && !isFragmentSentence(s2[at] || "")) s2.splice(at, 0, pick(["Stille.", "Warte.", "So.", "Gut."]));
    }
  }
  if (rhythm === "long") {
    if (s2.length >= 6 && chance(0.6)) {
      const i = Math.floor(1 + Math.random() * (s2.length - 3));
      const first = s2[i].replace(/[.!?…]+$/, "");
      const next = s2[i + 1];
      const joiner = /^(und|aber|doch|denn|sondern)\b/i.test(next) ? ", " : chance(0.5) ? ", und " : "; ";
      s2[i] = first + joiner + next.charAt(0).toLowerCase() + next.slice(1);
      s2.splice(i + 1, 1);
    }
    if (chance(0.4)) s2.push("Und w\xE4hrend all das geschieht, bleibt etwas in der Luft h\xE4ngen, als w\xE4re es nie f\xFCr Menschen gedacht gewesen.");
  }
  if (rhythm === "fracture") {
    insertFrag(0.7);
    if (s2.length >= 5 && chance(0.6)) {
      const i = Math.floor(1 + Math.random() * (s2.length - 2));
      s2[i] = s2[i].replace(/[.!?…]+$/, "") + " \u2014";
      s2.splice(i + 1, 0, "und genau dort bricht die Erkl\xE4rung ab.");
    }
    if (chance(0.45)) s2.splice(Math.floor(s2.length * 0.65), 0, "(Dieser Satz war nicht geplant.)");
  }
  return s2.join(" ");
}
...
function applyTension(text, peak, material) {
  ...
  for (let i = s2.length - 1; i >= 0; i--) {
    const it = intensity(i, s2.length);
    if (it > 0.6 && chance(it * 0.7)) {
      ...
      if (cut > 10 && cut < 90 && !unteilbar) {
        s2[i] = t.slice(0, cut) + ".";
        s2.splice(i + 1, 0, cap(rest));
      }
    }
  }
  for (let pass = 0; pass < 2; pass++) {
    const idx = Math.round(center * (s2.length - 1));
    if (idx > 0 && idx < s2.length && chance(0.55) && !isFragmentSentence(s2[idx - 1] || "") && !isFragmentSentence(s2[idx] || "")) {
      s2.splice(idx, 0, pick(FRAGMENTS));
    }
  }
  ...
  if (mat.length) {
    for (let k = 0; k < 2; k++) {
      const cand = pick(mat);
      if (!cand || s2.join(" ").toLowerCase().includes(cand.toLowerCase())) continue;
      if (!chance(0.7)) continue;
      const idx = Math.max(1, Math.min(s2.length, Math.round(center * (s2.length - 1)) + k));
      s2.splice(idx, 0, cap(cand.replace(/[.!?…]+$/, "")) + ".");
    }
  }
  {
    const idx = Math.round(center * (s2.length - 1));
    if (idx > 0 && idx < s2.length - 1 && chance(0.5)) {
      const t = s2[idx].replace(/[.!?…]+$/, "");
      if (t.length > 12 && !isFragmentSentence(t)) {
        s2[idx] = t + " \u2014";
        s2.splice(idx + 1, 0, pick(["und genau hier kippt es.", "kein Zur\xFCck.", "jetzt.", "und nichts h\xE4lt mehr."]));
      }
    }
  }
  return s2.join(" ");
...
function paragraphize(txt) {
  const s2 = splitSentences(txt);
  if (s2.length <= 3) return txt;
  const breaks = /* @__PURE__ */ new Set();
  const target = chance(0.6) ? 2 : 1;
  while (breaks.size < target) breaks.add(Math.min(s2.length - 2, Math.max(1, Math.floor(1 + Math.random() * (s2.length - 2)))));
  const out = [];
...
function applyPerspective(paras, perspective, who, objName) {
  ...
  const swap = (s2, person, pronoun) => {
    ...
    try {
      ...
      return s2.replace(new RegExp("\\b" + escapeRegExp(P2) + "\\b", "gi"), pronoun);
    }
  };
  const toFirst = (s2) => swap(s2, "ich", "ich");
  const toSecond = (s2) => swap(s2, "du", "du");
  const toWe = (s2) => swap(s2, "wir", "wir");
  const toObject = (s2) => `${pick(OBJEKT_ZWISCHENRUF)} ${s2}`;
...
function pronominalize(text, P2, pronoun) {
  ...
  return text.split(/\n\n+/).map((par) => {
    const s2 = splitSentences(par);
    for (let i = 0; i < s2.length; i++) {
      if (!re.test(s2[i])) continue;
      if (!seen) {
        seen = true;
        lastReplaced = false;
        continue;
      ...
      s2[i] = cap(pronoun) + s2[i].slice(name.length);
      lastReplaced = true;
    }
    return s2.join(" ");
  }).join("\n\n");
...
function entferneDubletten(text) {
  ...
  const ohne = text.split(/\n{2,}/).map((absatz) => {
    const s2 = splitSentences(absatz);
    if (s2.length < 2) return absatz;
    const raus = [];
    for (const satz of s2) {
      const k = kern(satz);
...
function applySatzlaenge(text, ziel) {
  ...
  return text.split(/\n{2,}/).map((absatz) => {
    let s2 = splitSentences(absatz);
    if (s2.length < 2) return absatz;
    const bleibtKurz = new Set(s2.filter(() => chance(0.2)));
    for (let runde = 0; runde < 200; runde++) {
      let beste = -1, kuerzeste = Infinity;
      for (let i = 0; i + 1 < s2.length; i++) {
        const n = w(s2[i]) + w(s2[i + 1]);
        if (n > ziel) continue;
        if (bleibtKurz.has(s2[i]) || bleibtKurz.has(s2[i + 1])) continue;
        if (!darfVerbinden(s2[i], s2[i + 1], ziel)) continue;
        if (n < kuerzeste) {
          kuerzeste = n;
          beste = i;
        }
      ...
      if (beste < 0) break;
      const satzartig = hatFinitesVerbLeicht(s2[beste]);
      s2 = [...s2.slice(0, beste), verbinde(s2[beste], s2[beste + 1], satzartig), ...s2.slice(beste + 2)];
    }
    return s2.join(" ");
  }).join("\n\n");
}
...
function coherencePass(text, input) {
  try {
    ...
    [input?.who, input?.where, input?.what].forEach((s2) => coherenceWords(s2 || "").forEach((w) => motif.add(w)));
    ...
    paras.forEach((p, pi) => {
      ...
      const kept = sents.filter((s2, si) => {
        const bare = s2.trim().replace(/["»«)\]]+$/, "").replace(/[.!?…]+$/, "").trim();
...
function coherenceRepairV2(t, input) {
  ...
  for (let s2 of sents) {
    const bare = s2.trim().replace(/["“”»«]+$/, "").replace(/[.!?…]+$/, "").trim();
    const opens = (s2.match(/„/g) || []).length, closes = (s2.match(/[“”»]/g) || []).length;
    if (/\bSatz\s+„/.test(s2) && opens > closes) continue;
    if (/,\s+(die|der|das|dem|den|des)\s+(die|der|das|dem|den|des)\s+\p{L}+$/iu.test(bare)) continue;
    if (opens > closes) s2 = s2.replace(/„\s*/g, "");
    const di = s2.search(/\bdu\b/i);
    if (di >= 0) {
      const head = s2.slice(0, di);
      let tail = s2.slice(di);
...
function corpusSanitize(text) {
  let s2 = (text ?? "").toString();
  s2 = s2.split(/\r?\n/).filter((z) => !/^\s*(SEQUENZ\s*—|(?:WER|WO|WANN|WAS|GESAMTLÄNGE)\s*:)/.test(z)).map((z) => z.replace(/^\s*(?:Shot\s*\d+\s*\([^)]*\)|(?:DE|EN)\s*:)\s*/, "")).join("\n");
  s2 = s2.replace(/\([^()]*\)/g, " ");
  s2 = s2.replace(/\b(?:gegen|um|ab|seit|bis)\s+\d{1,2}:\d{2}\b\s*(?:—|–)?\s*/gi, "");
  s2 = s2.replace(/\b\d{1,2}:\d{2}\b\s*—\s*/g, "");
  s2 = s2.replace(/\b(Schluss|Notiz|Rand|Gestern|Jetzt|Später|Drei Tage später)\s*—\s*/g, "");
  s2 = s2.replace(/\bSZENE:\s*/g, "");
  s2 = s2.split(/\r?\n/).filter((z) => !GERUEST_ZEILE.test(z)).join("\n");
  s2 = s2.replace(/Faktenkasten\s*·[^\n]*?(?:\.(?=\s+[A-ZÄÖÜ])|$)/g, " ");
  s2 = s2.replace(/—\s*(?=[.—])/g, "");
  s2 = s2.replace(/\.{2,}/g, ".");
  s2 = s2.replace(/\s+/g, " ").trim();
  return s2;
}
function isSaneMarkov(s2) {
  if (!s2 || s2.length < 20) return false;
  ...
  const sentences = s2.split(/[.!?]+/).filter(Boolean);
  ...
  if (/\b(Schluss|Notiz|Rand)\s*—|\bSZENE:|dass\s*—|,\s*dass\s*$/i.test(s2)) return false;
  if (/[—–]\s*$/.test(s2.trim())) return false;
...
function smoothMarkov(s2) {
  let words = (s2 || "").trim().split(/\s+/).filter(Boolean);
...
var pickSymbol = () => pick(["\u2297", "\u27C2", "\u27E1", "\u2301", "\u27DF", "\u27D0", "\u2736", "\u27C1"]);
var stripTailPunct = (s2) => clean(s2).replace(/[.!?…]+$/, "");
...
function bogenSaetze(d, kit) {
  const s2 = (a) => Array.isArray(a) ? a.filter(Boolean) : [];
  const P2 = kit.P;
  const fest = [];
  const einstieg = s2(d.einstieg), mitte = s2(d.mitte), hoehe = s2(d.hoehepunkt), aend = s2(d.veraenderungen);
  ...
  const frei = [];
  for (const r of s2(d.regeln)) frei.push(`Regel: ${ensurePunct(r)}`);
  for (const z of s2(d.zeitanomalien)) frei.push(ensurePunct(z));
  ...
  s2(d.konflikte).forEach((k, i) => frei.push(`${K_RAHMEN[i % K_RAHMEN.length]} ${stripTailPunct(k)}.`));
  s2(d.ausloeser).forEach((a, i) => frei.push(`${A_RAHMEN[i % A_RAHMEN.length]} ${stripTailPunct(a)}.`));
  return { fest, frei };
}
...
var count = (s2) => (s2 || "").trim().split(/\s+/).filter(Boolean).length;
...
function pruefeAbgleich(endtext) {
  ...
  abweichung = [];
  for (const s2 of spur) {
    const kern = norm(s2.text);
    if (!kern) continue;
    const w = kern.split(" ");
    const probe = w.length > 4 ? w.slice(1, -1).join(" ") : kern;
    if (probe && !ziel.includes(probe)) abweichung.push(s2.text);
  }
...
function buildRekombination(bank2, input, model) {
  ...
  for (let s2 = 0; s2 < 600; s2++) {
    ...
    const a = ziehe(kand, kurve[s2 % kurve.length], out.join(" "), phase);
    if (!a) break;
...
function satzSchluessel(s2) {
  return (s2 || "").toLowerCase().replace(/[.!?…,;:]+/g, "").replace(/\s+/g, " ").trim();
}
var istIchOderDu = (s2) => isFirstPerson(s2) || isSecondPerson(s2);
...
function satzOhneZahl(bank2, kats, benutzt, zusatz = []) {
  ...
  if (!kandidaten.length) return null;
  const s2 = pick(kandidaten);
  benutzt.add(satzSchluessel(s2));
  return s2.replace(/[.!?…]+$/, "");
}
...
var worte = (s2) => (s2.match(/[A-Za-zÄÖÜäöüß0-9][A-Za-zÄÖÜäöüß0-9.,-]*/g) || []).length;
...
var normalizeNewlines = (s2) => String(s2 || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
var capLine = (s2) => String(s2).replace(/\s+([,.;:!?])/g, "$1").replace(/^[-–—]\s*/g, "").trim();
...
function buildSyllableLine(stream, targetSyll) {
  ...
  while (stream.length) {
    const w = stream[0];
    const s2 = estimateSyllables(w);
    if (syll > 0 && syll + s2 > targetSyll + 1) break;
...
function applyReimPoem(rawText, anchorLine = "", lenTarget = 0, atome = []) {
  ...
  if (atome.length >= 6) {
    ...
    for (const s2 of splitSentences(t)) phrases.push(...String(s2).split(/[,;:—–]\s*/g).map((p) => p.trim()).filter(Boolean));
  }
...
var isActionSentence = (s2) => /\b(löscht|zerstört|brennt|bricht|entscheidet|verrät|verlässt|kippt|stürzt|reißt|schneidet|stoppt|öffnet|schließt|nimmt|gibt|dreht|setzt|zieht|drückt|schaltet|speichert|überschreibt|friert)\b/i.test(s2);
var isConcreteLossSentence = (s2) => /\b(verlier|verliert|stirbt|sterben|löscht|gelöscht|brennt|zerstört)\b/i.test(s2) && /\b(mich|dich|ihn|sie|es|uns|euch|ihre|seine|foto|speicher|name|körper|gegenwart|stimme)\b/i.test(s2);
var isDecisionSentence = (s2) => /\b(also|darum|deshalb|ich entscheide|ich wähle|ich tue es|ich lasse|ich stoppe|ich öffne|ich schließe|wir entscheiden|wir lassen|ich weigere)\b/i.test(s2);
var isDisturbanceSentence = (s2) => /\b(plötzlich|dann kippt|kippt|störung|fehler|alarm|rauschen|knackt|springt|unterbricht|glitch|friert ein|rückwärts)\b/i.test(s2);
var isToneLine = (s2) => /\b(riecht|kälte|blaues licht|atem|stille)\b/i.test(s2) && !isActionSentence(s2);
...
function scoreStrangLine(l) {
  let s2 = 0;
  if (isActionSentence(l)) s2 += 3;
  if (isConcreteLossSentence(l)) s2 += 3;
  if (isDecisionSentence(l)) s2 += 2;
  if (/\b(aber|wenn|dann)\b/i.test(l)) s2 += 1;
  if (l.length > 70) s2 -= 1;
  if (/(paradoxon|omen|inkonsistenz|oberfläche)/i.test(l) && !isActionSentence(l)) s2 -= 2;
  return s2;
}
function smoothStrangLine(line) {
  let s2 = String(line || "").trim().replace(/\s+/g, " ").replace(/\s+([,.!?;:])/g, "$1").trim();
  s2 = s2.replace(/\bWir\s+will\b/gi, "Wir wollen").replace(/\bDu\s+will\b/gi, "Du willst").replace(/\bIch\s+wird\b/gi, "Ich werde").replace(/\bWir\s+nimmt\b/gi, "Wir nehmen").replace(/\bWir\s+hält\b/gi, "Wir halten").replace(/\bDer\s+Namen\b/gi, "Die Namen");
  if (/^[a-zäöüß]/.test(s2)) s2 = s2.charAt(0).toUpperCase() + s2.slice(1);
  return s2.trim();
}
...
var toSentences = (text) => normalizeNewlines(text).replace(/\s+/g, " ").split(/(?<=[.!?])\s+/).map((s2) => s2.trim()).filter(Boolean);
...
function scoreUnit(u, c) {
  const low2 = u.toLowerCase();
  let s2 = 0;
  if (isActionSentence(u)) s2 += 3;
  if (isConcreteLossSentence(u)) s2 += 3;
  if (isDecisionSentence(u)) s2 += 4;
  if (isDisturbanceSentence(u)) s2 += 2;
  if (low2.includes("aber")) s2 += 1;
  if (low2.includes("wenn")) s2 += 1;
  if (c.whoA && low2.includes(c.whoA.toLowerCase())) s2 += 1;
  if (c.whoB && low2.includes(c.whoB.toLowerCase())) s2 += 1;
  if (/(paradoxon|omen|inkonsistenz|oberfläche|bedeutung)/i.test(u) && !isActionSentence(u)) s2 -= 2;
  if (u.length > 180) s2 -= 1;
  return s2;
}
...
function asProsePoem(text) {
  const s2 = text.replace(/\s+/g, " ").split(/(?<=[.!?…])\s+/).filter(Boolean);
  const lines = [];
  for (let i = 0; i < s2.length; i++) {
    lines.push(s2[i]);
    if ((i + 1) % 2 === 0 && i < s2.length - 1) lines.push("");
  }
  return lines.join("\n");
}
...
function buildStory(bank2, input, model) {
  ...
  if (input.form === "bericht") return buildBericht(bank2, input, input.ressort ?? "auto").text;
  ...
  return entferneDubletten(enforceWordTarget(finalText, lenTarget, bank2, model, input.markovMode || "mix"));
}

// src/generation/scoring.ts
function splitSentences2(raw) {
  return raw.replace(/\s+/g, " ").trim().split(/(?<=[.!?…])\s+/).filter((s2) => s2.trim().length > 0);
}
...
function repetitionRatio(txt) {
  const tokens2 = (txt || "").toLowerCase().match(/[a-zäöüßA-ZÄÖÜ]+/g) || [];
...
function flowMetrics(txt) {
  ...
  const s2 = splitSentences2(raw);
  if (!s2.length) return { startMonotony: 0, colonExcess: 0, fragPairs: 0 };
  let same = 0, fragPairs = 0;
  for (let i = 1; i < s2.length; i++) {
    const a = (s2[i - 1].split(/\s+/)[0] || "").toLowerCase();
    const b = (s2[i].split(/\s+/)[0] || "").toLowerCase();
    if (a && a === b) same++;
    if (isFragmentSentence(s2[i - 1]) && isFragmentSentence(s2[i])) fragPairs++;
  }
  const colons = (raw.match(/:/g) || []).length;
  return { startMonotony: same / Math.max(1, s2.length - 1), colonExcess: Math.min(1, Math.max(0, colons - 2) / 3), fragPairs: Math.min(1, fragPairs / 2) };
}
function analyzeText(txt, lenTarget) {
  ...
  const sentLens = splitSentences2(raw).map((s2) => (s2.toLowerCase().match(/[a-zäöüßA-ZÄÖÜ]+/g) || []).length);
...
function misseStellung(bank2, basis2, def, wert, n, model) {
  const e = def.setzen({ ...basis2 }, wert);
  const reihen = {};
  for (let i = 0; i < n; i++) {
    let text = "";
    try {
      text = buildStory(bank2, e, model);
    } catch {
      text = "";
    }
    if (!text) continue;
...
function fasseZusammen(id, label, stellungen) {
  ...
  const n = Math.max(1, mittel(stellungen.map((s2) => s2.n)));
  const zufall = spannErwartung(k) / Math.sqrt(n);
  let best = 0, bestName = "";
  for (const { name } of MASSE) {
    const mittelwerte = stellungen.map((s2) => s2.mittel[name] ?? 0);
    const rauschen = mittel(stellungen.map((s2) => s2.sigma[name] ?? 0));
...
var bank = BUILTIN_PRESETS[Object.keys(BUILTIN_PRESETS)[0]];
var basis = {
  where: "im Archiv",
  when: "am Morgen",
  who: "die Archivarin",
...
var s = (id, label, werte, feld) => ({ id, label, werte, setzen: (e, w) => ({ ...e, [feld]: w }) });
var RICHTIG = [
  s("tone", "Ton", ["neutral", "dark", "uplifting", "melancholisch", "ironisch", "humorous"], "tone"),
  ...
  s("markovMode", "Markov", ["off", "mix", "on"], "markovMode"),
  s("disruptor", "Disruptor", ["off", "on"], "disruptor"),
  s("archetypeA", "Archetyp A", ["neutral", "skorpion", "psychopath", "entdecker"], "archetypeA"),
  s("structure", "Struktur", ["linear", "reverse", "circle", "fragment", "object"], "structure"),
  { id: "instability", label: "Instabilit\xE4t", werte: ["0", "1", "2"], setzen: (e, w) => ({ ...e, instability: Number(w) }) },
  { id: "blind", label: "Blindprobe", werte: ["a", "b", "c"], setzen: (e) => ({ ...e }) }
];
for (const N2 of [14, 40]) {
  console.log(`
\u2500\u2500 Mit den RICHTIGEN Stellungen, N=${N2} \u2500\u2500`);
  for (const r of RICHTIG) {
    const stg = r.werte.map((w) => misseStellung(bank, basis, r, w, N2));
    const m = fasseZusammen(r.id, r.label, stg);
    console.log(`  ${r.label.padEnd(16)} ${m.wirkung.toFixed(2).padStart(6)}  ${m.staerkstesMass}`);
  }
}
