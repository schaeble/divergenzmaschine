...
function buildRekombination(bank, input, model) {
  ...
  for (let s = 0; s < 600; s++) {
    ...
    kand = kand.filter((a2) => !a2.stelle || (a2.stelle === "anfang" ? fortschritt < 0.3 : fortschritt > 0.7));
    if (!kand.length) {
      if (!nachlegen()) break;
      continue;
    }
...
function buildKit(bank, input, model) {
  ...
  if (input.rhythm === "auto") {
    const tr2 = toneRhythm(input.tone);
    if (tr2 && RHYTHMS.includes(tr2) && chance(0.7)) rhythm = tr2;
  }
...
for (let i = 0; i < 200; i++) {
  const t = buildStory(BUILTIN_PRESETS[ids[i % ids.length]], {
    ...
    cast: "auto",
    mode: "bureau",
    structure: ["linear", "reverse", "circle", "fragment", "object", "rekombination"][i % 6],
    perspective: "third",
    rhythm: "clean",
    disruptor: "off",
    instability: 0,
  ...
  n++;
  const m = t.match(/\b(Nur|Auch|Selbst|Sogar|Erst|Schon|Noch|Dann|Doch|Nun)\s+(Der|Die|Das|Den|Dem|Des|Ein|Eine|Einen|Einem|Einer|Sie|Er|Es|Ich|Du|Wir|Man)\b/g);
  if (m) {
    tr += m.length;
    if (bsp.length < 3) bsp.push(m[0]);
  }
}
console.log(`Adverb + gro\xDFgeschriebener Artikel: ${tr} F\xE4lle in ${n} Texten \xB7 ${bsp.join(" | ")}`);
