...
var dubletten = (t) => {
  const s = t.toLowerCase().split(/(?<=[.!?…])\s+/).map((x) => x.replace(/[^a-zäöüß ]/g, "").trim()).filter((x) => x.split(" ").length >= 4);
  return s.length - new Set(s).size;
};
var mess = (bank, lt, structure, n = 6) => {
  let w = 0, d = 0;
  for (let i = 0; i < n; i++) {
    const t = buildStory(bank, { ...inp, lenTarget: lt, structure });
    w += t.split(/\s+/).length;
    d += dubletten(t);
  }
  return { woerter: w / n, dubl: d / n };
};
var kafka = BUILTIN_PRESETS["kafka"];
for (const st of ["linear", "rekombination", "dramaturgie"]) {
  ...
  wahr(`${st} bei 400 ohne Dubletten (h\xF6chstens 1 im Schnitt)`, m.dubl <= 1, m.dubl.toFixed(1));
}
{
  const m = mess(DEFAULT_BANK, 400, "dramaturgie");
  wahr("kleine Bank: lieber k\xFCrzer als wiederholt", m.dubl <= 1, `${m.woerter.toFixed(0)} W\xF6rter, ${m.dubl.toFixed(1)} Dubletten`);
}
{
  ...
  const qw = (0, import_fs.readFileSync)("src/features/wuerfeln.ts", "utf8");
  wahr("der W\xFCrfel kennt die 400", /schluessel: "lenTarget", min: 40, max: 400/.test(qw));
  const ql = (0, import_fs.readFileSync)("src/generation/length.ts", "utf8");
  wahr("der F\xFCller h\xF6rt auf, statt zu wiederholen", /if \(!fresh\.length\) return null;/.test(ql));
}
console.log(`Pr\xFCfstand Textl\xE4nge \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Textl\xE4nge: ${fails.length} Fehler:`);
  fails.forEach((f) => console.error("  - " + f));
  proc.process?.exit(1);
} else console.log(`
\u2705 Textl\xE4nge: alle ${geprueft} Pr\xFCfungen bestanden.`);
