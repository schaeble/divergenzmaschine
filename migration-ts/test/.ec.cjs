...
function zeitlupeStufe(name, text2) {
  if (!an) return;
  const st = { name, text: String(text2 || ""), kurz: STUFEN_ERKLAERUNG[name] || "" };
...
function zeitlupeLesen(text2) {
  if (text2 === void 0) return stufen;
  return aufzeichnungen.get(schluessel(text2)) || [];
}
...
function praesensUmschreiben(entry) {
  ...
  const ok = !isPastTense(text2) && unklar === 0;
  if (ok && changed) zaehle("umgeschrieben", `${entry} \u2192 ${text2}`);
  else if (!ok && unklar) zaehle("unklar", entry);
  else if (!ok) zaehle("praeteritumVerworfen", entry);
  return { text: text2, ok, changed };
}
function toPresentSicher(entry) {
  ...
  let text2 = r.text;
  marker.forEach((w, k) => {
    text2 = text2.replace(`\xA7${k}\xA7`, w);
  });
  return { text: text2, changed: r.changed, unsure: r.unsure };
}
...
function atomisiere(text2, max) {
  const teile = atomisiereRoh(text2, max);
  const t = trimSatz(text2 || "");
  if (t && max >= 6 && wc(t) > max) {
...
function atomisiereRoh(text2, max) {
  const t = trimSatz(text2 || "");
  if (!t) return [];
...
function ueberlaenge(text2, max) {
  if (!max || max < 6) return 0;
  return Math.max(0, wc(text2) - max);
}
...
function setzeEchos(text2, bank2, maxEchos = 2) {
  const saetze = splitSentences(text2.replace(/\n\n+/g, " "));
  const n = saetze.length;
  if (n < 8) return { text: text2, echos: [] };
  const drinnen = new Set(saetze.map(norm));
  const vorrat = [...bank2.motifs || [], ...bank2.hooks || [], ...bank2.turns || []].map((x) => x.trim()).filter((x) => x && !drinnen.has(norm(x)));
  ...
  if (!kandidaten.length) return { text: text2, echos: [] };
  ...
  for (const g of gewaehlt) {
    ...
    let e2 = g.echo.trim().replace(/^[a-z]/, (c) => c.toUpperCase());
    if (!/[.!?…]$/.test(e2)) e2 += ".";
    out.splice(pos, 0, e2);
    echos.push({ kern: g.kern, kernbild: saetze[g.i], echo: e2, position: pos });
    zaehle("echoGesetzt", `${saetze[g.i].slice(0, 50)} \u2192 ${e2.slice(0, 60)}`);
...
function ladeKurve() {
  try {
    const v = JSON.parse(localStorage.getItem(KEY3) || "null");
...
function applyToneRegister(text2, tone) {
  ...
  if (reg === "plain") {
    let t = text2.replace(/\b(gleichsam|wie Honig im Winter|wie ein halb vergessenes Gedicht[^.,;]*)\b/gi, "").replace(/\s{2,}/g, " ");
    t = t.split(/\n\n+/).map((para) => {
...
function applyDisruptor(text2, level) {
  ...
  if (!chance(p)) return { text: text2, fired: false, kind: "\u2013" };
  const kinds = [
...
function applyTension(text2, peak, material, kurve) {
  if (!kurve && (!peak || peak === "off")) return text2;
...
function coherencePass(text2, input) {
  try {
    ...
    const t = String(text2 || "").replace(/\.\s*\.+/g, ".");
    const paras = t.split(/\n{2,}/);
    const freq = {};
...
function buildObjectCentric(kit) {
  ...
  const e2 = kit.AisClause ? `${P2} sp\xFCrt: ${kit.Apure}. ${kit.obstacle}.` : `${P2} ${kit.AleadVerb || "will"} ${kit.Apure}. ${kit.obstacle}.`;
...
function stueckPlausibel(text2) {
  ...
  zaehle("angenommen", Math.random() < 0.1 ? text2 : void 0);
  return true;
}
...
function buildVideoShots(kit, shotCount, lenTarget = 0, bank2, tone = "neutral") {
  ...
  const bilder = reihenfolge([...s(bank2?.motifs), ...s(bogen?.mitte)]);
  const mitFolge = !!(bogen?.folge && bogen.folge.length);
  const bewegungen = reihenfolge([...s(bank2?.hooks), ...s(bank2?.turns), ...mitFolge ? [] : s(bogen?.veraenderungen)]);
  const requisiten = reihenfolge([...s(bank2?.props), ...s(bogen?.ausloeser)].filter(istNP));
  const hindernisse = reihenfolge([...s(bank2?.obstacles)]);
  ...
  const setze = (teile, text2) => {
    if (text2 && text2.replace(/[^A-Za-zÄÖÜäöüß]/g, "").length > 2) teile.push(text2);
  };
...
function buildVideoSequenceText(kit, shotCount = 5, totalSec = 15, lenTarget = 0, bank2, tone = "neutral") {
  const n = clampShotCount(shotCount);
  const total = clampTotalSec(totalSec);
  const dur = total / n;
  const { shots, kamera } = buildVideoShots(kit, n, lenTarget, bank2, tone);
...
function enforceWordTarget(text2, target, bank2, model, markovMode = "mix") {
  const t0 = (text2 || "").trim();
  if (!t0) return t0;
  const tol = 10;
  let out = t0;
  ...
  const nominativ = (x) => x.replace(/^einen\s/i, (m) => m[0] === "E" ? "Ein " : "ein ").replace(/^den\s/i, (m) => m[0] === "D" ? "Der " : "der ");
  const endungen = new Set((bank2.endings || []).map((e2) => clean(e2).toLowerCase().replace(/[.!?…]+$/, "")));
  ...
  const addition = (davor) => {
    ...
    const cands = [...bank2.motifs || [], ...bank2.turns || [], ...bank2.hooks || [], ...bank2.obstacles || [], ...(bank2.props || []).map(nominativ)];
...
function buildStory(bank2, input, model) {
  ...
  if (input.form === "video") {
    return postProcessText(buildVideoSequenceText(kit, input.shots ?? 5, input.totalSec ?? 15, lenTarget, bank2, input.tone), input);
  }
  if (input.form === "poem") {
  ...
  zeitlupeStufe("Bau", text2);
  if (input.form === "prose" && kit.cast.length >= 2) {
    text2 = weaveCast(text2, kit.P, kit.cast);
    zeitlupeStufe("Ensemble", text2);
  }
  if (input.form === "prose" && input.emphasis) {
    text2 = applyEmphasis(text2, kit, input.emphasis);
    zeitlupeStufe("Betonung", text2);
  }
  if (input.form === "prose") {
    const e2 = setzeEchos(text2, bank2);
    if (e2.echos.length) {
      text2 = e2.text;
      zeitlupeStufe("Echo", text2);
    }
  }
  text2 = applyDisruptor(text2, input.disruptor).text;
  zeitlupeStufe("St\xF6rung", text2);
  text2 = applyRhythm(text2, kit.rhythm);
  zeitlupeStufe("Rhythmus", text2);
  const kurve = ladeKurve();
  if (input.form === "prose") text2 = kurve.an ? applyTension(text2, input.tension, { motifs: bank2.motifs, hooks: bank2.hooks }, (p) => kurveWert(kurve.werte, p)) : applyTension(text2, input.tension, { motifs: bank2.motifs, hooks: bank2.hooks });
  zeitlupeStufe("Spannung", text2);
  ...
  zeitlupeStufe("Perspektive", text2);
  ...
  const gefuellt = entferneDubletten(enforceWordTarget(finalText, lenTarget, bank2, model, input.markovMode || "mix"));
  zeitlupeStufe("Auff\xFCllen", gefuellt);
  const verwandelt = verwandleMotive(gefuellt, leseVerwandlungen(bank2.verwandlungen));
...
wahr("Kernw\xF6rter: Nomen ab f\xFCnf Buchstaben, nicht am Satzanfang", [...kernwoerter("Im Spiegel bewegt sich etwas eine Sekunde zu sp\xE4t.")].join(",") === "spieg,sekun");
wahr("Satzanfang und Funktionsw\xF6rter z\xE4hlen nicht", !kernwoerter("Dann bleibt eine Tasche stehen.").has("dann") && kernwoerter("Dann bleibt eine Tasche stehen.").has("tasch"));
var bank = { motifs: ["Der Spiegel zeigt eine Bewegung, die niemand im Raum macht", "ein Spiegel aus altem Glas", "die Uhr ohne sichtbare Zeiger", "eine Uhr, die schneller l\xE4uft, als jemand z\xE4hlen kann"], hooks: [], props: [], turns: [], obstacles: [], stakes: [], endings: ["Nur der Nebel, der mir den R\xFCcken deckt."] };
var text = "Das Licht f\xE4llt so, dass Worte fast \xFCberfl\xFCssig werden. Im Spiegel bewegt sich etwas eine Sekunde zu sp\xE4t. Ebi beobachtet den Raum. Eine Tasche bleibt stehen. Der Bahnsteig atmet Beton und kaltes Metall. Ich warte, wie man an einem Ort wartet. Ein Tonbandger\xE4t liegt darin, vergessen von einem Handwerker. Die Melodie wiederholt sich, aber die Worte \xE4ndern sich. Der Ort merkt sich jede Bewegung. Nur der Nebel, der mir den R\xFCcken deckt.";
setDramaData({ einstieg: [], mitte: [], hoehepunkt: ["Die Melodie wiederholt sich, aber die Worte \xE4ndern sich"], schluss: [], ausloeser: [], veraenderungen: [], konflikte: [], zeitanomalien: [], regeln: [] });
var e = setzeEchos(text, bank);
ist("ein Echo gesetzt", e.echos.length, 1);
ist("der Kern ist der Spiegel", e.echos[0].kern, "spieg");
ist("das st\xE4rkere Bild (Steigerungswort \u201Eniemand\u201C) wird gew\xE4hlt", e.echos[0].echo, "Der Spiegel zeigt eine Bewegung, die niemand im Raum macht.");
{
  ...
  ist("das Echo steht unmittelbar vor dem H\xF6hepunkt", iH - iE, 1);
  wahr("mindestens drei S\xE4tze nach dem Kernbild", iE - iK >= 3);
  wahr("der Schluss bleibt der Schluss", /Nur der Nebel, der mir den Rücken deckt\.$/.test(e.text));
  wahr("sonst ist der Text unver\xE4ndert", s.length === text.split(/(?<=[.!?…])\s+/).length + 1);
}
setDramaData(null);
ist("ohne H\xF6hepunkt: bei drei Vierteln", (() => {
  const r = setzeEchos(text, bank);
  const s = r.text.split(/(?<=[.!?…])\s+/);
  const iE = s.findIndex((x) => /Der Spiegel zeigt/.test(x));
  return iE >= Math.floor(s.length * 0.6) && iE < s.length - 2;
})(), true);
ist("zu kurz: kein Echo", setzeEchos("Ein Satz. Noch einer. Der Spiegel steht.", bank).echos.length, 0);
ist("kein zweites Bild im Preset: kein Echo", setzeEchos(text, { ...bank, motifs: ["ein Fenster ohne Glas"] }).echos.length, 0);
ist("Echo steht nicht schon im Text", setzeEchos(text + " Der Spiegel zeigt eine Bewegung, die niemand im Raum macht.", { ...bank, motifs: ["Der Spiegel zeigt eine Bewegung, die niemand im Raum macht"] }).echos.length, 0);
{
  ...
  let mitEcho = 0;
  for (let i = 0; i < 6; i++) {
    const t = buildStory(BUILTIN_PRESETS["kafka"], inp);
    if (zeitlupeLesen(t).some((x) => x.name === "Echo")) mitEcho++;
  }
  zeitlupeSchalten(false);
  wahr("im Zusammenbau setzt die Maschine meistens ein Echo (6 L\xE4ufe)", mitEcho >= 3, String(mitEcho));
  const q = (0, import_fs.readFileSync)("src/generation/buildStory.ts", "utf8");
  wahr("das Echo sitzt an beiden Wegen vor der St\xF6rung", (q.match(/setzeEchos\(/g) || []).length === 2 && /setzeEchos\(rk, bank\)/.test(q));
}
console.log(`Pr\xFCfstand Echo \u2014 ${geprueft} Pr\xFCfungen, ${bestanden} bestanden`);
var proc = globalThis;
if (fails.length) {
  console.error(`
\u274C Echo: ${fails.length} Fehler:`);
  fails.forEach((f) => console.error("  - " + f));
  proc.process?.exit(1);
} else console.log(`
\u2705 Echo: alle ${geprueft} Pr\xFCfungen bestanden.`);
