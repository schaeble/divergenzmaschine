...
var KEY, BEISPIELE_JE, cache, schreibTimer;
...
var SCHWACH_KONSONANT, SCHWACH_E;
var init_assemble = __esm({
  "src/atoms/assemble.ts"() {
    ...
    init_knobs();
    init_atomisieren();
    SCHWACH_KONSONANT = /^(Herr|Mensch|Held|Fürst|Prinz|Graf|Bär|Elefant|Nachbar|Bauer|Herz|Narr|Tor|Christ|Zar|Architekt|Soldat|Advokat|Kamerad|Katholik|Ochs|Spatz|Fink|Pfau|Ahn)$/;
    SCHWACH_E = /^(Hase|Junge|Kollege|Zeuge|Bote|Erbe|Riese|Löwe|Affe|Rabe|Neffe|Kunde|Gefährte|Experte|Komplize|Insasse|Gatte|Bube|Falke|Franzose|Schwede|Türke|Russe|Pole|Däne|Ire|Brite|Jude|Sklave|Ahne|Zeuge)$/;
  }
});
...
var tone_data_exports = {};
__export(tone_data_exports, {
  TONE_DATA: () => TONE_DATA
});
var TONE_DATA;
...
var LINE_FORMS, isLineForm, ABGESCHNITTEN, NUR_OHNE_VERB, NEBENSATZ_ENDE, FUNKTION, verbMoeglich, DU, NEBENSATZ;
var init_postprocess = __esm({
  "src/generation/postprocess.ts"() {
    ...
    verbMoeglich = (w) => /^[a-zäöüß]{2,}$/.test(w) && !FUNKTION.has(w) && !KEIN_VERB.has(w) && !/(em|er|es)$/.test(w);
    DU = [
      [/\btritt\b/g, "trittst"],
      [/\bhält\b/g, "h\xE4ltst"],
      [/\bnimmt\b/g, "nimmst"],
...
{
  const { postProcessText: pp } = (init_postprocess(), __toCommonJS(postprocess_exports));
  const { TONE_DATA: TD } = (init_tone_data(), __toCommonJS(tone_data_exports));
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const f of TD["uplifting"].flavor) {
      const k = f.toLowerCase().replace(/[.!?…]+$/, "");
      if (t.split(k).length - 1 > 1) {
        doppelt++;
        break;
      }
    }
