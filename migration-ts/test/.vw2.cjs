...
var ZIEL = paare.map((p) => p.nach);
var zaehl = (t, w) => (t.match(new RegExp("(^|[^A-Za-z\xC4\xD6\xDC\xE4\xF6\xFC\xDF])" + w + "(?![A-Za-z\xC4\xD6\xDC\xE4\xF6\xFC\xDF])", "gi")) || []).length;
var inp = (i) => ({
  where: "im H\xF6rsaal",
  when: "am Abend",
  who: "die Denkerin",
...
var texteOhne = 0;
for (let i = 0; i < 200; i++) {
  const a = buildStory(b, inp(i));
  const c = buildStory(ohne, inp(i));
  const za = ZIEL.reduce((s, w) => s + zaehl(a, w), 0);
  const zc = ZIEL.reduce((s, w) => s + zaehl(c, w), 0);
  mitZ += za;
  ohneZ += zc;
  if (za) texteMit++;
  if (zc) texteOhne++;
}
console.log(`Zielw\xF6rter (${ZIEL.join(", ")}):`);
console.log(`  MIT Verwandlungen : ${mitZ} Vorkommen in ${texteMit} von 200 Texten`);
console.log(`  OHNE              : ${ohneZ} Vorkommen in ${texteOhne} von 200 Texten`);
console.log(`  Zuwachs           : ${mitZ - ohneZ} Verwandlungen`);
for (let i = 0; i < 40; i++) {
  const t = buildStory(b, inp(i));
  const tr = paare.find((p) => zaehl(t, p.nach) > 0 && zaehl(t, p.von) > 0);
  if (tr) {
    const s = t.split(/(?<=[.!?…])\s+/).filter((x) => x.includes(tr.von) || x.includes(tr.nach));
    console.log(`
Beispiel (${tr.von}\u2192${tr.nach}):`);
    s.slice(0, 3).forEach((x) => console.log("   " + x.trim()));
    break;
  }
}
