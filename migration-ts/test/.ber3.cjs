...
console.log("\u2500\u2500 H\xE4lt der Bericht seine Ziell\xE4nge? \u2500\u2500");
for (const ziel of [80, 140, 220, 320, 450, 600]) {
  const je = {};
  for (const r of RESSORT_IDS) {
    let w = 0, n = 0;
    for (let i = 0; i < 25; i++) {
      const t = buildBericht(BUILTIN_PRESETS[ids[i % ids.length]], {
  ...
  const alle = Object.values(je);
  console.log(`  Ziel ${String(ziel).padStart(3)} \u2192 ${Math.round(alle.reduce((a, b) => a + b, 0) / alle.length)} W\xF6rter im Schnitt (${Math.round(100 * alle.reduce((a, b) => a + b, 0) / alle.length / ziel)} %)   Wetter ${je["wetter"]}, Wirtschaft ${je["wirtschaft"]}, Sport ${je["sport"]}`);
}
