...
var PROBEN = [
  "Faktenkasten \xB7 Betroffen: 3.840 Haushalte \xB7 Dauer: 50 Stunden \xB7 Volumen: 86 Millionen Euro Umsatz.",
  "Gegen 00:39 \u2014 und der Blick blieb.",
  "Zeitzeichen \xB7 Nr. 44 \xB7 Freitag, 21. August 2026",
  "Fiktive Zeitung \xB7 maschinell erzeugt",
  "Kurz gemeldet",
  "Die T\xFCr steht offen und niemand geht hindurch."
];
for (const p of PROBEN) {
  const rein = corpusSanitize(p);
  const hyg = corpusHygiene(p).text;
  console.log((GERUESTZEILE.test(p) ? "Ger\xFCst " : "durch  ") + JSON.stringify(p.slice(0, 52)));
  console.log("        Reinigung: " + JSON.stringify(rein.slice(0, 70)));
  console.log("        Hygiene  : " + JSON.stringify(hyg.slice(0, 70)));
}
